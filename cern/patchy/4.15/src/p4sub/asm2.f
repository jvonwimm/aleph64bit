...
C CERN PROGLIB#         FCASPLIT        .VERSION KERNFOR  4.30  910819
C ORIG. 01/10/88  JZ
      ...
      CHARACTER    CHLINE*80, CHNAME*80, CHTEXT*100
      CHARACTER    CHUSE*1
 ...
 9002 FORMAT (
     F ' FCASPLIT   [-f  nmft] [-c  nmcc] [-a  nmas]   [-noh]'
     F/'            [+fo incf] [+co incc] [+ao incs]'
     F/'            [-fo optf] [-co optc] [-ao opts]'
     F/'     f.e [optx] [optf      [optc      [opts]]]'/
...
C--                Open input and .shfca file
      ...
      OPEN (21, FILE=CHFSH(1:NCHFSH),STATUS=CHOVER)
...
C--       Open the output file if first line ready
   ...
   44 OPEN  (27, FILE=CHNAME(1:NCUM),STATUS=CHOVER)
      REWIND 27

      NTEXT = 0
      NROUT = NROUT + 1
      PRINT 9041, NROUT,CHNAME(1:NCUM)
 9041 FORMAT (' make',I4,1X,A)
...
C----              Done
   ...
   84 WRITE (22,8000) CHMKLN(1:NXMKLN-1)
...
C--       Fortran inference
     ...
     +            '$*' // CHEXFOR
      ...
      WRITE (22,8003) CHNAME(1:NPUT), CHTEXT(1:NTEXT)
