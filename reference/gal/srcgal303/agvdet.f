...
C
      ...
      IYEAR = VDYEAR ()
C
C - position faces depending on the setup code and the VDET91 or VDET95
C
      IF (IVDST.LT.3) THEN
         CALL ALTELL ('AGVDET: VDET 89-90 is not implemented ',0,'STOP')
      ELSEIF (IYEAR.EQ.91) THEN
...
C         VDET91-93 old geom. description
         ...
         ELSE
C         VDET94-95 old VDET , with new geom.description a la VDET9510
