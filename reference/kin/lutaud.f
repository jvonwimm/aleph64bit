...
C
      ...
      SUBROUTINE TAUOLAI
C--------------------------------------------------------------------
C!  initialiase KORLAZ for TAUOLA
...
C default  values
      ENE    = 45.625
      AMZ    = PMAS(LUCOMP(23),1)
      AMTOP  = PMAS(LUCOMP( 6),1)
      AMH    = PMAS(LUCOMP(25),1)
      AMNUTA = 0.001
      AMNEUT = 0.010
      SINW2  = PARU(102)
      GAMM   = PMAS(LUCOMP(23),2)
      KEYGSW = 0                    ! no rad. in Z-> tau tau for LUTAUD !
      KEYRAD = 0
      KEYWLB = 1
      ITFIN  = 1
      NNEUT  = 3
...
C Lund7.3 identifier for electron = 11
   ...
   10  E2(I) = 0.
      JAK1   =  0
      JAK2   =  0
C - Allow the user to choose tau decay channels via data cards...
C      CALL UTDCHA( JAK1, JAK2 )
      ISPIN  =  1
      ITDKRC =  1
      XK0DEC =  .001
      GV     =  1.
...
C PB1, PB2 : four momenta of beams
...
C
    ...
    2  PB2(I) = -PB1(I)
       PB2(4) =  PB1(4)
C  never change this otherwise all the logic of tau+ vs tau- is wrong
       KFB    =  11
C
...
C let the user overwrite the polarisation for special cases
      ...
      UTOPOL = TPOL
      END
      SUBROUTINE BUKERD(NMODE,ITFIN)   ! was used to book histos
      RETURN
      END
      SUBROUTINE DEXAY(K,P)            ! changed into TAUDEKAY
      RETURN
      END
