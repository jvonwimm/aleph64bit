CBB    remove subroutine TRALO4 from origine , 
C      moved to KORALW 9/12/96
C      following A.Valassi/Z.Was advices
CBB    set /HEPEVT/ as REAL*8 and MXHEP=4000
C      comment out RANMAR(RMARIN,RMARUT) code, so it uses ALEPH one
...
C      RR=0.3
C      CALL CHOICE(100+JNPI,RR,ICHAN,PROB1,PROB2,PROB3,
C     $            AMROP,GAMROP,AMRX,GAMRX,AMRB,GAMRB)
      PREZ=PROB1+PROB2
C TAU MOMENTUM
      PT(1)=0.
      PT(2)=0.
...
C
      ...
      RETURN
      END
CBB      SUBROUTINE RANMAR(RVEC,LENV)
C ----------------------------------------------------------------------
...
C ----------------------------------------------------------------------
C      COMMON / INOUT / INUT,IOUT
C      DIMENSION RVEC(*)
C      COMMON/RASET1/U(97),C,I97,J97
C      PARAMETER (MODCNS=1000000000)
C      DATA NTOT,NTOT2,IJKL/-1,0,0/
C
C      IF (NTOT .GE. 0)  GO TO 50
C
...
C      GO TO 1
C
C      ENTRY      RMARIN(IJKLIN, NTOTIN,NTOT2N)
...
C      TWOM24 = 1.0
C      DO 4 I24= 1, 24
C    4 TWOM24 = 0.5*TWOM24
C      C  =   362436.*TWOM24
C      CD =  7654321.*TWOM24
C      CM = 16777213.*TWOM24
C      I97 = 97
C      J97 = 33
C       Complete initialization by skipping
C            (NTOT2*MODCNS + NTOT) random numbers
C      DO 45 LOOP2= 1, NTOT2+1
C      NOW = MODCNS
C      IF (LOOP2 .EQ. NTOT2+1)  NOW=NTOT
C      IF (NOW .GT. 0)  THEN
C       WRITE (IOUT,'(A,I15)') ' RMARIN SKIPPING OVER ',NOW
C       DO 40 IDUM = 1, NTOT
C       UNI = U(I97)-U(J97)
C       IF (UNI .LT. 0.)  UNI=UNI+1.
C       U(I97) = UNI
C       I97 = I97-1
...
C         IF (UNI .EQ. 0.) UNI= TWOM24*TWOM24
C         ENDIF
C      RVEC(IVEC) = UNI
C  100 CONTINUE
C      NTOT = NTOT + LENV
C         IF (NTOT .GE. MODCNS)  THEN
C         NTOT2 = NTOT2 + 1
C         NTOT = NTOT - MODCNS
C         ENDIF
C      RETURN
C           Entry to output current status
C      ENTRY RMARUT(IJKLUT,NTOTUT,NTOT2T)
C      IJKLUT = IJKL
C      NTOTUT = NTOT
C      NTOT2T = NTOT2
C      RETURN
...
C --- end of the non omega current (3 possibilities)
 101   CONTINUE
      ...
      END
CBB      SUBROUTINE TRALO4(KTOS,PHOI,PHOF,AM)
C      double precision Q1(4),Q2(4),P1(4),P2(4),P3(4),P4(4)
C      double precision PIN(4),POUT(4)
C      COMMON / MOMDEC / Q1,Q2,P1,P2,P3,P4
C      REAL*4 PHOI(4),PHOF(4)
C      SAVE
C      AM=SQRT(ABS
C     $   (PHOI(4)**2-PHOI(3)**2-PHOI(2)**2-PHOI(1)**2))
C      DO K=1,4
C       PIN(K)=PHOI(K)
C      ENDDO
C      IF(KTOS.EQ.2) THEN
C        CALL BOSTDQ(-1,P1,PIN,POUT)
C      ELSE
C        CALL BOSTDQ(-1,P4,PIN,POUT)
C      ENDIF
C      DO K=1,4
C       PHOF(K)=POUT(K)
C      ENDDO
