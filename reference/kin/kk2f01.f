...
C -   book fragmentation info
      ...
      IF ( IST.ne.0) then
         ista = ista+ist
        WRITE(iw(6),
     & '('' -ERROR booking KINE/VERT AT EVENT #  '',I10)') IFI,ist
      ENDIF
...
C Look for flavor generated
 ...
 998  IF (ISTA.EQ.0 ) THEN
      ...
      ELSEIF ( ISTA.LT.0) THEN
         ...
         ICOULU(9) = ICOULU(9) +1
      ENDIF
      if ( kffin.le.20) igflav(kffin) = igflav(kffin)+1
C
...
C HISTOGRAMMING
C      PARAMETER (NVAR = 20)
C      CHARACTER*6 TITRE(NVAR)
C      DATA  TITRE/'X     ','Nobj  ','Px    ','Py    ',
C     &            'Pz    ','Etot  ','Nobhep','Etohep',
C     &            'KFcode','Npi   ','Npihep','Spheri',
C     &            'Aplan ','Thrust','Oblat ','PXgamm',
C     &            'PYgamm','PZgamm','Egamma','Ngamma'/
C      PARAMETER (NVARP = 4)
C      CHARACTER*6 PHOTON(NVARP)
C      DATA  PHOTON/'PX    ','PY    ','PZ    ','E     '/
...
C
        call bookhis
C       CALL HROPEN(99,'testhis','plot.hst','N',1024,istat)
C       CALL HBOOKN(500,'NTUPLE',NVAR,' ',1024,TITRE)
C       CALL HBOOKN(501,'NTUPL2',NVARP,' ',1024,PHOTON)
...
C      CALL CLOSEHISTO
  ...
  102  FORMAT(/5X,'# OF GENERATED EVENTS                = ',I10,
     ...
     &        /5X,'# OF REJECTED  EVENTS                = ',I10)
       do i=1,20
         if (igflav(i).gt.0) WRITE(IUT,103) i,igflav(i)
       enddo
...
*//////////////////////this is kk2f.h/////////////////////////////////////////
      ...
      CHARACTER*14        m_Date
      PARAMETER ( m_Version     =          4.12d0 )
      PARAMETER ( m_Date        =  ' 27 Oct. 1999')
*////////////////////////////////////////////////////////////////////////////
      INTEGER     m_phmax             ! maximum photon multiplicity ISR+FSR
      PARAMETER ( m_phmax = 100)
      INTEGER     m_jlim
      PARAMETER(  m_jlim  = 10000)
      INTEGER     m_lenwt             ! maximum number of auxiliary weights in
                                      ! WtSet
      PARAMETER ( m_lenwt = 1000)
*/////////////////////////////////////////////////////////////////////////////
      DOUBLE PRECISION       m_xpar,   m_ypar
      INTEGER            m_out,    m_Idyfs,  m_nevgen, m_idgen,
     &  m_KeyWgt
      INTEGER            m_npmax,  m_idbra,  m_nphot
      INTEGER            m_KeyHad, m_KeyISR, m_KeyFSR, m_KeyINT,
     &  m_KeyGPS
      INTEGER            m_Phel,   m_isr,    m_KFini,  m_IsBeamPolarized
      DOUBLE PRECISION       m_CMSene, m_DelEne, m_Xcrunb, m_WTmax,
     &  m_HadMin, m_MasPhot,  m_Emin
      DOUBLE PRECISION       m_BornCru,m_WtCrud, m_WtMain, m_sphot
      DOUBLE PRECISION       m_WtSet,  m_alfinv, m_vcut,   m_Xenph
...
*
     ...
     $    m_xpar(m_jlim),     ! input parameters, READ ONLY, never touch
                              ! them!!!!
     ...
     $    m_vcut(3),          ! technical cut on E/Ebeam for non-IR real
                              ! contributions
...
* through getter KK2f_GetXsecMC(xSecPb, xErrPb)
      ...
      print *,' x-section (pb)',m_xSecPb,' +-',m_xErrPb
         XTOT = m_xSecPb/big
         RTOT = m_xErrPb/big
...
C      ENDIF
      ...
      do i=1,15
        if (igflav(i).gt.0) then
          is = is+1
          CALL GLK_MgetAll(m_idBRa+i,
     $        AverWt, ERrela, WtSup, AvUnd, AvOve,
     $        Ntot, Nacc, Nneg, Nove, Nzer)
          NTOT = igflav(i)
          NACC = NTOT
          eps = float(igflav(i))/float(icoulu(10))
C          deps = 1./float(igflav(i)) - 1./float(icoulu(10))
          XPAR = XTOT*eps
          XACC = XPAR
          RTOT = XPAR*errela
C          RTOT = xpar*sqrt(deps + erel*erel )
          RACC = RTOT
      ISEC =  KSECBK(IS,IDC,I,NTOT,NACC,XPAR,RTOT,XACC,RACC)
        endif
      enddo
 100  continue
      CALL PRTABL('KSEC',0)
