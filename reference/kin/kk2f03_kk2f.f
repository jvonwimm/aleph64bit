...
* Local
      ...
      DOUBLE COMPLEX  PropW,Coef,Coef1  
...
!         next will be mass fro common /cdzwsm/ from  DZface.f it is actually raw mass as above ...
        ...
        Coef  =1.d0/2.d0/m_Sw2
        Coef1=m_Gmu*m_MW**2* m_Alfinv/4D0
...
!            if (abs(row-1).gt.0.016) stop
           WVPi= WVPi*GSW(5)  ! note that for low energies gsw is not tabulated with angle ROW
        ENDIF
      ENDIF
      END
...
* CosThetD = 1d0 is special
      ...
      ENDIF
      END
      SUBROUTINE  GPS_KinExt(p1,m1,p2,m2,p3,m3,p4,m4,ph,mph,p3v,p4v)
*/////////////////////////////////////////////////////////////////////////////////////
*//                                                                                 //
...
*/////////////////////////////////////////////////////////////////////////////////////
      IMPLICIT NONE
      DOUBLE PRECISION      PX(4),p1(4),p2(4),p3(4),p4(4),ph(4),PFAT(4),PSUM(4),PTEST(4),p3v(4),p4v(4)
...
*
      ...
      DOUBLE PRECISION      PX(4),p1(4),p2(4),p3(4),p4(4),ph(4),PTEST(4),p3v(4),p4v(4)
      ...
      DOUBLE COMPLEX        Csum1,Csum2,Csum3,Csum4,U(2,2),V(2,2),s1(2),s2(2),UW(2,2),VW(2,2)
      ...
      DOUBLE PRECISION      pr1,pr2,pr3,pr4,Fleps,F,G,f0,g0,h,h0,R,R0
...
C reduction procedure ....
        CALL GPS_KinExt(p1,m1,p2,m2,p3,m3,p4,m4,ph,mph,p3v,p4v)
C--------
         sa=(p1(4)+p2(4))**2-(p1(3)+p2(3))**2-(p1(2)+p2(2))**2-(p1(1)+p2(1))**2
         ta=(p4v(4)-p2(4))**2-(p4v(3)-p2(3))**2-(p4v(2)-p2(2))**2-(p4v(1)-p2(1))**2
         ua=(p3v(4)-p2(4))**2-(p3v(3)-p2(3))**2-(p3v(2)-p2(2))**2-(p3v(1)-p2(1))**2

         CALL GPS_BornWPlus(1,0,KFi,KFf,sa,ta,ua, ph,mph,    p2,-Fleps,  p3,m3,   p4,-m4,   AmpBornU)

C--------
         sb=(p1(4)+p2(4))**2-(p1(3)+p2(3))**2-(p1(2)+p2(2))**2-(p1(1)+p2(1))**2
         tb=(p3v(4)-p1(4))**2-(p3v(3)-p1(3))**2-(p3v(2)-p1(2))**2-(p3v(1)-p1(1))**2
         ub=(p4v(4)-p1(4))**2-(p4v(3)-p1(3))**2-(p4v(2)-p1(2))**2-(p4v(1)-p1(1))**2
...
* Fermion propagarotors
      ...
      pr3 = 1d0/(p3v(4)*ph(4)-p3v(3)*ph(3)-p3v(2)*ph(2)-p3v(1)*ph(1))/2d0
      pr4 =-1d0/(p4v(4)*ph(4)-p4v(3)*ph(3)-p4v(2)*ph(2)-p4v(1)*ph(1))/2d0
      Sig = 3-2*Hel
      IF( m_KeyArb .EQ. 0 ) THEN
         CALL GPS_MakeU(ph,Sig,  ph,mph,  p1,m1,    U)
         CALL GPS_MakeV(ph,Sig,  p2,m2,   ph,mph,   V)
         CALL GPS_MakeU(ph,Sig,  p3,m3,   p1,m1,    UW0)
         CALL GPS_MakeV(ph,Sig,  p2,m2,   p4,m4,    VW0)
      ELSE
         CALL GPS_MakeUb(ph,Sig, ph,mph,  p1,m1,    U)
         CALL GPS_MakeVb(ph,Sig, p2,m2,   ph,mph,   V)
         CALL GPS_MakeUb(ph,Sig, p3,m3,   p1,m1,    UW0)
         CALL GPS_MakeVb(ph,Sig, p2,m2,   p4,m4,    VW0)
      ENDIF
         CALL  GPS_MatrW(WC)             ! W-e-nu couplingss
         CALL  GPS_times(Cnor,VW0,WC,VW) ! we add v-a coupl
         CALL  GPS_MatrWm(WC)             ! W-e-nu couplingss
         CALL  GPS_times(Cnor,WC,UW0,UW) ! we add v-a coupl

         CALL GPS_MakeUX(ph,Fleps, p3,m3,   p1,m1,    UWX) ! v-a inside
         CALL GPS_MakeVX(ph,Fleps, p2,m2,   p4,m4,    VWX) ! v-a inside
...
*--------------------
      IF( m_KeyArb  .EQ.  0 ) THEN
         ...
         IF (ABS(KFf).EQ.12) THEN 
           EpsDot(1)=0.5D0*(-GPS_Sof1x( 1,ph,p1)+GPS_Sof1x( 1,ph,p3v)  ! 0.5 to compensate 2 from Sof1(b)x
     $                      +GPS_Sof1x( 1,ph,p2)-GPS_Sof1x( 1,ph,p4v)) ! minis sign is in pr2/4
         ENDIF
      ELSE
         s1(1)  = -GPS_Sof1b( 1,ph,p1,m1)
         s2(1)  =  GPS_Sof1b( 1,ph,p2,m2)
         IF (ABS(KFf).EQ.12) THEN
          EpsDot(1)=0.5D0*(-GPS_Sof1bx( 1,ph,p1,m1)+GPS_Sof1bx( 1,ph,p3v,m3)  ! 0.5 to compensate 2 from Sof1(b)x
     $                    +GPS_Sof1bx( 1,ph,p2,m2)-GPS_Sof1bx( 1,ph,p4v,m4)) ! minis sign is in pr2/4
         ENDIF
      ENDIF
      s1(2) = -DCONJG(s1(1))
      s2(2) = -DCONJG(s2(1))
...
*
     ...
     $                      Qe*s1(Hel) *(PropWa*WVPia-PropW0*WVPi0)                   ! t-channel W-prop variation
     $                    + Qe*s2(Hel) *(PropWb*WVPib-PropW0*WVPi0)                   ! t-channel W-prop variation
...
*-----------
     ...
     $                      -m2*GPS_XiProd(  ph,p2)*GPS_iProd1( 1,p1,m_b) )
      ...
      SUBROUTINE GPS_MakeUX(ph,mh,p1,m1,p2,m2,U)
*/////////////////////////////////////////////////////////////////////////////////////
*//                                                                                 //
...
*-----------
      ...
      CALL  GPS_times(Cnor,C,AB,U)
      END                       !!!! GPS_MakeUb

      SUBROUTINE GPS_MakeVX(ph,mh,p1,m1,p2,m2,V)
*/////////////////////////////////////////////////////////////////////////////////////
*//                                                                                 //
...
*-----------------------------------------------------------------------------
      ...
      Fleps =  1d-100
         ...
         CALL GPS_MakeUX(k1,Fleps, p3,m3,   p1,m1,    UWX1) ! v-a inside
         CALL GPS_MakeVX(k1,Fleps, p2,m2,   p4,m4,    VWX1) ! v-a inside
         CALL GPS_MakeUX(k1m,Fleps, p3,m3,   p1,m1,    UWX1m) ! v-a inside
         CALL GPS_MakeVX(k1m,Fleps, p2,m2,   p4,m4,    VWX1m) ! v-a inside

         CALL GPS_MakeUX(k2,Fleps, p3,m3,   p1,m1,    UWX2) ! v-a inside
         CALL GPS_MakeVX(k2,Fleps, p2,m2,   p4,m4,    VWX2) ! v-a inside
         CALL GPS_MakeUX(k2m,Fleps, p3,m3,   p1,m1,    UWX2m) ! v-a inside
         CALL GPS_MakeVX(k2m,Fleps, p2,m2,   p4,m4,    VWX2m) ! v-a inside
           write(*,*) k1
           write(*,*) fleps,m3,m1
           write(*,*) p3
           write(*,*) p1
...
!           CALL GPS_UPrint(0,' UWX2    ',UWX2)
         ...
         CALL GPS_MakeUX(k3,Fleps, p3,m3,   p1,m1,    UWX3) ! v-a inside
         CALL GPS_MakeVX(k3,Fleps, p2,m2,   p4,m4,    VWX3) ! v-a inside
         CALL GPS_MakeUX(k3m,Fleps, p3,m3,   p1,m1,    UWX3m) ! v-a inside
         CALL GPS_MakeVX(k3m,Fleps, p2,m2,   p4,m4,    VWX3m) ! v-a inside
...
* Final state fermions
      ...
      KStat = 1
      CALL HepEvt_Fil1(3,KStat, KFfin, 1,2,0,0, qf1,amfin,.FALSE.)
      CALL HepEvt_Fil1(4,KStat,-KFfin, 1,2,0,0, qf2,amfin,.FALSE.)
* Radiative photons (5 ... 4+nphot) (pdg-code for gamma is 22)
...
*                 (b) photons for CEEX model cannot be split into ISR and FSR
      ...
      IF(nphox .NE. 0) THEN
         DO i=1,nphox
            ...
            END DO
            ip=ip+1
            CALL HepEvt_Fil1(4+ip,1,22, 1,2,0,0, aph,0d0,.FALSE.) ! ISR
         END DO
      END IF
      IF(nphoy .NE. 0) THEN
         DO i=1,nphoy
            ...
            END DO
            ip=ip+1
            CALL HepEvt_Fil1(4+ip,1,22, 2,1,0,0, aph,0d0,.FALSE.) ! FSR
         END DO
      END IF
* add beamstrahlung photons to the record along with ISR photons, can be missleading!!
      ...
      IF( aph(4).NE.0d0) THEN
         ip=ip+1
         CALL HepEvt_Fil1(4+ip,1,22, 1,2,0,0, aph,0d0,.FALSE.)
         DO j=1,4
            Psum(j)= Psum(j)+aph(j)
         ENDDO
      ENDIF
      IF( bph(4).NE.0d0) THEN
         ip=ip+1
         CALL HepEvt_Fil1(4+ip,1,22, 1,2,0,0, bph,0d0,.FALSE.)
         DO j=1,4
            Psum(j)= Psum(j)+bph(j)
         ENDDO
      ENDIF
...
* Quarks only, KeyHad=1 required
      ih1=3  ! fermion is here
      ih2=4  ! antifermion is here
...
* It translates between DOUBLE PRECISION  /c_HepEvt/ and REAL              Lund commons.
...
* ----------------------------------------------------------------------
      ...
      REAL              p,v,paru,parj,pmas,parf,vckm  !!! STJ
      SAVE
* ---------------------------------------------------------------------- 
...
* ----------------------------------------------------------------------
      nphot=0
      DO j=1,100
         ih = 4+j               !photons start always at position ih=5
         kf = idhep(ih)
* STOP if /hepevt/ ended or non-photon found
         IF(ih .GT. nhep) GOTO 110
         IF(kf .NE. 22)   GOTO 110
...
*----------------------------------------------------------------------
      IF( m_KeyFSR .NE. 0) CALL LuGive("MSTJ(41)=1;")
*----------------------------------------------------------------------
      m_nevgen=0
*----------------------------------------------------------------------
