...
C-----------------------------------------------------------------------
      ...
      REAL ptrak(4,2)
      REAL ulmass, xmm, rx, ry, rz, dum
      REAL ecms,weit,RXX,RYY,RZZ
...
C
      REAL*4 PARU, PARJ, PMAS, PARF, VCKM, BRAT, P7LU, V7LU
...
C
      INTEGER loutbe,idb1,idb2,nevent,ism,iklei,icar,ifvrt,ncol
      REAL*4 ecm,sdvrt,vrtx,tabl,empir,empirm,xrad,XVRT,sXVRT
      PARAMETER (ncol=32)
      COMMON / miscl / loutbe,ecm,idb1,idb2,xrad,empir,empirm,ism,
     &                 iklei,icar,sdvrt(3),vrtx(4),tabl(ncol),
     &                 nevent(11),ifvrt,XVRT(3),sXVRT(3)
C
      PARAMETER (maxpro= 10)
     ...
     .                                /
      ...
      DOUBLE PRECISION brai, braz, fracz
      DIMENSION brai(11), kcode(11)
      DATA brai/.03364,.06684,.03364,.06684,.03364,.06684,
     .          .15133,.12228,.15133,.12228,.15134/
      DATA kcode/11,12,13,14,15,16,1,2,3,4,5/
      COMMON / zzdec / braz(11), kselec(11)
C
      DOUBLE PRECISION sigma1,sigma2,sigma3,sigma4,sigma5
      DOUBLE PRECISION sigma6,sigma7,sigma8,sigma9,sigma10
      DOUBLE PRECISION sigmat
      EXTERNAL sigma1,sigma2,sigma3,sigma4,sigma5
      EXTERNAL sigma6,sigma7,sigma8,sigma9,sigma10
      EXTERNAL sigmat
C
      REAL*4 AMARSET,AMAR
      EXTERNAL AMARSET
...
C
      ENDIF
C
C Switch to second random sequence for frag and decays
C
      amar = amarset(1)
C
C
...
C
C Analyse particle event content
C
      call lutabu(11)
      call lutabu(21)
C
...
C
      REAL*8 sw2, alpha, gweak2, alphas, g_f, deltar, alpha2,
     .       sw, cw2, cw, fbb, fww, fb, fw, dkappag,
     .       aaa, bbb
      COMMON / elweak / sw2,alpha(0:nhig),gweak2(0:nhig),
     .                  alphas(0:nhig),g_f,deltar,alpha2,sw,cw2,cw
      COMMON / anomalous / fbb, fww, fb, fw, dkappag
      COMMON / anomalhz  / aaa, bbb
C
      PARAMETER (maxpro= 10)
      REAL*8 cross, sthr, reduc
      REAL*8 chnnz, chnni, chnns
...
C
     ...
     &                 nevent(11),ifvrt,XVRT(3),sXVRT(3)
C
      REAL*8 PI,PI2,PI4,TWOPI,PIBY2,PIBY4,PIBY6,PIBY8,PIBY12,PIBY3
...
C
      ...
      jgsgu = IW(ngsgu)
C
C Not the two cards at the same time!
C
      IF ( jgsus .NE. 0 .AND. jgsgu .NE. 0 ) THEN
        WRITE(6,*) 'You cannot have both GSUS and GSGU cards'
        STOP
C
      ELSEIF(jgsus.NE.0) THEN
C
        amarun  =      RW(jgsus+1)
        tb      =      RW(jgsus+2)
        susM2   =      RW(jgsus+3)
...
C
      ...
      IF(jpryn.NE.0) THEN
        DO ipro = 1, maxpro
          ...
          ENDIF
        ENDDO
      ELSEIF ( iproc .GE. 1 .AND. iproc .LE. maxpro ) THEN
        iproyn(iproc) = 1
      ...
      ELSE
        WRITE(loutbe,*) '   +++++++++++++++++++++++++++++++++++++ '
        WRITE(loutbe,*) '   +++ Please, try to be serious and +++ '
        WRITE(loutbe,*) '   +++ select at least the processes +++ '
        WRITE(loutbe,*) '   +++        to be generated!       +++ '
        WRITE(loutbe,*) '   +++++++++++++++++++++++++++++++++++++ '
        STOP 99
      ENDIF
C
...
C
     ...
     .      susMSNU .LE. 0. ) ) ) STOP 99
      WRITE(6,'(A,4(1X,A,G12.5),/)') ' Lifetimes (ps) h, H, A, H+-',
     .    ': ',tauh(2)*1E12,', ',tauh(1)*1E12,
     .    ', ',tauh(3)*1E12,', ',tauhp*1E12
C
      TABL(1)  = iklei
      TABL(2)  = iproc
      TABL(3)  = xrad
...
C...Vertex smearing
      ...
      tabl(26) = sdvrt(3)
C
C   get an offset for position of interaction point     <== new
...
C
      ...
      CALL PRTABL('KHHG',0)
C
C Keep track of particle event content
C
      call lutabu(10)
      call lutabu(20)
C
...
C      DO ipr = 1, 4
C        ipro = ipr + 4
C        IF ( ntry(ipr) .GT. 0 ) THEN
C          sigto = wtot(ipr)/FLOAT(ntry(ipr))
C          dsig  = SQRT((wtot2(ipr)/FLOAT(ntry(ipr)) - sigto**2)
C     .          / FLOAT(ntry(ipr)))
C        WRITE(6,*) ' '
C        WRITE(6,*) '--------------------------------------------------'
C        WRITE(6,*) ' Generated cross section for ',chapro(ipro),' :'
C        WRITE(6,*) '    Sigma = ',sigto,' +/- ',dsig,' fb'
C        WRITE(6,*) '    # of trials          : ',ntry(ipr)
C        WRITE(6,*) '    # of accepted events : ',nacc(ipr)
C        WRITE(6,*) '--------------------------------------------------'
C        WRITE(6,*) ' '
C        ENDIF
C      ENDDO
...
C      imstj = nlink('MSTJ',28)
C      IF(imstj.gt.0)then
C         if ( iw(imstj+1).eq.2) call taudkay(itau,ndec,100)
C      endif
      CALL ugtsec
C
...
C
C Printout particle event content
C
      call lutabu(12)
      call lutabu(22)
C
      ...
      SUBROUTINE UGTSEC
C --------------------------------------------------------------------
C     B.Bloch vreate x-section bank November 1999
C --------------------------------------------------------------------
C --------------------------------------------------------------------
      PARAMETER(igcod=7024)
      PARAMETER(iver =105)
...
C --------------------------------------------------------------------
C
C store cross-section in KSEC bank
C
      NTOT = 0
      do ix = 1,maxpro
        if(cross(ix).gt.0.) then
          xtot = xtot + cross(ix)
          npro = npro +1
          NTOT = NTOT + nevpro(ix)
        endif
      ...
      do ix = 1,maxpro
        if(cross(ix).gt.0.) then
         IS = IS + 1
         xtot = cross(ix)*1.E-06
         XACC = XTOT
         NTOT = nevpro(ix)
         NACC = NTOT
         isec = KSECBK(IS,IDC,IX,NTOT,NACC,XTOT,RTOT,XACC,RACC)
        endif
      enddo
      CALL PRTABL('KSEC',0)
      RETURN
...
C       WRITE(6,*) jhig,branch1
        IF ( ism .LE. 0 )
     .         branch1 = branch1  ! Generic two-doublet model (020498)
...
C         WRITE(6,*) jhig,branch3
          IF ( ism .LE. 0 )
     .       branch3 = branch3 ! Generic two-doublet model (020498)
C Charged Higgs bosons
     . + (1.-2.*sw2) / (sw*cw)
...
C
      ...
      pmas(36,2) = width(3)
C
      pmas(25,3) = AMIN1(pmas(25,1),10.*SNGL(width(2)))
      pmas(35,3) = AMIN1(pmas(35,1),10.*SNGL(width(1)))
      pmas(36,3) = AMIN1(pmas(36,1),10.*SNGL(width(3)))
C
      IF( ism.LE.0 ) THEN ! Generic two-doublet model (020498)
...
C
     ...
     &                  / (1.-8./3.*deltar       )
          ...
          ENDIF
          IF ( chnnz(ipro) .LE. 0D0 ) chnnz(ipro) = 0D0
        ENDIF
C
...
C--------------------------------------------------------------------
      IMPLICIT REAL*8 (A-H,L,M,O-Z)
      REAL*8 cma,ctb,cmq,cmur,cmdr,cmtop,cau,cad,cmu,cmglu,cmchi
      REAL*8 cmh,cmhp,chm,chmp,camp,cmhch,csa,cca,csab,ccab
...
c      COMMON/QMASS/ TMPOLE
      real*8 ZMASS,WMASS,SINW2,ALPHA_EM,BMASS,G2,GP2,SINB,COSB,COTB
      COMMON /HPARAM/ ZMASS,WMASS,SINW2,ALPHA_EM,BMASS,
     &                 G2,GP2,SINB,COSB,COTB
      real*8 SINA,COSA,SIN2A,COS2A,SINBPA,COSBPA,SINBMA,COSBMA
      COMMON/ANG/ SINA,COSA,SIN2A,COS2A,SINBPA,COSBPA,SINBMA,COSBMA
      real*8 SLM,SQM,SQK,SQU,SQD,XMU,STA2,STB2,SBA2,SBB2
      COMMON/SQPARM/ SLM,SQM,SQK,SQU,SQD,XMU,STA2,STB2,SBA2,SBB2
      real*8 TMPOLE
      COMMON/QMASS/ TMPOLE
...
C                qh[4]
C                qa[4]
C                hh[4],  the final state particles (H,Z,fermion,gamma)
...
C-------------------------------------------------------------------
C
C  calculate some (soft susy breaking) scalar mass from m0, m1/2 and
C  tan(beta)
C
...
C tan(beta). The top and bottom Yukawa effects are not taken into accoun
C as this would require a SUGRA-like model, where the higgs spectrum is
...
C
     ...
     .         / ( 1.
     &         + 4./3. *alpha_3/pi
     &         + factork*(alpha_3/pi)**2 ! Should be used with 2-loops
     &                                     )
      ENDIF
C
      amhig(1) = amt
...
C
     ...
     .       / ( 1.
     &         + 4./3. *alpha_3/pi
C     &         + factork*(alpha_3/pi)**2  ! With full two-loop
     &                                   )
      amhig(1) = amt
      runamb = amb * SQRT(runmas(amb,1,radb))
      amhig(1) = 0.
...
C - Superseed GHCC ...
        ...
        END IF
      ELSE
C - Data card missing: enable all the channels ...
        DO I=1,NDCH
          HCCON(I)= 1
        END DO
      END IF
...
C
C     Input:     OPT    0= full propagator in,
C                       1= propagator out (for integration)
