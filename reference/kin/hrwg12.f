...
C --------------------------------------------------------------------
C
C Modified - B.Bloch, November 2000 from HERWIG 6.2 interface
C --------------------------------------------------------------------
C
...
C Flags and counters for the Herwig interface
      INTEGER NCOLM,MAXFLA
C
      PARAMETER (NCOLM = 100)
      PARAMETER (MAXFLA=7)
C
      INTEGER NEVENT,IFLC,IOUT,IDEBB,IDEBE,IFVRT
      REAL *4 SDVRT,TABL,XVRT,SXVRT
C
      COMMON / DTMILL / XVRT(3),sXVRT(3),SDVRT(3),TABL(NCOLM),
     &                  NEVENT(10),IFLC(MAXFLA),IFVRT
      COMMON / INPOUT / IOUT
...
CB      parameter (datemod=' November 27 2000')
      parameter (datemod=' November 8 2002')
C
...
C Standard variables
      INTEGER IGCOD,NDAT,IMAXER,IPROCI,NCOL
...
C
 ...
 101  FORMAT(/,10X,
     ...
     & ,/,10X,' Version 6.2 bugs fixed up to  1st December 2000'
     & ,/,10X,' Aleph version ',i4,' bugs fixed up to July 2002'
     & ,/,10X,'***********************************************',
     &'********************',//)
C
C initialization
C
      ...
      CALL HWIGIN
      DUMMY=HWRGET(NRN)
C   NRN is kept to start the first event
C   IBRN is use to start the search for maximal weight in HWEINI
      IBRN(1)= nrn(1)
      IBRN(2)= nrn(2)
C
...
C K0s K0l
       ...
       rmass(50)  = rmass(60)
C F0(980)
       rmass(293) = 0.996   ! 293 = F0P0 IDPDG= 9010221
       rmass(294) = 1.000   ! 294 = FH_00 IDPDG= 10221
C A0(980)
       rmass(290) = 0.996   ! 290 = A_00 IDPDG= 9000111
C XIb s
       rmass(227) = 5.800   ! 227 XI_B0    IDPDG= 5232
       rmass(228) = rmass(227)
       rmass(251) = rmass(227)
       rmass(252) = rmass(227)
...
C
      PART1 = 'E-      '
      PART2 = 'E+      '
      PBEAM1 = 46.1D0   ! beam momentum
      IPROC = 100       ! HERWIG process identifier
      IHARD = 0      ! initial process (e+e- --> qqbar) simulated by HER
      IPPART = 1     ! print PART bank per default
C
...
C  scheme for final states!
      ...
      IF(IGPRO.GT.0) then
        PBEAM1 = 0.5*RW(IGPRO+1)
C        JTYPE = IW(IGPRO+2)
C        IF(JTYPE.eq.1)  ITYPE = 12 ! ddbar
C        IF(JTYPE.eq.2)  ITYPE = 11 ! uubar
C        IF(JTYPE.eq.3)  ITYPE = 13 ! ssbar
C        IF(JTYPE.eq.4)  ITYPE = 14 ! ccbar
C        IF(JTYPE.eq.5)  ITYPE = 15 ! ddbar
C        IF(JTYPE.eq.10)  ITYPE = 10 ! udbar
C        IF(JTYPE.eq.11)  ITYPE = 1 ! electrons
C        IF(JTYPE.eq.13)  ITYPE = 2 ! muons
C        IF(JTYPE.eq.15)  ITYPE = 3 ! taus
      ENDIF
...
C
      ...
      ENDIF
C finally we redefine some steering parameters of the HERWIG program:
C don't print out event listings
C increase number of possible HERWIG errors
C
      MAXPR = 0
      MAXER = 100
C increase with number of events as recommended
      NAMI = NAMIND('TRIG')
      JDEBU = IW(NAMI)
      IF(JDEBU.gt.0) THEN
         IDEBB = IW(JDEBU+2)
         print * , '  Going for ',IDEBB,' events'
         maxev = idebb
      ENDIF
      if (IDEBB/1000.gt.1) maxer = maxer * IDEBB/1000
      IPROCI = IPROC-10000*nosoft
      IF ((IPROCI.gt.500).and.(IPROCI.ne.550)) MAXER = MAX(MAXER,IMAXER)
        WRITE ( 6,1010) maxer,maxev
...
C
      ...
      TABL(4) = FLOAT(IHARD)
      TABL(5) = SNGL(RMASS(6))
      TABL(6) = SNGL(RMASS(13))
      TABL(7) = SNGL(RMASS(199))
      TABL(8) = SNGL(RMASS(200))
      TABL(9) = SNGL(RMASS(201))
      TABL(10) = FLOAT(NFLAV)
      TABL(11) = SNGL(GAMZ)
      TABL(12) = SNGL(GAMW)
      TABL(13) = SNGL(SWEIN)
      ...
      TABL(21) = SNGL(QDIQK)
      TABL(22) = SNGL(PDIQK)
      TABL(23) = SNGL(PSPLT(1))
      TABL(24) = SNGL(PSPLT(2))
      TABL(25) = SNGL(CLSMR(1))
      TABL(26) = SNGL(CLSMR(2))
      TABL(27) = FLOAT(CLDIR(1))
      TABL(28) = FLOAT(CLDIR(2))
      TABL(29) = XVRT(1)
      TABL(30) = XVRT(2)
      TABL(31) = XVRT(3)
      TABL(32) = SDVRT(1)
...
C---DO HEAVY OBJECT DECAYS
      CALL HWDHOB                    !   new from version >=6.1
...
C
      ...
      PARAMETER (MAXFLA=7)
C
      INTEGER NEVENT,IFLC,IOUT,IDEBB,IDEBE,IFVRT,nrej,i
      REAL *4 SDVRT,TABL,XVRT,SXVRT
C
      COMMON / DTMILL / XVRT(3),sXVRT(3),SDVRT(3),TABL(NCOLM),
     &                  NEVENT(10),IFLC(MAXFLA),IFVRT
...
C
     ...
     &          50,-1.,1.,0.)
C
      call hbook1(10301,'COS POLAR ANGLE electron',
     &          100,-1.,1.,0.)
      call hbook1(10302,'energy electron',
     &          55,0.,110.,0.)
      call hbook1(10303,'COS POLAR ANGLE positron',
     &          100,-1.,1.,0.)
      call hbook1(10304,'energy positron',
     &          55,0.,110.,0.)
      RETURN
C
      END
...
C
          if (abs(IDHEP(ITRK)).EQ.11) then
C
            
            IF (PMOM.GT.0) then
             COSTHE = PZ / PMOM
             if (IDHEP(ITRK) .EQ.11) then
               call hfill(10301,costhe,0.,1.)
               call hfill(10302,SNGL(PHEP(4,ITRK)),0.,1.)
             else if (IDHEP(ITRK) .EQ.-11) then
               call hfill(10303,costhe,0.,1.)
               call hfill(10304,SNGL(PHEP(4,ITRK)),0.,1.)
             endif
            endif
          endif
        ENDIF
...
*
         ...
         ENDIF
        if(dtag)WRITE(iw(6),*) ' Double tag requested '
        if(tag)WRITE(iw(6),6000) TAGINN,TAGOUT,ETAG
 6000   FORMAT(' Amcsel: Tag between',2f10.3,' radians, energy >',f10.3)
        if(tag)write(iw(6),*) 'Amcsel: No. of good tracks',itkcut,tkcut
      ENDIF
*
      nrej(1) = nrej(1) +1
         DO 10 JJ=1,NHEP
...
*
     ...
     +               (THETA.LT.TAGOUT).AND.(ABSE.GT.ETAG)) THEN
                     LTAG = .TRUE.
                     NTAG=NTAG+1
                     if (nrej(1).lt.200) print *,'kept track',jj,
     &  IDHEP(JJ),'with E = ',ABSE,' theta =',THETA
                  ENDIF
...
C===============================================================
      IF ( IHARD.eq.0) THEN
       IF (NWGTS.NE.0) THEN
         ...
         NTOT = NEVENT(1)    ! NEVHEP
         XTOT = 2.*AVWGT
         RTOT = 2.*ERWGT
         NACC = NEVENT(10)   ! accepted
         XACC = XTOT*float(NEVENT(10))/float(NEVENT(1))
         RACC = xacc*sqrt((RTOT/xtot)**2 + 1./float(NEVENT(10)) 
     $                              -1./float(NEVENT(1)))
         is =1
         IDC = IGCO
         IVERS = IVER
