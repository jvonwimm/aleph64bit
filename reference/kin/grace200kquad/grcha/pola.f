...
CX          EM(3) = SIGN(1.0q0, PP)*PROP
        ...
        ELSE IF (I.EQ.4) THEN
          ...
          ELSE
            ...
            EP(4) = P(4)*RPPI
              AM2 = AM*AM
CX          IF (A.NE.1.q0.AND.A.LT.100.q0) THEN
            IF (A.LT.100.q0) THEN
              IF (A*AM2.NE.PP) THEN
              EM(4) = SIGN(1.0q0, PP)*(AM2-PP)/(PP-A*AM2)*A
              ELSE
              EM(4)= -SIGN(1.0q0, PP)
              WRITE(6,*) ' Caution : p*p = a*m**2 '
              WRITE(6,*) '  a = 1 was taken. '
              END IF
CX          ELSE IF (A.EQ.1.q0) THEN
CX            EM(4)= -SIGN(1.0q0, PP)
            ELSE
            IF (AM.GT.0.q0) THEN
              EM(4)=  SIGN(1.0q0, PP)*(PP/AM**2-1.0q0)
            ELSE
              EM(4) =-SIGN(1.0q0, PP)*A
            END IF
            END IF
          ENDIF
        END IF
