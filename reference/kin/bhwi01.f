...
C--------------------------------------------------------------------
C Initialization for BHWIDE    J. Von Wimmersperg  30th OCT '96
C--------------------------------------------------------------------
...
C
     ...
     +              ACOLLI,EPSCMS,KEYWGT,KEYRND,KEYOPT,KEYEWC,KEYLIB,
...
C
 ...
 101  FORMAT(/,10X,'BHWIDE - CODE NUMBER =',I4,
     &       /,10X,'**************************',
     &       /,10X,' SUBVERSION  :',I10 ,
     &   /,10x,'Interface last mod on December 17, 1998')
C
...
C
      CMSENE  = 91.2  ! 2*Ebeam [GeV]
      THMINP  = 10.   ! Detector range ThetaMin [deg] for positrons
      THMAXP  =170.   ! Detector range ThetaMax [deg] for positrons
      THMINE  = 10.   ! Detector range ThetaMin [deg] for electrons
      THMAXE  =170.   ! Detector range ThetaMax [deg] for electrons
      ENMINP  =0.18   ! Energy minimum [GeV] for detected positrons
      ENMINE  =0.18   ! Energy minimum [GeV] for detected electrons
      ACOLLI  =180.   ! Maximum acollinearity [deg] of final e+e-
      EPSCMS  =1E-4   ! Infrared cut on photon energy
...
C
      ...
      TABL(14) =  VPOS(2)
      TABL(15) =  VPOS(3)
      TABL(16) =  FLOAT(KEYOPT)
      TABL(17) =  FLOAT(KEYRAD)
C
...
C--------------------------------------------------------------------
C J. Von Wimmersperg  30th OCT '96
C--------------------------------------------------------------------
C--------------------------------------------------------------------
C     input     : none
C
...
C
  ...
  106  FORMAT(/10X,'XSEC_ACCEP (nanobarns)               =  ',G15.8,
     &        /10X,'ERROR      (nanobarns)               =  ',G15.8,//)
C
      RETURN
      END
      SUBROUTINE UGTSEC
