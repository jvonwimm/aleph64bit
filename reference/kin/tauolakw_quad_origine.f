...
C
      REAL(16)            AMTAU,AMNUTA,AMEL,AMNUE,AMMU,AMNUMU
     *                 ,AMPIZ,AMPI,AMRO,GAMRO,AMA1,GAMA1
     *                 ,AMK,AMKZ,AMKST,GAMKST
      COMMON / DECPAR / GFERMI,GV,GA,CCABIB,SCABIB,GAMEL
      REAL(16)            GFERMI,GV,GA,CCABIB,SCABIB,GAMEL
      COMMON /TESTA1/ KEYA1
      COMPLEX(16) BWIGN,FORMOM
      DATA ICONT /1/
* THIS INLINE FUNCT. CALCULATES THE SCALAR PART OF THE PROPAGATOR
      BWIGN(XM,AM,GAMMA)=1.q0/(XM**2-AM**2 +(0.q0,1.q0)*GAMMA*AM)
* HADRON CURRENT
      FRO  =0.266q0*AMRO**2
      ELPHA=- 0.1q0
      AMROP = 1.7q0
      ...
      GCOUP=12.924q0
      GCOUP=GCOUP*AROMEG
      FQED  =SQRT(4.0q0*3.1415926535q0/137.03604q0)
      FORMOM=FQED*FRO**2/SQRT(2.0q0)*GCOUP**2*BWIGN(XMOM,AMOM,GAMOM)
     $     *(BWIGN(XMAA,AMRO,GAMRO)+ELPHA*BWIGN(XMAA,AMROP,GAMROP))
     $     *(BWIGN( 0.0q0,AMRO,GAMRO)+ELPHA*BWIGN( 0.0q0,AMROP,GAMROP))
      END
...
C     ==================================================================
C
      COMPLEX(16) FORM1,WIGNER,WIGFOR,FPIKM,BWIGM
      COMMON / PARMAS / AMTAU,AMNUTA,AMEL,AMNUE,AMMU,AMNUMU
     *                 ,AMPIZ,AMPI,AMRO,GAMRO,AMA1,GAMA1
     *                 ,AMK,AMKZ,AMKST,GAMKST
C
     ...
     *                 ,AMPIZ,AMPI,AMRO,GAMRO,AMA1,GAMA1
     *                 ,AMK,AMKZ,AMKST,GAMKST
      WIGNER(A,B,C)= (1.0q0 +(0.q0,1.q0)*0.0q0)/(A-B**2+(0.q0,1.q0)*B*C)
      IF     (MNUM.EQ.0) THEN
...
C     ==================================================================
C
      COMPLEX(16) FORM2,WIGNER,WIGFOR,FPIKM,BWIGM
      COMMON / PARMAS / AMTAU,AMNUTA,AMEL,AMNUE,AMMU,AMNUMU
     *                 ,AMPIZ,AMPI,AMRO,GAMRO,AMA1,GAMA1
     *                 ,AMK,AMKZ,AMKST,GAMKST
C
...
C
      END
      COMPLEX(16) FUNCTION BWIGM(S,M,G,XM1,XM2)
      IMPLICIT REAL(16) (A-H,O-Z)
      IMPLICIT INTEGER (I-N)
C **********************************************************
C     P-WAVE BREIT-WIGNER  FOR RHO
C **********************************************************
      REAL(16) S,M,G,XM1,XM2
      REAL(16) PI,QS,QM,W,GS
      DATA INIT /0/
C ------------ PARAMETERS --------------------
      IF (INIT.EQ.0) THEN
      INIT=1
...
C -------  BREIT-WIGNER -----------------------
       ...
       ELSE
         GS=0.0q0
       ENDIF
         BWIGM=M**2/(M**2-S -(0.q0,1.q0)* SQRT(S)*GS)
      RETURN
      END
      COMPLEX(16) FUNCTION FPIKM(W,XM1,XM2)
      IMPLICIT REAL(16) (A-H,O-Z)
      IMPLICIT INTEGER (I-N)
C **********************************************************
C     PION FORM FACTOR
C **********************************************************
      COMPLEX(16) BWIGM
      REAL(16) ROM,ROG,ROM1,ROG1,BETA1,PI,PIM,S,W
      EXTERNAL BWIG
      DATA  INIT /0/
C
C ------------ PARAMETERS --------------------
...
C -----------------------------------------------
     ...
     & /(1+BETA1)
      RETURN
      END
      COMPLEX(16) FUNCTION FPIKMD(W,XM1,XM2)
      IMPLICIT REAL(16) (A-H,O-Z)
      IMPLICIT INTEGER (I-N)
C **********************************************************
C     PION FORM FACTOR
C **********************************************************
      COMPLEX(16) BWIGM
      REAL(16) ROM,ROG,ROM1,ROG1,PI,PIM,S,W
      EXTERNAL BWIG
      DATA  INIT /0/
C
C ------------ PARAMETERS --------------------
...
C
     ...
     *                 ,AMPIZ,AMPI,AMRO,GAMRO,AMA1,GAMA1
     *                 ,AMK,AMKZ,AMKST,GAMKST
      COMPLEX(16) FORM5,WIGNER,FPIKM,FPIKMD,BWIGM
      WIGNER(A,B,C)=(1.0q0 +(0.q0,1.q0)*0.0q0)/(A-B**2 +(0.q0,1.q0)*B*C)
      IF     (MNUM.EQ.0) THEN
C ------------  3 pi hadronic state (a1)
        FORM5=0.0q0
...
C ARBITRARY FIXING OF THE FOUR PI X-SECTION NORMALIZATION
      ...
      REAL(16)  PIM1(4),PIM2(4),PIM3(4),PIM4(4),PAA(4)
      COMPLEX(16) HADCUR(4),FORM1,FORM2,FORM3,FPIKM
      COMPLEX(16) BWIGN
      REAL(16) PA(4),PB(4)
      REAL(16) AA(4,4),PP(4,4)
      DATA PI /3.141592653589793238462643q0/
      DATA  FPI /93.3q-3/
      BWIGN(A,XM,XG)=1.0q0/(A-XM**2 +(0.q0,1.q0)*XM*XG)
C
...
C
      FRO=0.266q0*AMRO**2
      COEF1=2.0q0*SQRT(3.0q0)/FPI**2*ARFLAT
      COEF2=FRO*G*AROMEG
...
C ... and the rest ...
     ...
     $         -(PAA(2)-PP(L,2))**2-(PAA(1)-PP(L,1))**2
          ...
          DO 205 J=1,4
           ...
           IF(J.NE.4) SIG=-SIG
           AA(I,J)=AA(I,J)
     $            -SIG*(PAA(I)-2.0q0*PP(L,I))*(PAA(J)-PP(L,J))/DENOM
 205      CONTINUE
         ENDIF
 204    CONTINUE
...
C
       ...
       DO 206 J=1,4
        HADCUR(I)=
     $  HADCUR(I)+(FIX*COEF1)*FORM1*AA(I,J)*(PP(K,J)-PP(4,J))
 206   CONTINUE
...
CCCCCCCCCCCCC       FORM1=WIGFOR(SK,AMRO,GAMRO)        (tests)
        ...
        DO 106 J=1,4
         HADCUR(I)=
     $   HADCUR(I)+(COEF1)*FORM1*AA(I,J)*(PP(K,J)-PP(4,J))
 106    CONTINUE
C --- end of the non omega current (3 possibilities)
 101   CONTINUE
      ENDIF
      ...
      IMPLICIT REAL(16) (A-H,O-Z)
      IMPLICIT INTEGER (I-N)
      COMPLEX(16) WIGFOR,WIGNOR
      WIGNOR=(-XM**2 +(0.q0,1.q0)*XM*XGAM)
      WIGFOR=WIGNOR/(S-XM**2 +(0.q0,1.q0)*XM*XGAM)
      END
...
C JAK=0 INCLUSIVE:  JAK=1,2,3,4,5,6,7,8
     ...
     &                ,NAMES
      ...
      REAL(16)  PDUM1(4),PDUM2(4),PDUM3(4),PDUM4(4),PDUM5(4),HDUM(4)
      REAL(16)  PDUMX(4,9)
      DATA IWARM/0/
      KTOM=KTO
      IF(KTO.EQ.-1) THEN
...
C     =====
 ...
 7001 FORMAT(///1X,15(5H*****)
     $ /,' *',     25X,'*****TAUOLA LIBRARY: VERSION 2.6q *****',9X,1H*,
...
C THIS ROUTINE  SIMULATES TAU+  DECAY
      ...
      REAL(16)            GAMPMC    ,GAMPER
      REAL(16)  HH(4)
      REAL(16)  HV(4),PNU(4),PPI(4)
      REAL(16)  PWB(4),PMU(4),PNM(4)
      REAL(16)  PRHO(4),PIC(4),PIZ(4)
      REAL(16)  PAA(4),PIM1(4),PIM2(4),PIPL(4)
      REAL(16)  PKK(4),PKS(4)
      REAL(16)  PNPI(4,9)
      REAL(16)  PHOT(4)
      REAL(16)  PDUM(4)
...
C ----------------------------------------------------------------------
     ...
     &                ,NAMES
      ...
      COMMON / INOUT / INUT,IOUT
      REAL(16)  POL(4)
      REAL(16)  PDUM1(4),PDUM2(4),PDUM3(4),PDUM4(4),PDUM5(4)
      REAL(16)  PDUM(4)
      REAL(16)  PDUMI(4,9)
      DATA IWARM/0/
      KTOM=KTO
...
C ----------------------------------------------------------------------
      REAL(16)  POL(4),HV(4),PWB(4),PNU(4),Q1(4),Q2(4),PH(4)
      REAL(16) RN
      DATA IWARM/0/
C
      IF(MODE.EQ.-1) THEN
...
300     CONTINUE
        ...
        WT=(1+POL(1)*HV(1)+POL(2)*HV(2)+POL(3)*HV(3))/2.q0
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      ...
      REAL(16)         PHX(4)
      COMMON / INOUT / INUT,IOUT
      REAL(16)  HHV(4),HV(4),PWB(4),PNU(4),Q1(4),Q2(4)
      REAL(16)  PDUM1(4),PDUM2(4),PDUM3(4),PDUM4(4),PDUM5(4)
      REAL(16) RRR(3)
      REAL(16) SWT, SSWT
      DATA PI /3.141592653589793238462643q0/
...
C     ===================
        ...
        IF(WT.GT.WTMAX/1.2q0) WTMAX=WT*1.2q0
15      CONTINUE
...
C     =======================
        ...
        PARGAM=SWT/ (1.q0*NEVRAW+1)
        ERROR=0
        IF(NEVRAW.NE.0) ERROR=SQRT(SSWT/SWT**2-1.q0/ (1.q0*NEVRAW))
        RAT=PARGAM/GAMEL
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      ...
      COMMON / INOUT / INUT,IOUT
      REAL(16)         PHX(4)
      REAL(16)  HHV(4),HV(4),PNU(4),PWB(4),Q1(4),Q2(4)
      REAL(16)  PDUM1(4),PDUM2(4),PDUM3(4),PDUM4(4),PDUM5(4)
      REAL(16) RRR(3)
      REAL(16) SWT, SSWT
      DATA PI /3.141592653589793238462643q0/
...
C *********************************************************************
      REAL(16)         PHX(4)
      REAL(16)  HVX(4),PAAX(4),XAX(4),QPX(4),XNX(4)
      REAL(16)  HV(4),PH(4),PAA(4),XA(4),QP(4),XN(4)
      REAL(16)  DGAMT
      IELMU=1
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      ...
      REAL(16)            XK0DEC
      REAL(16)  HV(4),PT(4),PH(4),PAA(4),XA(4),QP(4),XN(4)
      REAL(16)  PR(4)
      REAL(16) RRR(6)
      LOGICAL IHARD
      DATA PI /3.141592653589793238462643q0/
      XLAM(X,Y,Z)=SQRT((X-Y-Z)**2-4.0q0*Y*Z)
...
*       NUNU  MOMENTUM
        PR(1)=0
        PR(2)=0
        PR(4)=1.q0/(2*AM3)*(AM3**2+AM2**2-AMU**2)
        PR(3)= SQRT(ABS(PR(4)**2-AM2**2))
        PPI  =          PR(4)**2-AM2**2
*       MUON MOMENTUM
        QP(1)=0
        QP(2)=0
        QP(4)=1.q0/(2*AM3)*(AM3**2-AM2**2+AMU**2)
        QP(3)=-PR(3)
...
C PARTIAL WIDTH CONSISTS OF PHASE SPACE AND AMPLITUDE
      ...
      DGAMT=1/(2.q0*AMTAX)*AMPLIT*PHSPAC
      END
...
C
     ...
     *                 ,AMPIZ,AMPI,AMRO,GAMRO,AMA1,GAMA1
     *                 ,AMK,AMKZ,AMKST,GAMKST
      REAL(16)  HV(4),QP(4),XN(4),XA(4),XK(4)
C
      HV(4)=1.q0
      AK0=XK0DEC*AMTAU
      IF(XK(4).LT.0.1q0*AK0) THEN
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      ...
      REAL(16)           ALFINV,ALFPI,XK0
      REAL(16)    QP(4),XN(4),XA(4),XK(4)
      REAL(16)    R(4)
      REAL(16)   HV(4)
      REAL(16) S0(3),RXA(3),RXK(3),RQP(3)
      DATA PI /3.141592653589793238462643q0/
C
      TMASS=AMTAU
      GF=GFERMI
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      ...
      REAL(16) HV(4)
      DIMENSION R(4)
      REAL(16) RXA(3),RXN(3),RQP(3)
      REAL(16) BORNPL(3),AM3POL(3),XM3POL(3)
      DATA PI /3.141592653589793238462643q0/
C
      TMASS=AMTAU
      GF=GFERMI
...
C V-A  AND  V+A COUPLINGS, BUT IN THE BORN PART ONLY
     ...
     &                                             32*(GFERMI**2/2.q0)
  5     HV(I)=(BORNPL(I)+AM3POL(I))/(BORN+AM3)-1.q0
      THB=BORN+AM3
      IF (THB/BORN.LT.0.1q0) THEN
...
C                      PPI   PION CHARGED
C ----------------------------------------------------------------------
      REAL(16)  POL(4),HV(4),PNU(4),PPI(4)
      REAL(16) RN
CC
      IF(MODE.EQ.-1) THEN
C     ===================
...
C       configurations only
        GAMM=(GFERMI*FPI)**2/(16.q0*PI)*AMTAU**3*
     $       (BRAK/AMTAU**4)*
...
C ----------------------------------------------------------------------
      COMMON / INOUT / INUT,IOUT
      REAL(16)  POL(4),HV(4),PRO(4),PNU(4),PIC(4),PIZ(4)
      REAL(16) RN
      DATA IWARM/0/
C
      IF(MODE.EQ.-1) THEN
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      ...
      COMMON / INOUT / INUT,IOUT
      REAL(16)  HHV(4)
      REAL(16)  HV(4),PRO(4),PNU(4),PIC(4),PIZ(4)
      REAL(16)  PDUM1(4),PDUM2(4),PDUM3(4),PDUM4(4)
      REAL(16) RRR(3)
      REAL(16) SWT, SSWT
      DATA PI /3.141592653589793238462643q0/
      DATA IWARM/0/
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      COMMON / DECPAR / GFERMI,GV,GA,CCABIB,SCABIB,GAMEL
      REAL(16)            GFERMI,GV,GA,CCABIB,SCABIB,GAMEL
      REAL(16)  HV(4),PT(4),PN(4),PR(4),PIC(4),PIZ(4),QQ(4)
      REAL(16) RR1
      DATA PI /3.141592653589793238462643q0/
      DATA ICONT /0/
C
C THREE BODY PHASE SPACE NORMALISED AS IN BJORKEN-DRELL
      PHSPAC=1.q0/2**11/PI**5
C TAU MOMENTUM
      PT(1)=0.q0
      PT(2)=0.q0
      PT(3)=0.q0
...
C TAU-NEUTRINO MOMENTUM
      PN(1)=0
      PN(2)=0
      PN(4)=1.q0/(2*AMTAU)*(AMTAU**2+AMNUTA**2-AMX**2)
      PN(3)=-SQRT((PN(4)-AMNUTA)*(PN(4)+AMNUTA))
C RHO MOMENTUM
      PR(1)=0
      PR(2)=0
      PR(4)=1.q0/(2*AMTAU)*(AMTAU**2-AMNUTA**2+AMX**2)
      PR(3)=-PN(3)
      PHSPAC=PHSPAC*(4*PI)*(2*PR(3)/AMTAU)
C
CAM
      ENQ1=(AMX2+AMPI**2-AMPIZ**2)/(2.q0*AMX)
      ENQ2=(AMX2-AMPI**2+AMPIZ**2)/(2.q0*AMX)
...
C AMPLITUDE
     ...
     &    +(GV**2-GA**2)*AMTAU*AMNUTA*QQ2
      ...
      DGAMT=1/(2.q0*AMTAU)*AMPLIT*PHSPAC
      DO 40 I=1,3
...
C ----------------------------------------------------------------------
      COMMON / INOUT / INUT,IOUT
      REAL(16)  POL(4),HV(4),PAA(4),PNU(4),PIM1(4),PIM2(4),PIPL(4)
      REAL(16) RN
      DATA IWARM/0/
C
      IF(MODE.EQ.-1) THEN
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      ...
      COMMON / INOUT / INUT,IOUT
      REAL(16)  HHV(4)
      REAL(16)  HV(4),PAA(4),PNU(4),PIM1(4),PIM2(4),PIPL(4)
      REAL(16)  PDUM1(4),PDUM2(4),PDUM3(4),PDUM4(4),PDUM5(4)
      REAL(16) RRR(3)
      REAL(16) SWT, SSWT
      DATA PI /3.141592653589793238462643q0/
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      ...
      REAL(16)            BRA1,BRK0,BRK0B,BRKS
      REAL(16)  HV(4),PN(4),PAA(4),PIM1(4),PIM2(4),PIPL(4)
...
C                      PKK   KAON CHARGED
C ----------------------------------------------------------------------
      REAL(16)  POL(4),HV(4),PNU(4),PKK(4)
      REAL(16) RN
C     
      IF(MODE.EQ.-1) THEN
C     ===================
...
CZW 7.02.93 here was an error affecting non standard model
C       configurations only
        GAMM=(GFERMI*FKK)**2/(16.q0*PI)*AMTAU**3*
     $       (BRAK/AMTAU**4)*
...
C ----------------------------------------------------------------------
      COMMON / INOUT / INUT,IOUT
      REAL(16)  POL(4),HV(4),PKS(4),PNU(4),PKK(4),PPI(4)
      REAL(16) RN
      DATA IWARM/0/
C
      IF(MODE.EQ.-1) THEN
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      ...
      COMMON / INOUT / INUT,IOUT
      REAL(16)  HHV(4)
      REAL(16)  HV(4),PKS(4),PNU(4),PKK(4),PPI(4)
      REAL(16)  PDUM1(4),PDUM2(4),PDUM3(4),PDUM4(4)
      REAL(16) RRR(3),RMOD
      REAL(16) SWT, SSWT
      DATA PI /3.141592653589793238462643q0/
...
C
     ...
     *                 ,AMPIZ,AMPI,AMRO,GAMRO,AMA1,GAMA1
     *                 ,AMK,AMKZ,AMKST,GAMKST
      REAL(16)  HV(4),PT(4),PN(4),PKS(4),PKK(4),PPI(4),QQ(4)
      REAL(16) RR1
      COMPLEX(16) BWIGS
      DATA PI /3.141592653589793238462643q0/
C
...
C K* MOMENTUM
        PKS(1)=0
        PKS(2)=0
        PKS(4)=1.q0/(2*AMTAU)*(AMTAU**2-AMNUTA**2+AMX**2)
        PKS(3)=-PN(3)
...
C A SIMPLE BREIT-WIGNER IS CHOSEN FOR K* RESONANCE
        FKS= ABS(BWIGS(AMX2,AMKST,GAMKST))**2
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
     ...
     &                ,NAMES
      ...
      REAL(16) WETMAX(20)
C
      REAL(16)  PN(4),PR(4),PPI(4,9),HV(4)
      REAL(16)  PNX(4),PRX(4),PPIX(4,9),HVX(4)
      REAL(16)  PV(5,9),PT(4),UE(3),BE(3)
      REAL(16)  PAWT,AMX,AMS1,AMS2,PA,PHS,PHSMAX,PMIN,PMAX
!!! M.S. to fix underflow >>>
      REAL(16)  PHSPAC
!!! M.S. to fix underflow <<<
      REAL(16)  GAM,BEP,PHI,A,B,C
      REAL(16)  AMPIK
      REAL(16) RRR(9),RRX(2),RR1,RN
C
      DATA PI /3.141592653589793238462643q0/
      DATA WETMAX /20*1q-15/
C
CC--      PAWT(A,B,C)=SQRT((A**2-(B+C)**2)*(A**2-(B-C)**2))/(2.*A)
C
      PAWT(A,B,C)=
     $  SQRT(MAX(0.q0,(A**2-(B+C)**2)*(A**2-(B-C)**2)))/(2.q0*A)
C
...
C TAU-NEUTRINO MOMENTUM
      PN(1)=0
      PN(2)=0
      PN(4)=1.q0/(2*AMTAU)*(AMTAU**2+AMNUTA**2-AMX2)
      PN(3)=-SQRT((PN(4)-AMNUTA)*(PN(4)+AMNUTA))
C W MOMENTUM
      PR(1)=0
      PR(2)=0
      PR(4)=1.q0/(2*AMTAU)*(AMTAU**2-AMNUTA**2+AMX2)
      PR(3)=-PN(3)
      PHSPAC=PHSPAC * (4.q0*PI) * (2.q0*PR(3)/AMTAU)
C
...
C     BRAK= 2*(AMTAU**2-AMX2) * (AMTAU**2+2.*AMX2)
      AMPLIT=CCABIB**2*GFERMI**2/2.q0 * BRAK * AMX2*SIGEE(AMX2,JNPI)
      DGAMT=1.q0/(2.q0*AMTAU)*AMPLIT*PHSPAC
C
...
CAM  GENERATE ND-2 EFFECTIVE MASSES
      PHS=1.q0
      PHSPAC = 1.q0/2.q0**(6*ND-7) /PI**(3*ND-4)
      AMX=AMW
      CALL TAURAN(RRR,ND-2)
      DO 230 IL=1,ND-2
      AMS1=.0q0
      DO 231 JL=IL+1,ND
...
C ---  2.02.94 ZW 1 line 
  ...
  230 CONTINUE
      ...
      CALL TAURAN(RN,1)
      WETMAX(JNPI)=1.2q0*MAX(WETMAX(JNPI)/1.2q0,PHS/PHSMAX)
      IF (NCONT.EQ.500000) THEN
          XNPI=0.0q0
          DO KK=1,ND
...
C...PERFORM SUCCESSIVE TWO-PARTICLE DECAYS IN RESPECTIVE CM FRAME
  280 DO 300 IL=1,ND-1
      PA=PAWT(PV(5,IL),PV(5,IL+1),AMPIK(IL,JNPI))
      CALL TAURAN(RRX,2)
      UE(3)=2.q0*RRX(1)-1.q0
      PHI=2.q0*PI*RRX(2)
      UE(1)=SQRT(1.q0-UE(3)**2)*COS(PHI)
      UE(2)=SQRT(1.q0-UE(3)**2)*SIN(PHI)
      DO 290 J=1,3
      PPI(J,IL)=PA*UE(J)
  290 PV(J,IL+1)=-PA*UE(J)
      PPI(4,IL)=SQRT(PA**2+AMPIK(IL,JNPI)**2)
      PV(4,IL+1)=SQRT(PA**2+PV(5,IL+1)**2)
      PHSPAC=PHSPAC *(4.q0*PI)*(2.q0*PA/PV(5,IL))
  300 CONTINUE
C...LORENTZ TRANSFORM DECAY PRODUCTS TO TAU FRAME
  ...
  320 BE(J)=PV(J,IL)/PV(4,IL)
      ...
      DO 330 J=1,3
  330 PPI(J,I)=PPI(J,I)+GAM*(GAM*BEP/(1.q0+GAM)+PPI(4,I))*BE(J)
...
C                                                                       
      DATA DATSIG/                                                      
     1  7.40q0,12.00q0,16.15q0,21.25q0,24.90q0,29.55q0,34.15q0,37.40q0,
     2 37.85q0,37.40q0,36.00q0,33.25q0,30.50q0,27.70q0,24.50q0,21.25q0,
     3 18.90q0, 1.24q0, 2.50q0, 3.70q0, 5.40q0, 7.45q0,10.75q0,14.50q0,
     4 18.20q0,22.30q0,28.90q0,29.35q0,25.60q0,22.30q0,18.60q0,14.05q0,
     5 11.60q0, 9.10q0,17*.0q0,17*.0q0, 9*.0q0,  .65q0, 1.25q0, 2.20q0,
     6  3.15q0, 5.00q0, 5.75q0, 7.80q0, 8.25q0,17*.0q0/
      DATA SIG0 / 86.8q0 /                                                
      DATA PI /3.141592653589793238462643q0/                              
      DATA INIT / 0 /                                                   
C                          
      ...
      IF(INIT.EQ.0) THEN                                                
        ...
        T2=T**2                                                         
        FACT=(T2/S2)**2*SQRT((S2-T2-AMPI2)**2-4.q0*T2*AMPI2)/S2*
     &                                             2.q0*T*.05q0 
...
C       WRITE(6,1000) DATSIG                                            
 1000   FORMAT(///1X,' EE SIGMA USED IN MULTIPI DECAYS'/                
     %        (17F7.2/))                                                
      ...
      IF(Q.LT.QMIN) THEN                                                
        SIGEE=DATSIG(1,JNPI)+                                           
     &       (DATSIG(2,JNPI)-DATSIG(1,JNPI))*(Q-1.q0)/.05q0                 
      ELSEIF(Q.LT.1.8q0) THEN                                             
        DO 1 I=1,16                                                     
        QMAX = QMIN + .05q0                                               
 ...
 2      SIGEE=DATSIG(I,JNPI)+                                           
     &       (DATSIG(I+1,JNPI)-DATSIG(I,JNPI)) * (Q-QMIN)/.05q0           
      ELSEIF(Q.GT.1.8q0) THEN                                             
        SIGEE=DATSIG(17,JNPI)+                                          
     &       (DATSIG(17,JNPI)-DATSIG(16,JNPI)) * (Q-1.8q0)/.05q0            
      ENDIF                                                             
      IF(SIGEE.LT..0q0) SIGEE=0.q0                                          
C                                                                       
      SIGEE = SIGEE/(6.q0*PI**2*SIG0)                                     
C                                                                       
      RETURN                                                            
      END                                                               
...
C
     ...
     3 18.90q0, 1.24q0, 2.50q0, 3.70q0, 5.40q0, 7.45q0,10.75q0,14.50q0,
     4 18.20q0,22.30q0,28.90q0,29.35q0,25.60q0,22.30q0,18.60q0,14.05q0,
     5 11.60q0, 9.10q0,17*.0q0, 9*.0q0,  .65q0, 1.25q0, 2.20q0, 3.15q0,
     6  5.00q0, 5.75q0, 7.80q0, 8.25q0/
      DATA SIG0 / 86.8q0 /
      DATA PI /3.141592653589793238462643q0/
      DATA INIT / 0 /
C
...
C ----------------------------------------------------------------------
     ...
     &                ,NAMES
      ...
      REAL(16)  HV(4),PN(4),PAA(4),PIM1(4),PIM2(4),PIPL(4),PNPI(4,9)
C MATRIX ELEMENT NUMBER:
      MNUM=JAA
C TYPE OF THE GENERATION:
      KEYT=4
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      COMMON / DECPAR / GFERMI,GV,GA,CCABIB,SCABIB,GAMEL
      REAL(16)            GFERMI,GV,GA,CCABIB,SCABIB,GAMEL
      REAL(16)  HV(4),PT(4),PN(4),PAA(4),PIM1(4),PIM2(4),PIPL(4)
      REAL(16)  PR(4)
      REAL(16) RRR(5)
      DATA PI /3.141592653589793238462643q0/
      DATA ICONT /0/
      XLAM(X,Y,Z)=SQRT(ABS((X-Y-Z)**2-4.0q0*Y*Z))
C AMRO, GAMRO IS ONLY A PARAMETER FOR GETING HIGHT EFFICIENCY
C
C THREE BODY PHASE SPACE NORMALISED AS IN BJORKEN-DRELL
C D**3 P /2E/(2PI)**3 (2PI)**4 DELTA4(SUM P)
...
*       RHO  MOMENTUM
        PR(1)=0
        PR(2)=0
        PR(4)=1.q0/(2*AM3)*(AM3**2+AM2**2-AMP1**2)
        PR(3)= SQRT(ABS(PR(4)**2-AM2**2))
        PPI  =          PR(4)**2-AM2**2
*       PI0 2 MOMENTUM
        PIM2(1)=0
        PIM2(2)=0
        PIM2(4)=1.q0/(2*AM3)*(AM3**2-AM2**2+AMP1**2)
        PIM2(3)=-PR(3)
...
* A1  MOMENTUM
      PAA(1)=0
      PAA(2)=0
      PAA(4)=1.q0/(2*AMTAU)*(AMTAU**2-AMNUTA**2+AM3**2)
      PAA(3)= SQRT(ABS(PAA(4)**2-AM3**2))
      PPI   =          PAA(4)**2-AM3**2
...
* TAU-NEUTRINO MOMENTUM
      PN(1)=0
      PN(2)=0
      PN(4)=1.q0/(2*AMTAU)*(AMTAU**2+AMNUTA**2-AM3**2)
      PN(3)=-PAA(3)
...
C
       ...
       ELSE
         XJAC3=XJAJE
       ENDIF
       IF (XJAC1.NE.0.0q0) A1=PROB1/XJAC1
       IF (XJAC2.NE.0.0q0) A2=PROB2/XJAC2
       IF (XJAC3.NE.0.0q0) A3=PROB3/XJAC3
C
       IF (A1+A2+A3.NE.0.0q0) THEN
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      ...
      REAL(16)            GFERMI,GV,GA,CCABIB,SCABIB,GAMEL
      COMMON /TESTA1/ KEYA1
      REAL(16)  HV(4),PT(4),PN(4),PIM1(4),PIM2(4),PIPL(4)
      REAL(16)  PAA(4),VEC1(4),VEC2(4)
      REAL(16)  PIVEC(4),PIAKS(4),HVM(4)
      COMPLEX(16) BWIGN,HADCUR(4),FPIK
      DATA ICONT /1/
C
...
* HADRON CURRENT SATURATED WITH A1 AND RHO RESONANCES
      IF (KEYA1.EQ.1) THEN
        ...
        FAROPI=1.0q0
        FRO2PI=1.0q0
        FNORM=FA1/SQRT(2.q0)*FAROPI*FRO2PI
        DO 45 I=1,4
        HADCUR(I)= (FNORM) *AMA1**2*BWIGN(XMAA,AMA1,GAMA1)
     $              *((VEC1(I))*AMRO**2*BWIGN(XMRO1,AMRO,GAMRO)
     $               +(VEC2(I))*AMRO**2*BWIGN(XMRO2,AMRO,GAMRO))
 45     CONTINUE
      ELSE
        FNORM=2.0q0*SQRT(2.q0)/3.0q0/FPI
        GAMAX=GAMA1*GFUN(XMAA**2)/GFUN(AMA1**2)
        DO 46 I=1,4
        HADCUR(I)= (FNORM) *AMA1**2*BWIGN(XMAA,AMA1,GAMAX)
     $              *((VEC1(I))*FPIK(XMRO1)
     $               +(VEC2(I))*FPIK(XMRO2))
 46     CONTINUE
      ENDIF
...
* SPIN INDEPENDENT PART OF DECAY DIFF-CROSS-SECT. IN TAU REST  FRAME
      BRAK= (GV**2+GA**2)*PT(4)*PIVEC(4) +2.q0*GV*GA*PT(4)*PIAKS(4)
     &     +2.q0*(GV**2-GA**2)*AMNUTA*AMTAU*BRAKM
      AMPLIT=(GFERMI*CCABIB)**2*BRAK/2.q0
C THE STATISTICAL FACTOR FOR IDENTICAL PI'S WAS CANCELLED WITH
C TWO, FOR TWO MODES OF A1 DECAY NAMELLY PI+PI-PI- AND PI-PI0PI0
C POLARIMETER VECTOR IN TAU REST FRAME
      DO 90 I=1,3
      HV(I)=-(AMTAU*((GV**2+GA**2)*PIAKS(I)+2.q0*GV*GA*PIVEC(I)))
...
C
       IF (QKWA.LT.(AMRO+AMPI)**2) THEN
          GFUN=4.1q0*(QKWA-9*AMPIZ**2)**3
     $        *(1.q0-3.3q0*(QKWA-9*AMPIZ**2)+5.8q0*(QKWA-9*AMPIZ**2)**2)
       ELSE
          GFUN=QKWA*(1.623q0+10.38q0/QKWA-9.32q0/QKWA**2+0.65q0/QKWA**3)
       ENDIF
      END
      COMPLEX(16) FUNCTION BWIGS(S,M,G)
      IMPLICIT REAL(16) (A-H,O-Z)
      IMPLICIT INTEGER (I-N)
C **********************************************************
...
C **********************************************************
      REAL(16) S,M,G
      REAL(16) PI,PIM,QS,QM,W,GS,MK
      DATA INIT /0/
      P(A,B,C)=SQRT(ABS(ABS(((A+B-C)**2-4.q0*A*B)/4.q0/A)
     $                    +(((A+B-C)**2-4.q0*A*B)/4.q0/A))/2.0q0)
C ------------ PARAMETERS --------------------
      IF (INIT.EQ.0) THEN
      INIT=1
      PI=3.141592654q0
...
C -------  BREIT-WIGNER -----------------------
         ...
         W=SQRT(S)
         GS=G*(M/W)*(QS/QM)**3
         BWIGS=M**2/(M**2-S -(0.q0,1.q0)* M*GS)
      RETURN
      END
      COMPLEX(16) FUNCTION BWIG(S,M,G)
      IMPLICIT REAL(16) (A-H,O-Z)
      IMPLICIT INTEGER (I-N)
C **********************************************************
C     P-WAVE BREIT-WIGNER  FOR RHO
...
C -------  BREIT-WIGNER -----------------------
         ENDIF
       IF (S.GT.4.q0*PIM**2) THEN
         QS=SQRT(ABS(ABS(S/4.q0-PIM**2)+(S/4.q0-PIM**2))/2.0q0)
         QM=SQRT(M**2/4.q0-PIM**2)
         W=SQRT(S)
         GS=G*(M/W)*(QS/QM)**3
       ELSE
         GS=0.0q0
       ENDIF
         BWIG=M**2/(M**2-S -(0.q0,1.q0)* M*GS)
      RETURN
      END
      COMPLEX(16) FUNCTION FPIK(W)
      IMPLICIT REAL(16) (A-H,O-Z)
      IMPLICIT INTEGER (I-N)
C **********************************************************
C     PION FORM FACTOR
...
C ----------------------------------------------------------------------
      ...
      COMPLEX(16) HJ(4),HN
C
      HN= HJ(4)*(PN(4))-HJ(3)*(PN(3))
      HH=     (HJ(4)*CONJG(HJ(4))-HJ(3)*CONJG(HJ(3))
     $        -HJ(2)*CONJG(HJ(2))-HJ(1)*CONJG(HJ(1)))
      DO 10 I=1,4
   10 PIV(I)=4.q0*    (HN*CONJG(HJ(I)))-2.q0*HH*PN(I)
      RETURN
      END
...
C ----------------------------------------------------------------------
      ...
      COMMON / IDFC  / IDFF
      REAL(16) PIA(4),PN(4)
      COMPLEX(16) HJ(4),HJC(4)
C     DET2(I,J)=AIMAG(HJ(I)*HJC(J)-HJ(J)*HJC(I))
C -- here was an error (ZW, 21.11.1991)
***MS      DET2(I,J)AIMAG(HJC(I)*HJ(J)-HJC(J)*HJ(I))
      DET2(I,J)=(0,-1)*(HJC(I)*HJ(J)-HJC(J)*HJ(I))
...
C
      DO 10 I=1,4
 10   HJC(I)=CONJG(HJ(I))
      PIA(1)= -2.q0*PN(3)*DET2(2,4)+2.q0*PN(4)*DET2(2,3)
      PIA(2)= -2.q0*PN(4)*DET2(1,3)+2.q0*PN(3)*DET2(1,4)
      PIA(3)=  2.q0*PN(4)*DET2(1,2)
      PIA(4)=  2.q0*PN(3)*DET2(1,2)
...
C
      CALL CLAXI(HJ,P,HV)
***MS      B=REAL( HJ(4)*AIMAG(HJ(4)) - HJ(3)*AIMAG(HJ(3))
***MS     C      - HJ(2)*AIMAG(HJ(2)) - HJ(1)*AIMAG(HJ(1))  )
      B= (HJ(4)**2 - HJ(3)**2 - HJ(2)**2 - HJ(1)**2)*(0,-0.5q0)
      RETURN
      END
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      ...
      REAL(16)  PAA(4),VEC1(4),VEC2(4)
      REAL(16)  PIVEC(4),PIAKS(4),HVM(4)
      COMPLEX(16) BWIGN,HADCUR(4),FNORM,FORMOM
      DATA ICONT /1/
* THIS INLINE FUNCT. CALCULATES THE SCALAR PART OF THE PROPAGATOR
      BWIGN(XM,AM,GAMMA)=1.q0/(XM**2-AM**2 +(0.q0,1.q0)*GAMMA*AM)
C
...
* CALCULATE PI-VECTORS: VECTOR AND AXIAL
      ...
      CALL CLNUT(HADCUR,BRAKM,HVM)
* SPIN INDEPENDENT PART OF DECAY DIFF-CROSS-SECT. IN TAU REST  FRAME
      BRAK=BRAK+(GV**2+GA**2)*PT(4)*PIVEC(4) +2.q0*GV*GA*PT(4)*PIAKS(4)
     &         +2.q0*(GV**2-GA**2)*AMNUTA*AMTAU*BRAKM
      DO 90 I=1,3
      HV(I)=HV(I)-(AMTAU*((GV**2+GA**2)*PIAKS(I)+2.q0*GV*GA*PIVEC(I)))
     &      +(GV**2-GA**2)*AMNUTA*AMTAU*HVM(I)
  90  CONTINUE
C HV IS DEFINED FOR TAU-    WITH GAMMA=B+HV*POL
 120  CONTINUE
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      COMMON / DECPAR / GFERMI,GV,GA,CCABIB,SCABIB,GAMEL
      REAL(16)            GFERMI,GV,GA,CCABIB,SCABIB,GAMEL
      REAL(16)  HV(4),PT(4),PN(4),PIM1(4),PIM2(4),PIM3(4)
      REAL(16)  PAA(4),VEC1(4),VEC2(4),VEC3(4),VEC4(4),VEC5(4)
      REAL(16)  PIVEC(4),PIAKS(4),HVM(4)
      REAL(16) FNORM(0:7),COEF(1:5,0:7)
      COMPLEX(16) HADCUR(4),FORM1,FORM2,FORM3,FORM4,FORM5,UROJ
...
C
      ...
      IF (ICONT.EQ.0) THEN
       ...
       FNORM(7)=CCABIB/FPI
C
       COEF(1,0)= 2.0q0*SQRT(2.q0)/3.0q0
       COEF(2,0)=-2.0q0*SQRT(2.q0)/3.0q0
       COEF(3,0)= 0.0q0
       COEF(4,0)= FPI
       COEF(5,0)= 0.0q0
C
       COEF(1,1)=-SQRT(2.q0)/3.0q0
       COEF(2,1)= SQRT(2.q0)/3.0q0
       COEF(3,1)= 0.0q0
       COEF(4,1)= FPI
       COEF(5,1)= SQRT(2.q0)
C
       COEF(1,2)=-SQRT(2.q0)/3.0q0
       COEF(2,2)= SQRT(2.q0)/3.0q0
       COEF(3,2)= 0.0q0
       COEF(4,2)= 0.0q0
       COEF(5,2)=-SQRT(2.q0)
C
       ...
       COEF(5,3)= 0.0q0
C
       COEF(1,4)= 1.0q0/SQRT(2.q0)/3.0q0
       COEF(2,4)=-1.0q0/SQRT(2.q0)/3.0q0
       COEF(3,4)= 0.0q0
       COEF(4,4)= 0.0q0
       COEF(5,4)= 0.0q0
C
       COEF(1,5)=-SQRT(2.q0)/3.0q0
       COEF(2,5)= SQRT(2.q0)/3.0q0
       COEF(3,5)= 0.0q0
       COEF(4,5)= 0.0q0
       COEF(5,5)=-SQRT(2.q0)
C
...
C be aware that sign of vec2 is opposite to sign of vec1 in a1 case
      DO 45 I=1,4
      HADCUR(I)= (FNORM(MNUM)) * (
     $(VEC1(I)*COEF(1,MNUM))*FORM1(MNUM,XMAA**2,XMRO1**2,XMRO2**2)+
     $(VEC2(I)*COEF(2,MNUM))*FORM2(MNUM,XMAA**2,XMRO2**2,XMRO1**2)+
     $(VEC3(I)*COEF(3,MNUM))*FORM3(MNUM,XMAA**2,XMRO3**2,XMRO1**2)+
     *(-1.0q0*UROJ)*
     $(VEC4(I)*COEF(4,MNUM))*FORM4(MNUM,XMAA**2,XMRO1**2,
     $                                      XMRO2**2,XMRO3**2)         +
     $(-1.0q0)*UROJ/4.0q0/PI**2/FPI**2*
     $(VEC5(I)*COEF(5,MNUM))*FORM5(MNUM,XMAA**2,XMRO1**2,XMRO2**2))
 45   CONTINUE
C
* CALCULATE PI-VECTORS: VECTOR AND AXIAL
      CALL CLVEC(HADCUR,PN,PIVEC)
...
C ----------------------------------------------------------------------
      COMMON / INOUT / INUT,IOUT
      REAL(16)  POL(4),HV(4),PAA(4),PNU(4),PNPI(4,9)
      REAL(16) RN
      DATA IWARM/0/
C
      IF(MODE.EQ.-1) THEN
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
     ...
     &                ,NAMES
      ...
      REAL(16) PNU(4),PWB(4),PNPI(4,9),HV(4),HHV(4)
      REAL(16) PDUM1(4),PDUM2(4),PDUMI(4,9)
      REAL(16) RRR(3)
      REAL(16) WTMAX(NMODE)
      REAL(16)              SWT(NMODE),SSWT(NMODE)
...
C       PRINT 7003
        ...
        DO  I=1,500
          ...
          ELSE
           GOTO 903
          ENDIF   
        IF(WT.GT.WTMAX(JNPI)/1.2q0) WTMAX(JNPI)=WT*1.2q0
        ENDDO
...
C     =======================
        DO 500 JNPI=1,NMOD
          ...
          PARGAM=SWT(JNPI)/ (1.q0*NEVRAW(JNPI)+1)
          ERROR=0
          IF(NEVRAW(JNPI).NE.0)
     &    ERROR=SQRT(SSWT(JNPI)/SWT(JNPI)**2-1.q0/ (1.q0*NEVRAW(JNPI)))
          RAT=PARGAM/GAMEL
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
     ...
     &         PMULT(4,9)
      ...
      REAL(16) UU,FF,FF1,FF2,FF3,FF4,GG1,GG2,GG3,GG4,RR
      DATA PI /3.141592653589793238462643q0/
      DATA ICONT /0/
      XLAM(X,Y,Z)=SQRT(ABS((X-Y-Z)**2-4.0q0*Y*Z))
C AMRO, GAMRO IS ONLY A PARAMETER FOR GETING HIGHT EFFICIENCY
...
*       PR   MOMENTUM
        PR(1)=0
        PR(2)=0
        PR(4)=1.q0/(2*AM3)*(AM3**2+AM2**2-AMP2**2)
        PR(3)= SQRT(ABS(PR(4)**2-AM2**2))
        PPI  =          PR(4)**2-AM2**2
*       PIM1  MOMENTUM
        PIM1(1)=0
        PIM1(2)=0
        PIM1(4)=1.q0/(2*AM3)*(AM3**2-AM2**2+AMP2**2)
        PIM1(3)=-PR(3)
C --- this part of jacobian will be recovered later
        FF3=(4*PI)*(2*PR(3)/AM3)
...
*       PR   MOMENTUM
        PR(1)=0
        PR(2)=0
        PR(4)=1.q0/(2*AM4)*(AM4**2+AM3**2-AMP1**2)
        PR(3)= SQRT(ABS(PR(4)**2-AM3**2))
        PPI  =          PR(4)**2-AM3**2
*       PIM2 MOMENTUM
        PIM2(1)=0
        PIM2(2)=0
        PIM2(4)=1.q0/(2*AM4)*(AM4**2-AM3**2+AMP1**2)
        PIM2(3)=-PR(3)
C --- this part of jacobian will be recovered later
        FF4=(4*PI)*(2*PR(3)/AM4)
...
* PAA  MOMENTUM
      PAA(1)=0
      PAA(2)=0
      PAA(4)=1.q0/(2*AMTAU)*(AMTAU**2-AMNUTA**2+AM4**2)
...
* TAU-NEUTRINO MOMENTUM
      PN(1)=0
      PN(2)=0
      PN(4)=1.q0/(2*AMTAU)*(AMTAU**2+AMNUTA**2-AM4**2)
      PN(3)=-PAA(3)
...
C --- JACOBIAN AVERAGED OVER THE TWO
        IF ( ( (FF+GG)*UU+FF*GG ).GT.0.0q0) THEN
          RR=FF*GG*UU/(0.5q0*PREZ*(FF+GG)*UU+(1.0q0-PREZ)*FF*GG)
          PHSPAC=PHSPAC*RR
        ELSE
          PHSPAC=0.0q0
        ENDIF
...
C
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST
      COMMON / DECPAR / GFERMI,GV,GA,CCABIB,SCABIB,GAMEL
      REAL(16)            GFERMI,GV,GA,CCABIB,SCABIB,GAMEL
      REAL(16)  HV(4),PT(4),PN(4),PIM1(4),PIM2(4),PIM3(4),PIM4(4)
      REAL(16)  PIVEC(4),PIAKS(4),HVM(4)
      COMPLEX(16) HADCUR(4),FORM1,FORM2,FORM3,FORM4,FORM5
      EXTERNAL FORM1,FORM2,FORM3,FORM4,FORM5
      DATA PI /3.141592653589793238462643q0/
      DATA ICONT /0/
C
...
* SPIN INDEPENDENT PART OF DECAY DIFF-CROSS-SECT. IN TAU REST  FRAME
      BRAK= (GV**2+GA**2)*PT(4)*PIVEC(4) +2.q0*GV*GA*PT(4)*PIAKS(4)
     &     +2.q0*(GV**2-GA**2)*AMNUTA*AMTAU*BRAKM
      AMPLIT=(CCABIB*GFERMI)**2*BRAK/2.q0
C POLARIMETER VECTOR IN TAU REST FRAME
      DO 90 I=1,3
      HV(I)=-(AMTAU*((GV**2+GA**2)*PIAKS(I)+2.q0*GV*GA*PIVEC(I)))
     &      +(GV**2-GA**2)*AMNUTA*AMTAU*HVM(I)
...
C                                                                       
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST                           
     ...
     &                ,NAMES
      CHARACTER NAMES(NMODE)*31
      REAL(16)  HV(4),PT(4),PN(4),PAA(4),PMULT(4,9) 
      REAL(16) PR(4),PI1(4),PI2(4),PI3(4),PI4(4),PI5(4)                   
      REAL(16) AMP1,AMP2,AMP3,AMP4,AMP5,ams1,ams2,amom,gamom
      REAL(16) AM5SQ,AM4SQ,AM3SQ,AM2SQ,AM5,AM4,AM3
      REAL(16) RRR(10)
      REAL(16) gg1,gg2,gg3,ff1,ff2,ff3,ff4,alp,alp1,alp2
      REAL(16) XM,AM,GAMMA
ccM.S.>>>>>>
      REAL(16) phspac
ccM.S.<<<<<<
...
C                                                                       
      ...
      BWIGN(XM,AM,GAMMA)=XM**2/(XM**2-AM**2 +(0.q0,1.q0)*GAMMA*AM)              
C                              
      AMOM=.782q0                                                         
      GAMOM=0.0085q0                                                      
c                                                                       
...
c pi2   momentum                                                        
      pi2(1)=0                                                          
      pi2(2)=0                                                          
      pi2(4)=1.q0/(2*am3)*(am3**2-am2**2+amp2**2)                         
      pi2(3)=-pr(3)                                                     
c --- this part of jacobian will be recovered later                     
      ff2=(4*pi)*(2*pr(3)/am3)                                          
...
c pr   momentum                                                         
      pr(1)=0                                                           
      pr(2)=0                                                           
      pr(4)=1.q0/(2*am4)*(am4**2+am3**2-amp5**2)                          
      pr(3)= sqrt(abs(pr(4)**2-am3**2))                                 
      ppi  =          pr(4)**2-am3**2                                   
c pi5  momentum                                                         
      pi5(1)=0                                                          
      pi5(2)=0                                                          
      pi5(4)=1.q0/(2*am4)*(am4**2-am3**2+amp5**2)                         
      pi5(3)=-pr(3)                                                     
c --- this part of jacobian will be recovered later                     
      ff3=(4*pi)*(2*pr(3)/am4)                                          
...
c pr   momentum                                                         
      pr(1)=0                                                           
      pr(2)=0                                                           
      pr(4)=1.q0/(2*am5)*(am5**2+am4**2-amp1**2)                          
      pr(3)= sqrt(abs(pr(4)**2-am4**2))                                 
      ppi  =          pr(4)**2-am4**2                                   
c pi1  momentum                                                         
      pi1(1)=0                                                          
      pi1(2)=0                                                          
      pi1(4)=1.q0/(2*am5)*(am5**2-am4**2+amp1**2)                         
      pi1(3)=-pr(3)                                                     
c --- this part of jacobian will be recovered later                     
      ff4=(4*pi)*(2*pr(3)/am5)                                          
...
c     ppi   =          paa(4)**2-am5**2                                 
      paa(4)=1.q0/(2*amtau)*(amtau**2-amnuta**2+am5sq)                    
...
* tau-neutrino momentum                                                 
      pn(1)=0                                                           
      pn(2)=0                                                           
      pn(4)=1.q0/(2*amtau)*(amtau**2+amnuta**2-am5**2)                    
      pn(3)=-paa(3)                                                     
c                                                                       
...
c     AMPLIT=CCABIB**2*GFERMI**2/2. * BRAK * AM5SQ*SIGEE(AM5SQ,JNPI)    
      AMPLIT=CCABIB**2*GFERMI**2/2.q0 * BRAK                              
...
C                                                                       
     ...
     *                 ,AMK,AMKZ,AMKST,GAMKST                           
      COMMON / DECPAR / GFERMI,GV,GA,CCABIB,SCABIB,GAMEL                
      REAL(16)            GFERMI,GV,GA,CCABIB,SCABIB,GAMEL                
      REAL(16)  HV(4),PT(4),PN(4),PR(4),PKC(4),PKZ(4),QQ(4),PMULT(4,9) 
      REAL(16) RR1
      DATA PI /3.141592653589793238462643q0/                              
      DATA ICONT /0/                                                    
C                                                                       
...
C                                                                       
CAM                                                                     
      ENQ1=(AMX2+AMK**2-AMKZ**2)/(2.q0*AMX)                               
      ENQ2=(AMX2-AMK**2+AMKZ**2)/(2.q0*AMX)                               
...
c     FPIRK= ABS(FPIKMK(W,AMK,AMKZ))**2                                 
      END                                                               
      COMPLEX(16) FUNCTION FPIKMK(W,XM1,XM2)                                
      IMPLICIT REAL(16) (A-H,O-Z)
      IMPLICIT INTEGER (I-N)
C **********************************************************            
C     Kaon form factor                                                  
...
C FILLING OF THE LUND COMMON BLOCK.
        DO 1002 I=1,4
 1002   QHOT(I)=PHX(I)
        IF (QHOT(4).GT.1.q-5) CALL DWLUPH(KTO,QHOT)
        RETURN
      END
...
C boost and append photon (gamma is 22)
      ...
      CALL FILHEP(0,1,22,NPS,NPS,0,0,PHOT,0.0q0,.TRUE.)
C
      RETURN
      END
...
C ----------------------------------------------------------------------
C
      REAL(16)  PNU(4),PWB(4),PEL(4),PNE(4)
      COMMON /TAUPOS/ NP1,NP2
C
C position of decaying particle:
      IF(KTO.EQ. 1) THEN
...
C ----------------------------------------------------------------------
C
      REAL(16)  PNU(4),PWB(4),PMU(4),PNM(4)
      COMMON /TAUPOS/ NP1,NP2
C
C position of decaying particle:
      IF(KTO.EQ. 1) THEN
...
C ----------------------------------------------------------------------
C
      REAL(16)  PNU(4),PRHO(4),PIC(4),PIZ(4)
      COMMON /TAUPOS/ NP1,NP2
C
C position of decaying particle:
      IF(KTO.EQ. 1) THEN
...
C ----------------------------------------------------------------------
C
      REAL(16)  PNU(4),PAA(4),PIM1(4),PIM2(4),PIPL(4)
      COMMON /TAUPOS/ NP1,NP2
C
C position of decaying particle:
      IF(KTO.EQ. 1) THEN
...
C ----------------------------------------------------------------------
C
      REAL(16)  PNU(4),PKS(4),PKK(4),PPI(4)
      COMMON /TAUPOS/ NP1,NP2
C
C position of decaying particle
      IF(KTO.EQ. 1) THEN
...
C
      IF(AAA.NE.0.0q0) AAA=AAA/SQRT(ABS(AAA))
      AMAST=AAA
      RETURN
      END
      FUNCTION AMAS4(PP)
...
C
      CALL TAURAN(RRR,2)
      COSTH=-1.q0+2.q0*RRR(1)
      SINTH=SQRT(1.q0-COSTH**2)
      X(1)=R*SINTH*COS(2*PI*RRR(2))
      X(2)=R*SINTH*SIN(2*PI*RRR(2))
      X(3)=R*COSTH
      RETURN
...
C
      ...
      RETURN
      END
      SUBROUTINE TAURAN(DRVEC,LENGT)
C     ***************************
C jvw copy of routine varran
C Switchable random number generator
C Translation to real(16)
C     ***************************
...
C random numbers 0 and 1 not accepted
      ...
      IF(RVEC(I).LE.0E0.OR.RVEC(I).GE.1E0) THEN
        WRITE(6,*) ' +++++ TAURAN: RVEC=',RVEC(I)
        GOTO 10
      ENDIF
      DRVEC(I)=RVEC(I)
   30 CONTINUE
      RETURN
  901 WRITE(6,*) ' +++++ STOP IN TAURAN: LENGT=',LENGT
      STOP
  902 WRITE(6,*) ' +++++ STOP IN TAURAN: WRONG KEYRND',KEYRND
      STOP
      END      
...
C          always come here to initialize
 ...
 201  FORMAT(1X,' RANMAR INITIALIZED: ',I10,2X,2I10)
    ...
    2 U(II) = S
      TWOM24 = 1.0q0
      DO 4 I24= 1, 24
    4 TWOM24 = 0.5q0*TWOM24
      C  =   362436.q0*TWOM24
      CD =  7654321.q0*TWOM24
      CM = 16777213.q0*TWOM24
      I97 = 97
      J97 = 33
...
C             An exact zero here is very unlikely, but let's be safe.
         IF (UNI .EQ. 0.q0) UNI= TWOM24*TWOM24
         ENDIF
      RVEC(IVEC) = UNI
  100 CONTINUE
      NTOT = NTOT + LENV
...
CERN      C304      VERSION    29/07/71 DILOG        59                C
      Z=-1.64493406684822q0
      IF(X .LT.-1.0q0) GO TO 1
      IF(X .LE. 0.5q0) GO TO 2
      IF(X .EQ. 1.0q0) GO TO 3
      IF(X .LE. 2.0q0) GO TO 4
      Z=3.2898681336964q0
    1 T=1.0q0/X
      S=-0.5q0
      Z=Z-0.5q0* LOG(ABS(X))**2
      GO TO 5
    2 T=X
      S=0.5q0
      Z=0.q0
      GO TO 5
    3 DILOGT=1.64493406684822q0
      RETURN
    4 T=1.0q0-X
      S=-0.5q0
      Z=1.64493406684822q0 - LOG(X)* LOG(ABS(T))
    5 Y=2.66666666666666q0 *T+0.66666666666666q0
      B=      0.000000000000001q0
      A=Y*B  +0.000000000000004q0
      B=Y*A-B+0.000000000000011q0
      A=Y*B-A+0.000000000000037q0
      B=Y*A-B+0.000000000000121q0
      A=Y*B-A+0.000000000000398q0
      B=Y*A-B+0.000000000001312q0
      A=Y*B-A+0.000000000004342q0
      B=Y*A-B+0.000000000014437q0
      A=Y*B-A+0.000000000048274q0
      B=Y*A-B+0.000000000162421q0
      A=Y*B-A+0.000000000550291q0
      B=Y*A-B+0.000000001879117q0
      A=Y*B-A+0.000000006474338q0
      B=Y*A-B+0.000000022536705q0
      A=Y*B-A+0.000000079387055q0
      B=Y*A-B+0.000000283575385q0
      A=Y*B-A+0.000001029904264q0
      B=Y*A-B+0.000003816329463q0
      A=Y*B-A+0.000014496300557q0
      B=Y*A-B+0.000056817822718q0
      A=Y*B-A+0.000232002196094q0
      B=Y*A-B+0.001001627496164q0
      A=Y*B-A+0.004686361959447q0
      B=Y*A-B+0.024879322924228q0
      A=Y*B-A+0.166073032927855q0
      A=Y*A-B+1.93506430086996q0
      DILOGT=S*T*(A-B)+Z
      RETURN
C=======================================================================
C===================END OF CPC PART ====================================
