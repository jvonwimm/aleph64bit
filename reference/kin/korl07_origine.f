...
C    in mode =1 ( end) KEYYFS is not used
      KEYYFS = 0
...
C       =  2, single bremsstrahlung, no QED ini-fin interference
C             note that internal variable KEYRAD is set to 1.
C KEYRAD= 10, single bremsstralung but exponentiaited spectrum
C       = 11, YFS2/YFS3 generation; backward compatibility option in TRA
...
C**     CUT OFF VALUES
        ...
        IF(KEYRAD.LE.1.AND.XK0.LT..0025D0)            GOTO 908
...
C
 7000 FORMAT(///1X,15(5HKKKKK)
     $ /,' K',     25X,'======== KORALZ VERSION 4.0 ===========',9X,1HK
     $ /,' K',     25X,'==To be published in Comp Phys Comm====',9X,1HK
     $ /,' K',     25X,'=last registerd corrections Nov 21, 93=',9X,1HK
...
cam   AMRO   = 0.773
cam   GAMRO  = 0.145
      AMA1   =  PMAS(LUCOMP(ia1),1)       ! AMA1   = 1.251
      GAMA1  =  PMAS(LUCOMP(ia1),2)       ! GAMA1  = 0.599
...
C** SOFT REAL AND VIRTUAL PHOTON CORRECTIONS - INITIALIZATION
     ...
     $           + ALM**2-ALP**2-2D0*DILOGY(UM/2D0)+2D0*DILOGY(UP/2D0)
...
*** PARAMETER an IMPLICIT interchanged in UURHO
...
C ...Beta0, up to first order
      CALL NDIST0(XX,P1,P2,YF1,YF2,ANDIS,DELI1,DELI2,DELF1,DELF2)
...
C ...Initial only
      ...
      IF(ICONT.LE.ICLIM) CALL NTEST0(XX,P1,P2,YF1,YF2)
...
C ...First INITIAL state radiation
   ...
   50   XPH1(K)=XPHOT(JPH,K)
        ...
        SFACX(JPH) = SFACJ
        CALL NDIST1(XX,P1,P2,YF1,YF2,XPH1,DIST,DXLI1)
        DIST11=  DIST*(1D0+DXLI1)*(1+DELF1)
        DIST10=  DIST
        DISI11=  DIST*(1D0+DXLI1)
        IF(ICONT.LE.ICLIM) CALL NTEST1(XX,P1,P2,YF1,YF2,XPH1)
...
C note minus sign here!!!!
  115   XPH1(K)=-YPHOT(JPH,K)
        CALL SFACH0(YF1,YF2,XPH1,SFACJ)
        SFACY(JPH) = SFACJ
        CALL FDIST1(XX,P1,P2,YF1,YF2,XPH1,DIST,DYLF1)
        DIST11=  DIST*(1D0+DYLF1)*(1+DELI1)
        DIST10=  DIST
        IF(ICONT.LE.ICLIM) CALL NTEST1(XX,YF1,YF2,P1,P2,XPH1)
...
C contributions from beta2, pure INITIAL state
  ...
  150       XPH2(K)=XPHOT(JPH2,K)
         CALL NDIST2(XX,P1,P2,YF1,YF2,XPH1,XPH2,DIST2)
         IF(ICONT.LE.ICLIM) CALL NTEST2(XX,P1,P2,YF1,YF2,XPH1,XPH2)
...
C note minus sign here!!!!
  ...
  218       XPH2(K)=-YPHOT(JPH2,K)
         CALL FDIST2(XX,P1,P2,YF1,YF2,XPH1,XPH2,DIST2)
         IF(ICONT.LE.ICLIM) CALL NTEST2(XX,P1,P2,YF1,YF2,XPH1,XPH2)
...
C note minus sign here!!!!
  230       XPH2(K)=-YPHOT(JPH2,K)
         CALL NFDIST(XX,P1,P2,YF1,YF2,XPH1,XPH2,DIST2)
...
C ...Model weight (the best)
      WTMDL     =   WTSET(73)
C ...tests on weights
CC    IF(ICONT.LE.ICLIM2) CALL DUMPBT(IOUT)
CC    IF(ICONT.LE.ICLIM2) CALL DUMPBT(   6)
c      IF(ICONT.LE.ICLIM2.AND.NPHOY.GT.3) CALL DUMPBT(IOUT)
...
C ......................Output window B...............................
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) '          MODEL  output - window B'
      WRITE(IOUT,BXTXT) '          FINAL   state only      '
      WRITE(IOUT,BXTXT) 'Analytical estimates of X-sections'
      WRITE(IOUT,BXTXT) 'Units of Born X-section           '
      PREC = 1D-5
      YS00 =      BREFKF( 300,PREC)
      YS01 =      BREFKF( 301,PREC)
      YS02 =      BREFKF( 302,PREC)
      WRITE(IOUT,BXL1F) YS02,'X-section','O(alf2)','B1'
      WRITE(IOUT,BXL1F) YS01,'X-section','O(alf1)','B2'
      WRITE(IOUT,BXL1F) YS00,'X-section','O(alf0)','B3'
      WRITE(IOUT,BXL1F) YS02-YS01,'(O(alf2)-O(alf1))','/O(alf1)','B4'
      WRITE(IOUT,BXL1F) YS01-YS00,'(O(alf1)-O(alf0))','/O(alf0)','B5'
      YS20 = BREFKF( 320,PREC)
      YS21 = BREFKF( 321,PREC)
      YS22 = BREFKF( 322,PREC)
      YS10 = BREFKF( 310,PREC)
      YS11 = BREFKF( 311,PREC)
      WRITE(IOUT,BXL1F) YS20,'Beta0          ','O(alf2)','B06'
      WRITE(IOUT,BXL1F) YS21,'     Beta1     ','       ','B07'
      WRITE(IOUT,BXL1F) YS22,'          Beta2','       ','B08'
      WRITE(IOUT,BXL1F) YS10,'Beta0          ','O(alf1)','B09'
      WRITE(IOUT,BXL1F) YS11,'     Beta1     ','       ','B10'
      WRITE(IOUT,BXCLO)
C --------------------------------------------------------------------
C ......................Output window C...............................
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) '             MODEL  output - window C'
      WRITE(IOUT,BXTXT) ' FINAL   State  only'
      WRITE(IOUT,BXTXT) 'Comparison of MC and Analytical calc.'
      WRITE(IOUT,BXTXT) '(MonteCarlo  - Analytical)/Analytical'
      RS01 = XS01/YS00 -1
      RS02 = XS02/YS01 -1
      RS03 = XS03/YS02 -1
      WRITE(IOUT,BXL2F) RS03,DXS03/YS02,'X-section','O(alf2)','C1'
      WRITE(IOUT,BXL2F) RS02,DXS02/YS01,'X-section','O(alf1)','C2'
      WRITE(IOUT,BXL2F) RS01,DXS01/YS00,'X-section','O(alf0)','C3'
      WRITE(IOUT,BXTXT) 'Beta contributions'
      RS20 = XS20/YS20 -1
      RS21 = XS21/YS21 -1
      RS22 = XS22/YS22 -1
      RS10 = XS10/YS10 -1
      ...
      DRS10 = DXS10/YS10
      DRS11 = DXS11/YS11
      WRITE(IOUT,BXL2F) RS20,DRS20,'Beta0          ','O(alf2)','C04'
      WRITE(IOUT,BXL2F) RS21,DRS21,'     Beta1     ','       ','C05'
      WRITE(IOUT,BXL2F) RS22,DRS22,'          Beta2','       ','C06'
      WRITE(IOUT,BXL2F) RS10,DRS10,'Beta0          ','O(alf1)','C07'
      WRITE(IOUT,BXL2F) RS11,DRS11,'     Beta1     ','       ','C08'
      WRITE(IOUT,BXCLO)
...
C ........................Output window A...............................
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) '        MODEL  output - window A '
      WRITE(IOUT,BXTXT) '  Monte Carlo INITIAL state ONLY '
      WRITE(IOUT,BXTXT) '           X-sections in R-units '
...
C))))))))
      ...
      XS04   =  XKARL*AVERWT
      DXS04  =  XKARL*AVERWT*SQRT(ERKARL**2+ERRELA**2)
      WRITE(IOUT,BXL2F) XS03,DXS03,'X-section','O(alf2)',  'A01'
      WRITE(IOUT,BXL2F) XS02,DXS02,'X-section','O(alf1)',  'A02'
      WRITE(IOUT,BXL2F) XS01,DXS01,'X-section','O(alf0)',  'A03'
      IF(XS02.NE.0D0) WRITE(IOUT,BXL2F)
     $ XS05/XS02,DXS05/XS02,'(O(alf2)-O(alf1))','/O(alf1)','A04'
      IF(XS01.NE.0D0) WRITE(IOUT,BXL2F)
...
C ...Beta contributions absolute
      ...
      XS11   =  XKARL*AVERWT
      DXS11  =  XKARL*AVERWT*SQRT(ERKARL**2+ERRELA**2)
      WRITE(IOUT,BXTXT) 'Beta contributions in R-units'
      WRITE(IOUT,BXL2F) XS20,DXS20,'Beta0          ','O(alf2)','A06'
      WRITE(IOUT,BXL2F) XS21,DXS21,'     Beta1     ','       ','A07'
      WRITE(IOUT,BXL2F) XS22,DXS22,'          Beta2','       ','A08'
      WRITE(IOUT,BXL2F) XS10,DXS10,'Beta0          ','O(alf1)','A09'
      WRITE(IOUT,BXL2F) XS11,DXS11,'     Beta1     ','       ','A10'
      WRITE(IOUT,BXL2F) XS01,DXS01,'Beta0          ','O(alf0)','A11'
C ...Beta contributions relative
      WRITE(IOUT,BXTXT) 'Beta contributions - relative'
      IF(XS03.NE.0D0) WRITE(IOUT,BXL2F)
...
C ......................Output window B...............................
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) '          MODEL  output - window B'
      WRITE(IOUT,BXTXT) '          INITIAL state only cont.'
      WRITE(IOUT,BXTXT) 'Analytical estimates of X-sections'
      PREC = 1D-5
...
C ......................Output window C...............................
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) '             MODEL  output - window C'
      WRITE(IOUT,BXTXT) '                  Initial State  only'
      WRITE(IOUT,BXTXT) 'Comparison of MC and Analytical calc.'
      WRITE(IOUT,BXTXT) '(MonteCarlo  - Analytical)/Analytical'
      RS01 = XS01/YS00 -1
      RS02 = XS02/YS01 -1
...
C ...the end of the initial-state report..............................
      ELSE
      WRITE(IOUT,*) ' +++++ WRONG MODE IN MONIN'
      ENDIF
C     =====
      END
      SUBROUTINE DUMPBT(IOUT)
C     ***********************
...
C ...Beta0
      WRITE(IOUT,2000) ' Beta00, Beta01/Beta00, Beta02/Beta00'
     & ,BETA00,BETA01/BETA00,BETA02/BETA00
C ...SFACI(i)
 2002 FORMAT(A16,4E16.7/(5E16.7))
      IF(NPHOX.GT.0) WRITE(IOUT,2002) ' SFACX(i) ',(SFACX(I),I=1,NPHOX)
      IF(NPHOY.GT.0) WRITE(IOUT,2002) ' SFACY(i) ',(SFACY(I),I=1,NPHOY)
C ...Beta1   INITIAL state ...............
      IF(NPHOX.GT.0) WRITE(IOUT,2000) ' Init. BetX10(i)/Beta00 ',
     $  ( BETX10(I)/SFACX(I)/BETA00  ,I=1,NPHOX)
      IF(NPHOX.GT.0) WRITE(IOUT,2000) ' Init. BetX11(i)/Beta00 ',
     $  ( BETX11(I)/SFACX(I)/BETA00  ,I=1,NPHOX)

C ...Beta1   final state ...............
      IF(NPHOY.GT.0) WRITE(IOUT,2000) ' Fin.  BetY10(i)/Beta00 ',
     $  ( BETY10(I)/SFACY(I)/BETA00  ,I=1,NPHOY)
      IF(NPHOY.GT.0) WRITE(IOUT,2000) ' Fin.  BetY11(i)/Beta00 ',
     $  ( BETY11(I)/SFACY(I)/BETA00  ,I=1,NPHOY)

C ...Beta2 pure initial
      IF(NPHOX.GE.2) THEN
      WRITE(IOUT,*) ' Init. state BetXX20(i,j)'
      DO 100 J=2,NPHOX
      WRITE(IOUT,'(  8F12.8)')
...
C ...Beta2 pure final
      IF(NPHOY.GE.2) THEN
      WRITE(IOUT,*) ' Final state BetYY20(i,j)'
      DO 110 J=2,NPHOY
      WRITE(IOUT,'(  8F20.16)')
...
C ...Beta2 initial/final
      ...
      WRITE(IOUT,*) ' In/fi state BetXY20(i,j)'
      DO 120 J=1,NPHOY
      WRITE(IOUT,'(  8F12.8)')
...
C
  ...
  10  QQR(K)=QR1(K)+QR2(K)-QQ1(K)
      ...
      WRITE(IOUT,'(A,3F20.12)') ' COSTH= ',COSTH
      END
...
C Single bremsstrahlung Xsection
      ...
      WRITE(IOUT,'(A,3F20.12)') ' PH(4)= ',PH(4)
      WRITE(IOUT,'(A,3F20.12)') ' COSTH= ',COSTH1-COSTH,COSTH2-COSTH
      WRITE(IOUT,'(A,3F20.12)') ' GF/SF= ',(GF1+GF2)/SFACJ
      END
...
C Single bremsstrahlung Xsection
      ...
      CALL REDUZ0(QQ,P1,P2,PX1,PX2)
      CALL GTHET0(PX1,QR1,COSTH)
      WRITE(IOUT,'(A,3F20.12)') 'PHi(4)= ',PH1(4),PH2(4)
      WRITE(IOUT,'(A,3F20.12)') ' COSTH= ',COSTH1-COSTH,COSTH2-COSTH
      WRITE(IOUT,'(A,3F20.12)') ' GF/SF= ',(GF1+GF2)/SFAC1/SFAC2
      END
...
CCC      PRMOD = SQRT(PX1(4)**2-AMEL2)
         PRMOD =      PX1(4)**2-AMEL2
         IF(PRMOD.LE.0D0) WRITE(IOUT,*) ' REDUZ1: PRMOD=', PRMOD
...
C     ***********************************
      ...
      DIMENSION PP(4),QQK(4),PPK(4)
      DIMENSION PX1(4),PX2(4),PH1X(4),PH2X(4),SPH(4),ppx(4)
C
      DO 20 K=1,4
      PP(K)   = P1(K)+P2(K)
...
CCCC     PRMOD  =  SQRT(PX1(4)**2-AMEL2)
         PRMOD  =      PX1(4)**2-AMEL2
         IF(PRMOD.LE.0D0) WRITE(IOUT,*) ' REDUZ2: PRMOD=', PRMOD
...
C FOR KEYFIX=1, EXCEPTIONALLY, IT PROVIDES INTEGRAND AT VV=VVMAX
C WITH BORN OMITTED
C     ************************
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      COMMON / KEYYFS / KEYZET,KEYBRM,KEYFIX,KEYRED,KEYWGT
...
C ...Beta2 Second Order
      ...
      ELSE
         WRITE(IOUT,*) ' ===--->  WRONG KEYDIS IN UURHO'
         STOP
      ENDIF
      UURHO = DISTR
      END
...
CCCC     IF(NEVGEN.LE.5) CALL DUMPI(IOUT)
CCCC     IF(NEVGEN.LE.5) CALL DUMPF(IOUT)
C ...Model weight
...
cc    CALL GPRINT(IDYFS+11)
      ...
      CALL GPRINT(IDYFS+ 6)
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) 'EXPAND output - window A'
      WRITE(IOUT,BXL1F) CMSENE,     'CMS energy total   ','CMSENE','A0'
      WRITE(IOUT,BXL2F) XSMC,ERABS, 'xs_tot MC R-units  ','XSMC  ','A1'
      WRITE(IOUT,BXL1F) XSMCNB,     'xs_tot    nanob.   ','XSMCNB','A3'
      WRITE(IOUT,BXL1F) ERABS2,     'absolute error     ','ERABS2','A4'
      WRITE(IOUT,BXL1F) EREL,       'relative error     ','EREL  ','A5'
      WRITE(IOUT,BXL1I) NEVTOT,     'total no of events ','NEVTOT','A6'
      WRITE(IOUT,BXL1I) NEVACC,     'accepted    events ','NEVACC','A7'
      WRITE(IOUT,BXL1I) NEVNEG,     'WT<0        events ','NEVNEG','A8'
      WRITE(IOUT,BXL1I) NEVOVE,     'WT>WTMAX    events ','NEVOVE','A9'
      WRITE(IOUT,BXL1F) WTMAX ,     'WTMAX              ','WTMAX ','A10'
      WRITE(IOUT,BXL1F) XBORNB,     'xs_Born   nanob.   ','XBORNB','A11'
      WRITE(IOUT,BXCLO)
      IF ( (NEVNEG+NEVOVE) .GT. NEVACC/4000) THEN
        WRITE(IOUT,BXOPE)
        WRITE(IOUT,BXTXT) ' possible loss of precision !!!!!! '
        WRITE(IOUT,BXTXT) ' WTMAX too small ?                 '
        WRITE(IOUT,BXTXT) ' Increase it in SUBROURTINE MODEL! '
        WRITE(IOUT,BXTXT) ' MANY events of negative weights?  '
        WRITE(IOUT,BXTXT) ' Contact JADACH at CERNVM,         '
        WRITE(IOUT,BXTXT) ' But check folowing weights first  '
        WRITE(IOUT,BXCLO)
        WRITE(IOUT,BXOPE)
        WRITE(IOUT,BXTXT) ' Rejection weight of YFS3               '
        WRITE(IOUT,BXCLO)
         CALL GMONIT(2,IDYFS+79,DUMM1,DUMM2,DUMM3)
        WRITE(IOUT,BXOPE)
        WRITE(IOUT,BXTXT) ' contribution of over and underflows    '
        WRITE(IOUT,BXCLO)
         CALL GMONIT(2,IDYFS+70,DUMM1,DUMM2,DUMM3)
        WRITE(IOUT,BXOPE)
        WRITE(IOUT,BXTXT) ' ratio averwt(id=70)/averwt(id=79)      '
        WRITE(IOUT,BXTXT) ' measures severity of error             '
        WRITE(IOUT,BXCLO)
      ENDIF
C     ==================================================================
...
C     ==================================================================
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) '        EXPAND output - window B '
      WRITE(IOUT,BXTXT) '           X-sections in R-units '
      CALL GMONIT(1,IDYFS+73,DUMM1,DUMM2,DUMM3)
      XS03   =  XKARL*AVERWT
      DXS03  =  XKARL*AVERWT*SQRT(ERKARL**2+ERRELA**2)
      ...
      XS04   =  XKARL*AVERWT
      DXS04  =  XKARL*AVERWT*SQRT(ERKARL**2+ERRELA**2)
      WRITE(IOUT,BXL2F) XS03,DXS03,'X-section','O(alf2)',  'B1'
      WRITE(IOUT,BXL2F) XS02,DXS02,'X-section','O(alf1)',  'B2'
      WRITE(IOUT,BXL2F) XS01,DXS01,'X-section','O(alf0)',  'B3'
      IF(XS02.NE.0D0) WRITE(IOUT,BXL2F)
...
C
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) '    YFS VERSION 3.30   FEBR.  93        '
      WRITE(IOUT,BXL1F) CMSENE,     'CMS energy total   ','CMSENE','A0'
      WRITE(IOUT,BXL1I) KEYBRM,     'general QED switch ','KEYBRM','A1'
      WRITE(IOUT,BXL1F) AMFIN,      'final fermionmass  ','AMFIN ','A2'
      WRITE(IOUT,BXL1F) AMAZ,       'Z mass   [GeV]     ','AMAZ  ','A3'
      WRITE(IOUT,BXL1F) GAMMZ,      'Z width  [GeV]     ','GAMMZ ','A4'
      WRITE(IOUT,BXL1F) SINW2,      'sin(theta_W)**2    ','SINW2 ','A5'
      WRITE(IOUT,BXL1F) VVMIN,      'dummy infrared cut ','VVMIN ','A6'
      WRITE(IOUT,BXL1F) VVMAX,      'v_max ( =1 )       ','VVMAX ','A7'
      WRITE(IOUT,BXL1I) KEYRED,     'reduction  switch  ','KEYRED','A8'
      WRITE(IOUT,BXL1I) KEYWGT,     'weighting  switch  ','KEYWGT','A9'
      WRITE(IOUT,BXL1I) KEYZET,     'elect_weak switch  ','KEYZET','A10'
      WRITE(IOUT,BXCLO)
      END
...
C     ==================================================================
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) 'Initialize KARLUD  start'
      SVAR = 4D0*ENE**2
      ...
      WRITE(IOUT,BXL1F) XCVESK,     'xs_crude  VESKO    ','XCVESK','  '
      WRITE(IOUT,BXL1F) XCGAUS,     'xs_crude  GAUSS    ','XCGAUS','  '
      WRITE(IOUT,BXL1F) XDEL       ,'XCVESK/XCGAUS-1    ','      ','  '
      WRITE(IOUT,BXTXT) 'Initialize KARLUD  end  '
      WRITE(IOUT,BXCLO)
      ELSEIF(MODE.EQ.0) THEN
C     ==================================================================
C     ====================GENERATION====================================
...
C NO PRINTOUT FOR MODE = 2
      ...
      ERKR   = XSKR*ERKRL
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) '     KARLUD  FINAL  REPORT     '
      WRITE(IOUT,BXTXT) '         window A              '
      WRITE(IOUT,BXL1I) NEVTOT,     'total no of events ','NEVTOT','A0'
      WRITE(IOUT,BXL1I) NEVNEG,     'WT<0        events ','NEVNEG','A1'
      WRITE(IOUT,BXL1F) XCVESK,     'xs_cru VESKO  [R]  ','XCVESK','A2'
      WRITE(IOUT,BXL2F) XSVE,ERVE,  'xs_est VESKO  [R]  ','XSVE  ','A3'
      WRITE(IOUT,BXL2F) XSGS,ERGS,  'xs_est Gauss  [R]  ','XSGS  ','A4'
      WRITE(IOUT,BXL2F) DDV,DDR,    'XCVE/XCGS-1        ','      ','A5'
      WRITE(IOUT,BXL2F) WTKARL,ERKRL,'   <WT>           ','WTKARL','A6'
      WRITE(IOUT,BXL2F) XSKR,ERKR,  'sigma_prim    [R]  ','XSKARL','A7'
      WRITE(IOUT,BXCLO)
C     ==================================================================
C     =============SUPPL. FINAL WEIGHT ANALYSIS=========================
C     ==================================================================
      ...
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) '     KARLUD  FINAL  REPORT     '
      WRITE(IOUT,BXTXT) '         window B              '
      WRITE(IOUT,BXL2F) DEL1,DWT1,  '<WF1>-1  mass wt   ','DEL1  ','B1'
      WRITE(IOUT,BXL2F) AWF2,DWT2,  '<WF2> dilat. weight','AWF2  ','B2'
      WRITE(IOUT,BXL2F) AWF3,DWT3,  '<WF3> dilat. weight','AWF3  ','B3'
      WRITE(IOUT,BXL2F) DEL3,DWT3,  '<WF3>-YGF(BETI2)   ','DEL3  ','B4'
      WRITE(IOUT,BXL2F) AWF4,DWT4,  '<WF1*WF3>          ','AWF4  ','B5'
      WRITE(IOUT,BXL2F) DEL4,DWT4,  '<WF1*WF3>-YGF(BETI)','DEL4  ','B6'
      WRITE(IOUT,BXCLO)
C     ==================================================================
      ...
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) '     KARLUD  FINAL  REPORT CONT.   '
      WRITE(IOUT,BXTXT) '         WINDOW C                  '
      WRITE(IOUT,BXTXT) 'BETI= 2*ALFA/PI*(LOG(S/MEL**2)-1)       '
      WRITE(IOUT,BXTXT) 'GAMFAP= 1-PI**2*BETI**2/12              '
      WRITE(IOUT,BXTXT) 'GAMFAC=EXP(-CEULER*BETI)/GAMMA(1+BETI)  '
      WRITE(IOUT,BXTXT) 'GAMFA2=EXP(-CEULER*BETI2)/GAMMA(1+BETI2)'
      WRITE(IOUT,BXL1F)  BETI,        '                =','BETI  ','C1'
      WRITE(IOUT,BXL1F)  GAMFAP,      '                =','GAMFAP','C2'
      WRITE(IOUT,BXL1F)  GAMFAC,      '                =','GAMFAC','C3'
      WRITE(IOUT,BXL1F)  GAMFA2,      '                =','GAMFA2','C4'
      WRITE(IOUT,BXL2F) AWF5,DWT5, ' <WF1*WF3*WF4>      ','AWF5  ','C5'
      WRITE(IOUT,BXL2F) DEL5,DWT5, ' <WF1*WF3>-YGF(BETI)','DEL5  ','C6'
      WRITE(IOUT,BXTXT) 'DELKAR=XREFER*AVER(WF1*WF1*WF3)/XSKARL-1'
      WRITE(IOUT,BXTXT) 'DELREF=XCRUDE*AVER(WTR)/XREFER-1        '
      WRITE(IOUT,BXL1F) XREFER,    'reference x_sect.   ','XREFER','C7'
      WRITE(IOUT,BXL1F) DELKAR,    'XREFER*AWF5/XSKR  -1','DELKAR','C8'
      WRITE(IOUT,BXL1F) DELREF,    'XCVESK*AWF6/XREFER-1','DELREF','C9'
      WRITE(IOUT,BXCLO)
      ENDIF
C     =====
      END
...
C NPHOX = POISSON(WITH AVERAGE = AVERG) + 1
...
C This is for tests of program at fixed multiplicity (for adv. users)
         ...
         IF(NPHFIX.NE.0.AND.NPHOX.NE.NPHFIX) GOTO 100
         IF(NPHOX.EQ.1) THEN
            XPH(1)=VV
...
C SCALE DOWN PHOTON ENERGIES AND MOMENTA
            DO 300 I=1,NPHOX
            DO 300 K=1,4
  300       XPHOT(I,K)=XPHOT(I,K)/EXPY
C CHECK ON LOWER ENERGY CUT-OFF
            IF(XPHOT(NPHOX,4).LT.VMIN) WT3 =0D0
         ENDIF
      ENDIF
C PHOTON MOMENTA IN GEV UNITS
  ...
  420 XPHUM(J)=0D0
      DO 480 I=1,NPHOX
      DO 480 J=1,4
      XPHOT(I,J)=XPHOT(I,J)*ENE
  480 XPHUM(J)=XPHUM(J)+XPHOT(I,J)
...
C OUTPUT : XPHOT  = LIST OF PHPTON FOUR-MOMENTA
...
C GENERATE MOMENTA OF FERMIONS IN THEIR REST FRAME WITH FLAT DISTR.
      XF1(4)=QSM/2D0
      XF2(4)=QSM/2D0
      QMD=SQRT(XF1(4)**2-AMFIN**2)
      CALL VARRAN(DRVEC,2)
      R1 = DRVEC(1)
      R2 = DRVEC(2)
      COSTH=-1D0+2D0*R1
      SINTH=SQRT(1D0-COSTH**2)
      XF1(1)=QMD*SINTH*COS(2D0*PI*R2)
      XF1(2)=QMD*SINTH*SIN(2D0*PI*R2)
      XF1(3)=QMD*COSTH
      DO 30 K=1,3
   30 XF2(K)=-XF1(K)
C TRANSFORM BACK TO CMS
      IF(NPHOX.GE.1) THEN
        CALL BOSTDQ(-1,QSU,XF1,XF1)
        CALL BOSTDQ(-1,QSU,XF2,XF2)
      ENDIF
C======================================================================
C==================END OF YFSGEN=======================================
C======================================================================
...
C     ===================
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) 'Initialize KARFIN  start'
      IDYFS = 0
...
C ...Additional check on WTCTRL, plot of its u-dependence
      ...
      NEVGEN=0
      MARTOT=0
      WRITE(IOUT,BXTXT) 'Initialize KARFIN  end  '
      WRITE(IOUT,BXCLO)
      ELSEIF(MODE.EQ.0) THEN
C     ======================
      NEVGEN=NEVGEN+1
     ...
     $           UU,NPHOY,YPHOT,YPHUM,MK,YF1,YF2,WT1,WT2,WTMAS)
      CALL KINF1(XXF,YF1,YF2,NPHOY,YPHOT,YPHUM)
      WT3=1D0
      WCTRL=1D0
      WTREM=1D0
      IF(WT1.NE.0.D0) THEN
...
C ...General information on weights
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) 'KARFIN output - window A'
      WRITE(IOUT,BXTXT) '    Weight Statistics   '
      WRITE(IOUT,BXL2F) AWT69,DWT69,' general weight    ','WT    ','A1'
      WRITE(IOUT,BXL1I) NEVGEN,     ' generated events  ','NEVGEN','A2'
      WRITE(IOUT,BXL1F) AVMULT,     ' aver. ph. multi.  ','AVMULT','A3'
      WRITE(IOUT,BXL1I) MARTOT,     ' Marked photons    ','MARTOT','A4'
      WRITE(IOUT,BXL2F) AWT61,DWT61,' Kinematics, smin  ','WT1   ','A5'
      WRITE(IOUT,BXL2F) AWT62,DWT62,' Jacobian          ','WT2   ','A6'
      WRITE(IOUT,BXL2F) AWT63,DWT63,' Photon ang. dist. ','WT3   ','A7'
      WRITE(IOUT,BXCLO)
C ...Specific details on mass weight rearrangenment, and rejection
      ...
      NTOT66 = NEVTOT
      WRITE(IOUT,BXOPE)
      WRITE(IOUT,BXTXT) 'KARFIN output - window B'
      WRITE(IOUT,BXTXT) '    on mass weights     '
      WRITE(IOUT,BXL2F) AWT66,DWT66,' removal wgt WTREM ','WT6 ','B1'
      WRITE(IOUT,BXL1I) NTOT66,     ' no. of raw events ','    ','B2'
      WRITE(IOUT,BXL1I) NZER66,     ' WT6=0      events ','    ','B3'
      WRITE(IOUT,BXL1F) AVMLT,      ' raw ph. multipl.  ','    ','B4'
      WRITE(IOUT,BXL2F) AWT65,DWT65,' control wgt WCTRL ','WT5 ','B5'
      WRITE(IOUT,BXL1G) EPS,        ' epsilon           ','    ','B6'
      WRITE(IOUT,BXL1G) DELTA,      ' delta             ','    ','B7'
      WRITE(IOUT,BXCLO)
C ...Histograms
...
C ...All sort of weights
      ALF1  =  1/PI/ALFINV
      QQK  = QQ(4)*YPHUM(4)-QQ(3)*YPHUM(3)-QQ(2)*YPHUM(2)-QQ(1)*YPHUM(1)
C ...delt1 and eps1 are cutoffs which sit in YFS formfactor
      DELT1 =  DELTA*(1+ 2*QQK/SPRIM)
      EPS1  =  SQRT(EMIN**2/YF1(4)/YF2(4))
...
C ...Control weight - its average should be precisely one
      ...
      IF(KEYPIA.EQ.0) THEN
         IF(ABS(DELB).GT.100D0 ) WRITE(IOUT,*) ' DELB= ',DELB
...
C ...in such a case WTM3A includes exp(BELB2)= <WT3> for removed ph.
         NPH=NPHOY
         DO 100 J=NPHOY,1,-1
         IF(YPHOT(J,4).LT.EMIN) THEN
            DO 60 I=J+1,NPH
            DO 60 K=1,4
   60       YPHOT(I-1,K)=YPHOT(I,K)
            NPH=NPH-1
         ENDIF
  100    CONTINUE
         NPHOY=NPH
...
C ...Momenta q1,q2 should be in CMS
     ...
     $                 UU,NPHOY,PHOT,PHSU,MK,Q1,Q2,WT1,WT2,WTM)
C     *****************************************************************
...
C GENERATE PHOTON MULTIPLICITY, AVERG = AVERAGE MULTIPLICITY (CRUDE)
      ...
      NMAX=50
    5 CONTINUE
      CALL POISSG(AVERG,NMAX,NPHOY,RR)
C This is for tests of program at fixed multiplicity (for adv. users)
      NPHFIX =  MOD(KEYBRM,100000)/10000
      IF(NPHFIX.NE.0.AND.NPHOY.NE.NPHFIX) GOTO 5
      IF(NPHOY.EQ.0) THEN
          SPRIM=SVAR
      ELSE
...
C ...care is taken of machine rounding errors
     ...
     $            -AMF/(1.D0+BTF )/2.D0*(1.D0/DEL1**2+1.D0/DEL2**2)
          IF(DIST1.LT.0.D0) THEN
              DIST1=0D0
              WRITE(IOUT,*) ' +++++++ MBREF: NEGATIVE WTMAS =',DIST1
...
C     ************************
  ...
  130 MULT=NN-1
      RETURN
  900 WRITE(IOUT,*) ' POISSG: TO SMALL NMAX'
      STOP
      END
...
C     **********************
      ...
      DIMENSION SUM(4)
      WRITE(IOUT,*) '=====================DUMPS===================='
      WRITE(IOUT,3100) 'QF1',(  QF1(  K),K=1,4)
      WRITE(IOUT,3100) 'QF2',(  QF2(  K),K=1,4)
      DO 100 I=1,NPHOT
  100 WRITE(IOUT,3100) 'PHO',(SPHOT(I,K),K=1,4)
      DO 200 K=1,4
  200 SUM(K)=QF1(K)+QF2(K)
      DO 210 I=1,NPHOT
      DO 210 K=1,4
  210 SUM(K)=SUM(K)+SPHOT(I,K)
      WRITE(IOUT,3100) 'SUM',(  SUM(  K),K=1,4)
 3100 FORMAT(1X,A3,1X,5F18.14)
      END

      SUBROUTINE DUMPF(IOUT)
C     **********************
C THIS PRINTS OUT FOUR MOMENTA OF PHOTONS
C ON UNIT NO. IOUT
C     **********************
      ...
      DIMENSION SUM(4)
      WRITE(IOUT,*) '=====================DUMPF===================='
      WRITE(IOUT,3100) 'QF1',(  YF1(  K),K=1,4)
      WRITE(IOUT,3100) 'QF2',(  YF2(  K),K=1,4)
      DO 100 I=1,NPHOY
  100 WRITE(IOUT,3100) 'PHO',(YPHOT(I,K),K=1,4)
      DO 200 K=1,4
  200 SUM(K)=YF1(K)+YF2(K)
      DO 210 I=1,NPHOY
      DO 210 K=1,4
  210 SUM(K)=SUM(K)+YPHOT(I,K)
      WRITE(IOUT,3100) 'SUM',(  SUM(  K),K=1,4)
 3100 FORMAT(1X,A3,1X,5F18.14)
      END

      SUBROUTINE DUMPI(IOUT)
C     **********************
C THIS PRINTS OUT FOUR MOMENTA OF PHOTONS
C ON UNIT NO. IOUT
C     **********************
      ...
      DIMENSION SUM(4)
      WRITE(IOUT,*) '=====================DUMPI===================='
      WRITE(IOUT,3100) 'QF1',(  XF1(  K),K=1,4)
      WRITE(IOUT,3100) 'QF2',(  XF2(  K),K=1,4)
      DO 100 I=1,NPHOX
  100 WRITE(IOUT,3100) 'PHO',(XPHOT(I,K),K=1,4)
      DO 200 K=1,4
  200 SUM(K)=XF1(K)+XF2(K)
      DO 210 I=1,NPHOX
      DO 210 K=1,4
  210 SUM(K)=SUM(K)+XPHOT(I,K)
      WRITE(IOUT,3100) 'SUM',(  SUM(  K),K=1,4)
 3100 FORMAT(1X,A3,1X,5F18.14)
      END

      SUBROUTINE DUMPN(IOUT,IEV)
C     **************************
C THIS PRINTS OUT FOUR MOMENTA OF final state
C and the serial number of event IEV on unit IOUT
C     **********************
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      COMMON / MOMINI / XF1(4),XF2(4),XPHUM(4),XPHOT(100,4),NPHOX
      SAVE   / MOMINI /
      DIMENSION SUM(4)
      WRITE(IOUT,*) '=============DUMPNEW====================>',IEV
      WRITE(IOUT,3100) 'QF1',(  XF1(  K),K=1,4)
      WRITE(IOUT,3100) 'QF2',(  XF2(  K),K=1,4)
      DO 100 I=1,NPHOX
  100 WRITE(IOUT,3100) 'PHO',(XPHOT(I,K),K=1,4)
...
ccc      write(6,*) '3B:x,x1=',x,xprim
      ...
      DJAC    =  (Q1+Q2)/DIST
      RETURN
  900 WRITE(IOUT,*) ' ++++ STOP IN CHBIN3: wrong parameters'
...
C
      ...
      DJAC=(Q1+Q2)/DIST
      RETURN
  900 WRITE(IOUT,*) ' ========= STOP IN CHBIN1: WRONG PARAMS'
      STOP
      END
...
C ------------------------------------------------------------
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      save
      PARAMETER( JLIM1 = 64, JLIM2 = 1000 , IOUT = 6 )
...
C precision check to terminate integration
      ...
      ENDIF
  400 NDIVI=NDIVI*2
      WRITE(IOUT,*) ' +++++ GAUSJD:  REQUIRED PRECISION TO HIGH!'
      WRITE(IOUT,*) ' +++++ GAUSJD:  ITER,ERELA=',ITER,ERELA
  800 RESULT= CALK16
      END
...
C          FOR MODE=2 IS JUST PRINTED OF UNIT IOUT
C ENDIF
...
C
      ...
      ELSEIF(MODE.EQ.0) THEN
           IF(NTOT(ID).LT.0) THEN
              WRITE(IOUT,*) ' ==== WARNING FROM WMONIT: '
              WRITE(IOUT,*) ' LACK OF INITIALIZATION, ID=',ID
           ENDIF
           NTOT(ID)=NTOT(ID)+1
           SWT(ID)=SWT(ID)+WT
      ...
      ELSEIF(MODE.EQ.2) THEN
           ...
           ELSE
              ...
              WWMAX=WWMX(ID)
           ENDIF
           WRITE(IOUT,1003) ID, AVERWT, ERRELA, WWMAX
           WRITE(IOUT,1004) NTOT(ID),NACC(ID),NNEG(ID),NOVE(ID),NZER(ID)
           WT=AVERWT
           WTMAX=ERRELA
           RN    =WWMX(ID)
      ELSE
           WRITE(IOUT,*) ' =====WMONIT: WRONG MODE',MODE
           STOP
      ENDIF
 1003 FORMAT(
     $  ' =======================WMONIT========================'
...
C
      ...
      ELSEIF(MODE.EQ.1) THEN
           IF(NTOT(ID).LT.0) THEN
              WRITE(IOUT,*) ' ==== WARNING FROM WMONI2: '
              WRITE(IOUT,*) ' LACK OF INITIALIZATION, ID=',ID
           ENDIF
           IF(NTOT(ID).LE.0.OR.SWT(ID).EQ.0D0)  THEN
              AVERWT=0D0
      ...
      ELSEIF(MODE.EQ.2) THEN
           ...
           RN    =WWMX(ID)
      ELSE
           WRITE(IOUT,*) ' =====WMONI2: WRONG MODE',MODE
           STOP
      ENDIF
 1003 FORMAT(
     $  ' =======================WMONI2========================'
...
C=======================================================================
C==
C==      PROGRAM PSEUDORAN      Wed, 23 Jan 91   F. James
...
C          always come here to initialize
    1 CONTINUE
      ...
      L = MOD(KL, 169)
      WRITE(6,'(A,5I10)')
     $' MARran INITIALIZED: IJ,KL,IJKL,NTOT,NTOT2=',IJ,KL,IJKL,NTOT,NTOT
      DO 2 II= 1, 97
      S = 0.
      T = .5
      DO 3 JJ= 1, 24
...
C Q MUST BE A TIMELIKE, P MAY BE ARBITRARY.
   ...
   10 Q(K)=QQ(K)
      ...
      ELSEIF(MODE.EQ. 1) THEN
         ...
         FAC  =-(R(4)+P(4))/(Q(4)+AMQ)
      ELSE
         WRITE(IOUT,*) ' ++++++++ WRONG MODE IN BOOST3 '
         STOP
      ENDIF
      R(1)=P(1)+FAC*Q(1)
      R(2)=P(2)+FAC*Q(2)
...
cam   rho' taken as in Dolinsky et al (PhysLett B174 (1986) 453)
...
C     QUASI TWO-BODY VARIABLES
      ...
      AL=AK0/TMASS
C     FORMFACTORS
      F0=2*U0/U3*(  DILOGY(1-(UM*WM/(UP*WP)))- DILOGY(1-WM/WP) +
     $DILOGY(1-UM/UP) -2*YU+ 2*LOG(UP)*(YW+YU) ) +
