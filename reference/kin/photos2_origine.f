C.----------------------------------------------------------------------
CBB   Modified B.Bloch Nov 98 : HEPEVT common , NMXHEP=4000, REAL*8 PHEP,VHEP
CBB                             PHORAN : RNDM(DUM) fo rALEPH
CBB                             PHORIN : CALL RDMIN(ISEED) for ALEPH
CBB                             PHOCIN : remove init of seeds for PHORAN
C.
...
C--   IMPLICIT NONE
      INTEGER NMXHEP
      PARAMETER (NMXHEP=4000)        ! bb for pythia 6.1
      LOGICAL QEDRAD
...
CBB      INTEGER ISEED,I97,J97
CBB      REAL URAN,CRAN,CDRAN,CMRAN
CBB      COMMON/PHSEED/ISEED(2),I97,J97,URAN(97),CRAN,CDRAN,CMRAN
      INTEGER PHOMES
      PARAMETER (PHOMES=10)
      INTEGER STATUS
...
C      REAL S,T
C      INTEGER I,IS1,IS2,IS3,IS4,IS5,J
C      INTEGER ISEED,I97,J97
C      REAL URAN,CRAN,CDRAN,CMRAN
C      COMMON/PHSEED/ISEED(2),I97,J97,URAN(97),CRAN,CDRAN,CMRAN
C--
C--   Check value range of seeds
C      IF ((ISEED(1).LT.0).OR.(ISEED(1).GE.31328)) THEN
C        DATA=ISEED(1)
C        CALL PHOERR(8,'PHORIN',DATA)
C      ENDIF
C      IF ((ISEED(2).LT.0).OR.(ISEED(2).GE.30081)) THEN
C        DATA=ISEED(2)
C        CALL PHOERR(9,'PHORIN',DATA)
C      ENDIF
C--
C--   Calculate Marsaglia and Zaman seeds (by F. James)
C      IS1=MOD(ISEED(1)/177,177)+2
C      IS2=MOD(ISEED(1),177)+2
C      IS3=MOD(ISEED(2)/169,178)+1
C      IS4=MOD(ISEED(2),169)
C      DO 20 I=1,97
C        S=0.
C        T=0.5
C        DO 10 J=1,24
C          IS5=MOD (MOD(IS1*IS2,179)*IS3,179)
C          IS1=IS2
C          IS2=IS3
C          IS3=IS5
C          IS4=MOD(53*IS4+1,169)
C          IF (MOD(IS4*IS5,64).GE.32) S=S+T
C   10   T=0.5*T
C   20 URAN(I)=S
C      CRAN=362436./16777216.
C      CDRAN=7654321./16777216.
C      CMRAN=16777213./16777216.
C      I97=97
C      J97=33
       CALL RDMIN(ISEED)
      RETURN
...
C      REAL URAN,CRAN,CDRAN,CMRAN
C      COMMON/PHSEED/ISEED(2),I97,J97,URAN(97),CRAN,CDRAN,CMRAN
C   10 PHORAN=URAN(I97)-URAN(J97)
C      IF (PHORAN.LT.0.) PHORAN=PHORAN+1.
C      URAN(I97)=PHORAN
C      I97=I97-1
...
C      CRAN=CRAN-CDRAN
C      IF (CRAN.LT.0.) CRAN=CRAN+CMRAN
C      PHORAN=PHORAN-CRAN
C      IF (PHORAN.LT.0.) PHORAN=PHORAN+1.
C      IF (PHORAN.LE.0.) GOTO 10
      PHORAN = RNDM(DUMMY)
      RETURN
      END
