...
*///////////////////////////////////////////////////////////////////////
C---------------------------------------------------
C    Modifications : b. Bloch december 99
C  1-  rename PHORAN and PHORIN to O_PHORAN and O_PHORIN, create new
C      PHORAN and PHORIN interfaced to ALEPH random number machinery
C  2-  PHCORK : write statements use " instead of ' in the formats
C  3-  add PHOVERS function to get the photos current version
C.----------------------------------------------------------------------
C.----------------------------------------------------------------------
C.
...
* jvw 20191002 this one line bug fix comes from version 2.09 and should
...
C.----------------------------------------------------------------------
      ...
      SUBROUTINE PHORIN
C---------------------------------
C   b.Bloch   use Aleph ranmar interface through RNDM
C--------------------------------------
      INTEGER ISEED
      INTEGER PHLUN
      COMMON/PHOLUN/PHLUN
      call rdmin(iseed)
      WRITE(PHLUN,1020)
 1020 FORMAT(1H ,'*',18X,'PHOTOS random is initialized within ALEPH')
      RETURN
      END
      SUBROUTINE O_PHORIN
C.----------------------------------------------------------------------
...
C--------------------------------------
      ...
      REAL*4 RNDM
      INTEGER IDUM
      phoran = dble(rndm(idum))
      RETURN
      END
      FUNCTION O_PHORAN(IDUM)
C.----------------------------------------------------------------------
C.
