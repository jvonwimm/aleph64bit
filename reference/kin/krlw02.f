...
C add CC03 info when running 4f    B.Bloch   March 2001
C--------------------------------------------------------------------
...
C
     ...
     &               cmax12, cmax13, cmax14, cmax23, cmax24, cmax34 !cav
      ...
      DIMENSION NPAR(100)
      REAL*8 sum40,sum41,sum42,sum1     ! to keep sum of weights
      COMMON/sumwt/sum40,sum41,sum42,sum1
C
...
C
     ...
     +100,0.,1.,0.)
        if ( Key4f.gt.0)call hbook1(800,
     +'      CC03/4f      (Wt.LT.WtMAX) - All flavours',
     +100,0.,1.,0.) 
        call hbook1(901,
...
C    mod B.Bloch   March 2001 : add CC03 and 4f weights for 4f events
C--------------------------------------------------------------------
C     input     : none
C
...
C
     ...
     &                KFDP(L2PARF,5)
      ...
      REAL*8 sum40,sum41,sum42,sum1,ratio
      COMMON/sumwt/sum40,sum41,sum42,sum1
...
C
     ...
     $        +1000000*abs(IFLAV(4))
      ...
      Key4f  = MOD(KeyMis,100)/10
      IF (Key4f .gt.0)  then 
         ratio = wtset(41)/wtset(42)
         sum1  = sum1  + 1.d0
         sum40 = sum40 + wtset(41)/wtset(42)
         sum41 = sum41 + wtset(41)     ! CC03 weight
         sum42 = sum42 + wtset(42)     ! 4f   weight
      endif 
...
C   store Coulomb weights in KWGT
      ...
      if (Key4f .gt.0)  then 
         weik = wtset(41)    ! CC03
         ind = kwgtbk(6,50,weik)
         weik = wtset(42)    ! 4f
         ind = kwgtbk(7,51,weik)
      endif
...
C   add info for reconnected evnts in KWGT
        ...
        JKWGT = NLINK('KWGT',0)
        nrow = 0
        if (JKWGT.gt.0) nrow = iw(jKWGT+LMHLEN+2) 
        ind = kwgtbk(nrow+1,2000,weik)
      ENDIF
C
...
C
     ...
     &                 (p3(3)+p4(3))**2   )), 0., 1.)
          ...
          If (key4f.gt.0) call hfill(800,real(ratio),0.,1.)
...
C     B.Bloch Mrch 2001  add CC03 x-section info when in 4f run
C --------------------------------------------------------------------
      PARAMETER ( IGCO = 5035 )
      PARAMETER ( IVER = 104  )
      COMMON / KGCOMM / ISTA,VRTEX(4),SDVRT(3),NEVENT(8),ECMI
...
!  4fermion introduced
        ...
        errl4f = dumm2
        print * , ' ================================================'
        print * , '  various x-sections within cuts                 '
        print * , ' ================================================'
        print * , ' 4f x-section    :',Xtot4f,' +- ', 
     $            Xtot4f*errl4f ,' pb'
        print * , ' CC x-section    :',XtotCC,' +- ', 
     $            XtotCC*errlCC ,' pb'
        print * , ' CC from weights :',Xtot4f*sum40/sum1,' +- ',
     $            Xtot4f*errl4f*sum40/sum1 ,' pb'  
        print * , ' ================================================' 
      ENDIF
...
C Store cross-section in KSEC bank
      ...
      IF( Key4f .NE. 0 ) THEN
        ...
        IVERS = 51
        XTOT = Xtot4f/1000.
        RTOT = Xtot4f*errl4f/1000.
        XACC = XTOT
        RACC = RTOT 
        isec = KSECBK(IS,IDC,IVERS,NTOT,NACC,XTOT,RTOT,XACC,RACC)
        IS = 4
        IVERS = 150
        XTOT = Xtot4f*sum40/1000./sum1 
        RTOT = XTOT*errlCC
        XACC = XTOT
        RACC = RTOT 
        isec = KSECBK(IS,IDC,IVERS,NTOT,NACC,XTOT,RTOT,XACC,RACC)
