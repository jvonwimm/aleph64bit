C--------------------------------------------------
C    Aleph modifications - Franck Martin/B.Bloch December 99
C  1- tabini.f routine to create tables in tabini.h
C  2- dizet : mw and  alphas(mt2) not defined amw2,calxi ???
C  3- zwrate : partz and partw need to be set to 0.
C  4- MY_DZface_Tabluj  has an extra input to set table index
C  5- DIZET_TO_KK new routine to transfer Table to KK2f
C  6- common /bos/ changed to /boson/
C  7- all include files have been incorporated to the code 
C  8- create a function DIZETVER(dum) to return the version number
C------------------------------------------------------
...
*-----------------------------------------------------------------------
      ...
      CDAL5H=DAL5H
cbardin
cfranck in ALQED calculation ALST = ALPHAS(AMT**2) et AMW2 are taken into ac-
cfranck count in top contribution, very small, but leads to different results
cfranck if dizet used in a loop. (ALST = CALXI)
      CALXI = 0.113189045497835D0     ! FM init
      AMW2 = 6460.60198684077D0       ! FM init
CF      print *,'control input dalpha',IALEM,IALE2,IHVP,DAL5H,-AMZ**2
        DALFA=AL4PI*DREAL(XFOTF3(IALEM,IALE2,IHVP,1,1,DAL5H,-AMZ**2))
        ALQED=1D0/ALFAI/(1D0-DALFA)
      CALQED=ALQED
CF      print *,'dizet inside 1',alqed,dalfa 
*-----------------------------------------------------------------------
      SW2=.2325D0
      ALPHST=ALFAS
...
*-----------------------------------------------------------------------
      ...
      SUBROUTINE DIZET_TO_KK(amz, amh, amtop)
C      INCLUDE 'tabini.h'
      DOUBLE COMPLEX m_cyy_p, m_czz_p, m_ctt_p, m_clc_p  ! Electroweak FFactors
      DOUBLE PRECISION   m_syy_p,m_szz_p,m_stt_p,m_slc_p   ! QCD corr.
      DOUBLE COMPLEX     m_GSW_p

      DOUBLE PRECISION   header

      INTEGER           m_poin1_p, m_poin2_p, m_poin3_p, m_poin4_p, m_poinT_p

* 340-point grid, only 80pt for NLC to be improved/tested in future
      PARAMETER( m_poin1_p = 120 ) ! wide energy range  (m_WminLEP1,m_WmaxLEP1)
      PARAMETER( m_poin2_p =  20 ) ! near z narrow range (amz +- 2*gammz)
      PARAMETER( m_poin3_p = 120 ) ! high energy range  (m_WmaxLEP1,m_WmaxLEP2)
      PARAMETER( m_poin4_p =  80 ) ! NLC energy range   (m_WmaxLEP2,m_WmaxNLC)
      PARAMETER( m_poinT_p =  14 ) ! cost(heta) grid

      COMMON /diztokk/
* Tables of EW formfactors
     $  m_cyy_p(m_poin1_p+1,7,16),           ! formfactor, table
     $  m_czz_p(m_poin2_p+1,7,16),           ! formfactor, table
     $  m_ctt_p(m_poin3_p+1,m_poinT_p+1,7,16), ! formfactor, table
     $  m_clc_p(m_poin4_p+1,m_poinT_p+1,7,16), ! formfactor, table
     $  m_syy_p(m_poin1_p+1,16),             ! QCD correction, table
     $  m_szz_p(m_poin2_p+1,16),             ! QCD correction, table
     $  m_stt_p(m_poin3_p+1,m_poinT_p+1,16),   ! QCD correction, table
     $  m_slc_p(m_poin3_p+1,m_poinT_p+1,16),   ! QCD correction, table
     $  m_GSW_p(100),             ! form-factors,   at the actual energy/angle
     $  header(7)
      SAVE /diztokk/
...
*//////////////////////////////////////////////////////////////////////////////
      ...
      DO i=0,m_poin1
       DO ii = 1,16
        DO kk=1,7
         m_cyy(i+1,kk,ii) = m_cyy_p(i+1,kk,ii)
        ENDDO
         m_syy(i+1,ii) = m_syy_p(i+1,ii)
       ENDDO
      ENDDO
* make strange and charm tables...
      DO i=0,m_poin1
        DO kk=1,7
         m_cyy(i+1,kk,3) = m_cyy_p(i+1,kk,1)   ! STRANGE = DOWN
         m_cyy(i+1,kk,4) = m_cyy_p(i+1,kk,2)   ! CHARM   = UP
        ENDDO
         m_syy(i+1,3) = m_syy_p(i+1,1)        
         m_syy(i+1,4) = m_syy_p(i+1,2)
      ENDDO
   
      DO i=0,m_poin2
       DO ii = 1,16
        DO kk=1,7
         m_czz(i+1,kk,ii) = m_czz_p(i+1,kk,ii)
        ENDDO
         m_szz(i+1,ii) = m_szz_p(i+1,ii)
       ENDDO
      ENDDO
* make strange and charm tables...
      DO i=0,m_poin2
        DO kk=1,7
         m_czz(i+1,kk,3) = m_czz_p(i+1,kk,1)
         m_czz(i+1,kk,4) = m_czz_p(i+1,kk,2)
        ENDDO
         m_szz(i+1,3) = m_szz_p(i+1,1)
         m_szz(i+1,4) = m_szz_p(i+1,2)
      ENDDO
      ...
      DO  i=0,m_poin3
       DO  j=0,m_poinT  
        DO ii = 1,16
         DO kk=1,7
          m_ctt(i+1,j+1,kk,ii) = m_ctt_p(i+1,j+1,kk,ii)
         ENDDO
         m_stt(i+1,j+1,ii) = m_stt_p(i+1,j+1,ii)
        ENDDO
       ENDDO
      ENDDO  
* make strange and charm tables...
      DO  i=0,m_poin3
       DO  j=0,m_poinT  
         DO kk=1,7
          m_ctt(i+1,j+1,kk,3) = m_ctt_p(i+1,j+1,kk,1)
          m_ctt(i+1,j+1,kk,4) = m_ctt_p(i+1,j+1,kk,2)
         ENDDO
         m_stt(i+1,j+1,3) = m_stt_p(i+1,j+1,1)
         m_stt(i+1,j+1,4) = m_stt_p(i+1,j+1,2)
        ENDDO
       ENDDO
      ...
      DO  i=0,m_poin4
       DO  j=0,m_poinT  
        DO ii = 1,16
         DO kk=1,7
          m_clc(i+1,j+1,kk,ii) = m_clc_p(i+1,j+1,kk,ii)
         ENDDO
         m_slc(i+1,j+1,ii) = m_slc_p(i+1,j+1,ii)
        ENDDO
       ENDDO
      ENDDO
* make strange and charm tables...
      DO  i=0,m_poin4
       DO  j=0,m_poinT  
         DO kk=1,7
          m_clc(i+1,j+1,kk,3) = m_clc_p(i+1,j+1,kk,1)
          m_clc(i+1,j+1,kk,4) = m_clc_p(i+1,j+1,kk,2)
         ENDDO
         m_slc(i+1,j+1,3) = m_slc_p(i+1,j+1,1)
         m_slc(i+1,j+1,4) = m_slc_p(i+1,j+1,2)
        ENDDO
       ENDDO
      ...
      WRITE(iout,*)'** Tabulation code using DIZET is finished ***********'
      WRITE(iout,*)'M_Z = ',m_MZ
      WRITE(iout,*)'M_top = ',m_amtop
      WRITE(iout,*)'M_Higgs = ',m_amh
      WRITE(iout,*)'** Parameters used in KK2f  ***********'
      RETURN
      END
...
*//  but produces look-pu tables on the disk which are read by BornV class.             //
*//  The tables are produced using TabMain program which is run separately for          //
*//  each flavor, producing one table per flavour.                                      //
*//  This is necessary, as there is no guarantee that EWK library can be reinitialized  //
*//  properly (although Dizet apparently can be).                                       //
*//  There is special testing program TabCheck in BornV/tabtest/ which tests            //
*//  whether interpolation is done correctly.                                           //
*//  Present version is of for sub-permille precision.                                  //
...
*//                                                                                     //
*/////////////////////////////////////////////////////////////////////////////////////////
      SUBROUTINE DZface_Initialize( KFini,  KFfin,   amz, amh,  amtop, ibox, iout)
*/////////////////////////////////////////////////////////////////////////////////////////
*//                                                                                     //
*//     Class initializator                                                             //
...
*//                     Pseudo-CLASS  DZface                                 //
*//                                                                          //
*//   Purpose: Calculate d_sigma/d_Omega(s,theta) from tables on disk        //
*//                                                                          //
*//                                                                          //
*//                                                                          //
*//                                                                          //
...
*//////////////////////////////////////////////////////////////////////////////
      ...
      INTEGER           m_poin1, m_poin2, m_poin3, m_poin4, m_poinT
*//////////////////////////////////////////////////////////////////////////////
* TEST grid, for fast tests only
*      PARAMETER( m_poin1 =  20 ) ! LEP1 range        (m_WminLEP1,m_WmaxLEP1)
*      PARAMETER( m_poin2 =  10 ) ! near Z pole range (amz +- 2*gammz)
*      PARAMETER( m_poin3 =  10 ) ! LEP2 energy range (m_WmaxLEP1,m_WmaxLEP2)
*      PARAMETER( m_poin4 =   5 ) ! NLC energy range  (m_WmaxLEP2,m_WmaxNLC)
*      PARAMETER( m_poinT =   2 ) ! cost(heta) grid
*//////////////////////////////////////////////////////////////////////////////
* 340-point grid, only 80pt for NLC to be improved/tested in future
      ...
      PARAMETER( m_poinT =  14 ) ! cost(heta) grid
*//////////////////////////////////////////////////////////////////////////////
* EW formfactors, single flavour
      DOUBLE COMPLEX     m_cyy
      DOUBLE COMPLEX     m_czz
      DOUBLE COMPLEX     m_ctt
      DOUBLE COMPLEX     m_clc
      DOUBLE COMPLEX     m_gsw
      DOUBLE PRECISION   m_QCDcor
*//////////////////////////////////////////////////////////////////////////////
      INTEGER m_KFini,   m_KFfin
      INTEGER m_IVini,   m_IVfin
      DOUBLE PRECISION   m_amz,    m_amh, m_amt
      DOUBLE PRECISION   m_swsq,   m_gammz, m_amw,  m_gammw
      DOUBLE PRECISION   m_alfinvMZ, m_alfQCDMZ
      INTEGER m_ibox,    m_out

      COMMON /c_DZface/
     $  m_cyy(m_poin1+1, 7),            ! formfactor, table
     $  m_czz(m_poin2+1, 7),            ! formfactor, table
     $  m_ctt(m_poin3+1, m_poinT+1, 7), ! formfactor, table
     $  m_clc(m_poin4+1, m_poinT+1, 7), ! formfactor, table
     $  m_GSW(100),                     ! formfactors at actual energy
     $  m_KFini,                        ! KF code of initial fermion
     $  m_KFfin,                        ! KF code of final fermion
     $  m_IVini,                        ! vertical id of initial ferm.
     $  m_IVfin,                        ! vertical id of final ferm.
     $  m_amz,                          ! Z mass
     $  m_amh,                          ! Higgs mass
     $  m_amt,                          ! Top mass
     $  m_swsq,                         ! sin(thetaW)**2
     ...
     $  m_amw,                          ! W mass
     $  m_gammw,                        ! W width
     $  m_alfinvMZ,                     ! 1/alphaQED at Z mass
     $  m_alfQCDMZ,                     ! alphaQCD   at Z mass ?????smiec!!!!
     $  m_QCDcor,                       ! QCD correction !!! to be improved!!!
     $  m_ibox,                         ! EW boxes on/off
     $  m_out                           ! output unit for printouts
*
* Formats for writing tables onto disk file.
      CHARACTER*80  m_fmt0, m_fmt1, m_fmt2
...
*/////////////////////////////////////////////////////////////////////////////
      ...
      m_amz   = amz     ! input MZ
      m_amh   = amh     ! input MH
      m_amt   = amtop   ! input MT
      m_amw   = 80d0    ! input, rededined by Dizet
      m_ibox  = ibox    ! =0,1, parameter for ROKANC, EWK boxes 
      m_out   = iout
      m_alfinvMZ  = 128.86674175d0
      m_alfQCDMZ  = 0.125d0

      WRITE(m_out,bxope)
      WRITE(m_out,bxtxt) 'DZface_Initialize, Interface to Dizet 6.xx   '
      WRITE(m_out,bxl1f) m_amz   ,   'Z mass             ','amz   ','a1'
      WRITE(m_out,bxl1f) m_amh   ,   'Higgs mass         ','amh   ','a2'
      WRITE(m_out,bxl1f) m_amt   ,   'Top mass           ','amtop ','a3'
...
* new parameters of 6.x version
     ...
     $ '  Ihvp =',Ihvp,   ' Iamt4 =',Iamt4,
     $ '  Iqcd =',Iqcd,   ' Imoms =',Imoms,
     $ ' Imass =',Imass,  ' Iscre =',Iscre,
     $ ' Ialem =',Ialem,  ' Imask =',Imask,
     $ ' Iscal =',Iscal,  ' Ibarb =',Ibarb,
     $ ' IFtjr =',Iftjr,  ' Ifacr =',IFacr,
     $ ' IFact =',Ifact,  ' Ihigs =',Ihigs,
     $ ' Iafmt =',Iafmt,  ' Iewlc =',Iewlc,
     $ ' Iczak =',Iczak,  ' Ihig2 =',Ihig2,
     $ ' Iale2 =',Iale2,  ' Igfer =',Igfer,
     $ ' Iddzz =',Iddzz
*     =================================================================
...
******      !!!!! Dizet 5.x, obsolete !!!!!
******      CALL dizet( Npar, m_amw, m_amz, m_amt, m_amh,
******     $            AlQedZ, AlStrZ, 
******     $            zpard, partz, partw)
*     =================================================================
* Dizet 6.x
CF      print *,'dizet call',m_alfinvMZ,AlQedZ
      CALL DIZET( 
     $   Npar,   ! Inp. Integer switches
     $  m_amw,   ! I/O. W mass (Out. for Imoms=Npar(4)=1,3)
     $  m_amz,   ! I/O. Z mass (Out. for Imoms=Npar(4)=2,4)
     $  m_amt,   ! Inp. t-quark mass 
...
c))))))))))))))))))))))))))))))))))))))
      ...
      SUBROUTINE DZface_MakeGSW(Mode,ww,cosi,GSW,QCDcorN)
*/////////////////////////////////////////////////////////////////////////
...
*//////////////////////////////////////////////////////////////////////////////
      ...
      INTEGER           Mode
      DOUBLE COMPLEX    GSW(*)
      DOUBLE PRECISION  ww,cosi,QCDcorN
      DOUBLE COMPLEX    xff(4),xfem,xfota
      INTEGER           kk,k,kdumm,kolor,ibfla,ibox
...
*     ---------------------------------------------------
      IF(    Mode.EQ.-1) THEN
         ...
         CALL DZface_GivIzo( m_IVini, 1,aizor,qe,kdumm)
         CALL DZface_GivIzo( m_IVfin, 1,aizor,qf,kolor)
         CALL DZface_QCDtab(-1,dum1,dum2)
         DO k=1,7
            m_GSW(k) =0d0
         ENDDO
      ELSEIF( Mode.EQ.0) THEN
         ...
         uu  = ss*(1d0+cosi)/2d0
         ibox = m_ibox
         IF( ww  .LT. m_WmaxLEP1 ) ibox=0 !<-- in order to be backward compatible...
         ...
         GSW(6 )=xfem
         GSW(7 )=xfota
         CALL DZface_QCDtab(0,ww,QCDcorN)
         GSW(8 )=QCDcorN        !! QCDcor storred as one of formfactors, test of pretabulation
      ELSE
         WRITE(*,*) '++++ STOP in DZface_MakeGSW, wrong Mode =',Mode
         STOP
      ENDIF
      END

      SUBROUTINE DZface_QCDtab(Mode,ww,QCDcorN)
*/////////////////////////////////////////////////////////////////////////
*//     Initialize for Mode=-1 and calculate for Mode=0
*//     the QCD corrections ,QCDcorN for a given flavor KFfin.
*/////////////////////////////////////////////////////////////////////////
      IMPLICIT NONE
C      INCLUDE 'DZface.h'
*//////////////////////////////////////////////////////////////////////////////
...
*//////////////////////////////////////////////////////////////////////////////
      ...
      INTEGER          Mode
      DOUBLE PRECISION QCDcorN,ww
      DOUBLE PRECISION QCDCOR(0:14)  
      DOUBLE PRECISION sqs,sqss,amt,sw2,ALQED_STJ,ALPHST,alphttx,alphtt,alphxi
      INTEGER          k,n
      SAVE
*     -------------------------------------------------------------
      IF (Mode.eq.-1) THEN
* we initialize QCDCOR calculation. Using ALPHST(MZ) as input we calculate
* what is the numerical value of ALPHST((M_T) and store it in ALPHTTx
         sqss   = m_amz
         amt    = m_amt
         sw2    = m_swsq
         ALQED_STJ  = 1d0/m_alfinvMZ
         ALPHST = m_alfQCDMZ
         CALL qcdcof_stj(sqss,AMT,SW2,ALQED_STJ,ALPHST,ALPHTT,ALPHXI,QCDCOR)
         alphttx = alphtt
      ELSE
* m_alfQCDMZ is an input for SQS=MZ, but we need ALPHST  for sqs as it is.
* we will find it iteratively requesting that ALPHST(MT)=ALPHTT recovers
* our previously stored value of alphttx
* we use as start for ALPHST iteration value left from previous table point.
* number of iterations can be decided later, now it is n=4 and seems to be fine
* or even an overkill.
* The 20 GeV is hard-coded minimum in qcdcof anyway ...
         sqs=max(ww,20d0)
         CALL qcdcof_stj(sqs,amt,sw2,alqed_STJ,alphst,alphtt,alphxi,qcdcor)
         n=4
         DO k=1,n
            ALPHST=ALPHST*alphttx/alphtt
            CALL qcdcof_stj(sqs,amt,sw2,alqed_STJ,alphst,alphtt,alphxi,qcdcor)
         ENDDO
         IF (abs(m_KFfin)  .EQ.  5) THEN
            QCDcorN = qcdcor(4)-1d0
         ELSEIF (abs(m_KFfin)  .LT.  5) THEN
            QCDcorN = qcdcor(9)-1d0
         ELSE
            QCDcorN=0d0
         ENDIF
      ENDIF
      ...
      SUBROUTINE DZface_Tabluj(DiskFile)
*/////////////////////////////////////////////////////////////////////////////////////////
*//                                                                                     //
...
*//////////////////////////////////////////////////////////////////////////////
      ...
      CHARACTER*(*)     DiskFile
      DOUBLE PRECISION  QCDcorN,ww,x,cosi
...
*/////////////////////////////////////
* Note that m_amz m_amh, m_amt are already defined.
* We get other params out of dizet using DZface_GetPrm.
      ...
      WRITE(ndisk,m_fmt0) m_amz, m_amh, m_amt, m_swsq, m_gammz, m_amw, m_gammw
*/////////////////////////////////////////////////////////////////
*//   basic s range LEP1 and below        m_cyy(m_poin1+1, 7)   //
*/////////////////////////////////////////////////////////////////
      WRITE(m_out,*) 'pretabulation, basic LEP1 range'
      WRITE(6    ,*) 'pretabulation, basic LEP1 range'
      CALL DZface_MakeGSW(-1, ww, 0d0, GSW, QCDcorN) !initialize QCDcor calculation.
      DO i=0,m_poin1
         x  =DFLOAT(i)/DFLOAT(m_poin1)
         ww =m_WminLEP1*(m_WmaxLEP1/m_WminLEP1)**x
         IF(MOD(i,10)  .EQ.  0) WRITE(    6,*) 'a: i,ww= ',i,ww
         IF(MOD(i,10)  .EQ.  0) WRITE(m_out,*) 'a: i,ww= ',i,ww
         CALL DZface_MakeGSW( 0, ww, 0d0, GSW, QCDcorN) ! at theta=pi
         WRITE(ndisk,m_fmt1) 'a',i,ww
         WRITE(ndisk,m_fmt2) (GSW(k),k=1,7), QCDcorN 
         DO kk=1,7
            m_cyy(i+1,kk)=GSW(kk)
         ENDDO
***      WRITE(6,*) 'KFf=',m_KFfin,'sqrt(s)=',ww,
***  $              'old=',m_QCDcor,'new=',QCDcorN
      ENDDO
*/////////////////////////////////////////////////////////////////
*/              near Z0 resonance    m_czz(m_poin2+1, 7)        //
*/////////////////////////////////////////////////////////////////
      WRITE(m_out,*) 'pretabulation, near Z0:'
      WRITE(    6,*) 'pretabulation, near Z0:'
      CALL DZface_MakeGSW( -1, ww, cosi, GSW, QCDcorN) !initialize QCDcor calculation.
      m_WminZ =m_amz-2d0*m_gammz
      m_WmaxZ =m_amz+2d0*m_gammz
      DO i=0,m_poin2
         X  =DFLOAT(i)/DFLOAT(m_poin2)
         ww =m_WminZ+(m_WmaxZ-m_WminZ)*X
         IF(MOD(i,10)  .EQ.  0) WRITE(    6,*) 'b: i,ww= ',i,ww
         IF(MOD(i,10)  .EQ.  0) WRITE(m_out,*) 'b: i,ww= ',i,ww
         CALL DZface_MakeGSW( 0, ww, 0d0, GSW, QCDcorN) ! at theta=pi
         WRITE(ndisk,m_fmt1) 'b',i,ww
         WRITE(ndisk,m_fmt2) (GSW(k),k=1,7), QCDcorN
         DO kk=1,7
            m_czz(i+1,kk)=GSW(kk)
         ENDDO
      ENDDO
*/////////////////////////////////////////////////////////////////////
*//   the region of boxes, LEP2,   m_ctt(m_poin3+1, m_poinT+1, 7)   //
*/////////////////////////////////////////////////////////////////////
      WRITE(m_out,*) 'LEP2 energy zone: pretabulation starts'
      WRITE(    *,*) 'LEP2 energy zone: pretabulation starts: it will take a bit time'
      CALL DZface_MakeGSW( -1, ww, cosi, GSW, QCDcorN)
      DO  i=0,m_poin3
         X  = DFLOAT(i)/DFLOAT(m_poin3)
         ww =m_WmaxLEP1+(m_WmaxLEP2-m_WmaxLEP1)*X
         IF(MOD(i,10)  .EQ.  0) WRITE(    6,*) 'c: i,ww= ',i,ww
         IF(MOD(i,10)  .EQ.  0) WRITE(m_out,*) 'c: i,ww= ',i,ww
         DO  j=0,m_poinT
            cosi = -1d0+2d0*DFLOAT(j)/DFLOAT(m_poinT)
            cosi = cosi*(1d0-1d-8)
            IF(j  .EQ.        0) cosi=cosi+.3d0/DFLOAT(m_poinT)
            IF(j  .EQ.  m_poinT) cosi=cosi-.3d0/DFLOAT(m_poinT)
            CALL DZface_MakeGSW( 0, ww, cosi, GSW, QCDcorN)
            WRITE(ndisk,m_fmt1) 'c',i,ww,j,cosi
            WRITE(ndisk,m_fmt2) (GSW(k),k=1,7), QCDcorN
            DO  kk=1,7
               m_ctt(i+1,j+1,kk)=GSW(kk) ! stored but, the storage unused
            ENDDO
         ENDDO
      ENDDO
*/////////////////////////////////////////////////////////////////////
*//   the region of boxes, NLC,    m_clc(m_poin4+1, m_poinT+1, 7)   //
*/////////////////////////////////////////////////////////////////////
      WRITE(m_out,*) 'NLC energy range: pretabulation starts'
      WRITE(    *,*) 'NLC energy range: pretabulation starts: it will take a bit time'
      CALL DZface_MakeGSW(-1,ww,cosi,GSW,QCDcorN)
      DO  i=0,m_poin4
         X   = DFLOAT(i)/DFLOAT(m_poin4)
         ww  =m_WmaxLEP2+(m_WmaxNLC-m_WmaxLEP2)*X
         ...
         DO  j=0,m_poinT
            ...
            WRITE(ndisk,m_fmt2) (GSW(k),k=1,7), QCDcorN
            DO  kk=1,7
               m_clc(i+1,j+1,kk) = GSW(kk) ! stored but, the storage unused
            ENDDO
         ENDDO
      ENDDO
      CLOSE(ndisk)
      WRITE(m_out,*) '... pretabulatin finished  now !'
      WRITE(6    ,*) '... pretabulatin finished  now !'
      END

      SUBROUTINE DZface_TablujOLD(DiskFile)
*----------------------------------------------------------------------
* Routine for evaluation of electroweak formfactors
* it provides formfactors for positive ss, tabulated in look up tables.
* !!!!! arguments of rokanc have random signs!!!!????
*----------------------------------------------------------------------
      IMPLICIT DOUBLE PRECISION (a-h,o-z)
C      INCLUDE 'DZface.h'
*//////////////////////////////////////////////////////////////////////////////
...
*//   basic s range LEP1 and below        m_cyy(m_poin1+1, 7)   //
*/////////////////////////////////////////////////////////////////
* first we initialize QCDcor calculation.
      CALL DZface_QCDtab(-1,dum,QCDcorc)
      WRITE(m_out,*) 'pretabulation, basic LEP1 range'
      WRITE(6    ,*) 'pretabulation, basic LEP1 range'
      DO i=0,m_poin1
         x  =DFLOAT(i)/DFLOAT(m_poin1)
...
***      WRITE(6,*) 'KFf=',m_KFfin,'sqrt(s)=',ww,
***  $              'old=',m_QCDcor,'new=',QCDcorN
         WRITE(ndisk,m_fmt2) (m_cyy(i+1,k),k=1,7), QCDcorN 
      ENDDO
*/////////////////////////////////////////////////////////////////
*/              near Z0 resonance    m_czz(m_poin2+1, 7)        //
*/////////////////////////////////////////////////////////////////
* first we initialize QCDcor calculation.
      ...
      DO i=0,m_poin2
         ...
         DO kk=1,4
            m_czz(i+1,kk)=xff(kk)
         ENDDO
         m_czz(i+1,5 )=123.456d0 ! UNUSED!!!!
         m_czz(i+1,6 )=xfem
         m_czz(i+1,7 )=xfota
         WRITE(ndisk,m_fmt1) 'b',i,ww
         CALL DZface_QCDtab(0,ww,QCDcorN)
         WRITE(ndisk,m_fmt2) (m_czz(i+1,k),k=1,7), QCDcorN
      ENDDO
*/////////////////////////////////////////////////////////////////////
*//   the region of boxes, LEP2,   m_ctt(m_poin3+1, m_poinT+1, 7)   //
*/////////////////////////////////////////////////////////////////////
* first we initialize QCDcor calculation.
      CALL DZface_QCDtab(-1,dum,QCDcorc)
      WRITE(m_out,*) 'LEP2 energy zone: pretabulation starts'
      WRITE(    *,*) 'LEP2 energy zone: pretabulation starts'
      WRITE(    *,*) 'warning: it will take a bit time'
      DO  i=0,m_poin3
         X  = DFLOAT(i)/DFLOAT(m_poin3)
         ww =m_WmaxLEP1+(m_WmaxLEP2-m_WmaxLEP1)*X
         IF(MOD(i,10)  .EQ.  0) WRITE(    6,*) 'c: i,ww= ',i,ww
         ...
         DO  j=0,m_poinT
            ...
            st=sss*(1d0-cosi)/2d0
            su=sss*(1d0+cosi)/2d0
            CALL rokanc(m_ibox,ibfla,-su,-sss,-st,qe,qf,xff,xfem,xfota)
            DO  kk=1,4
               m_ctt(i+1,j+1,kk)=xff(kk)
            ENDDO
            m_ctt(i+1,j+1,5 )=123.456d0
            m_ctt(i+1,j+1,6 )=xfem
            m_ctt(i+1,j+1,7 )=xfota
            WRITE(ndisk,m_fmt1) 'c',i,ww,j,cosi
            CALL DZface_QCDtab(0,ww,QCDcorN)
            WRITE(ndisk,m_fmt2) (m_ctt(i+1,j+1,k),k=1,7), QCDcorN
         ENDDO
      ENDDO
*/////////////////////////////////////////////////////////////////////
*//   the region of boxes, NLC,    m_clc(m_poin4+1, m_poinT+1, 7)   //
...
* first we initialize QCDcor calculation.
      CALL DZface_QCDtab(-1,dum,QCDcorc)
      WRITE(m_out,*) 'NLC energy range: pretabulation starts'
      WRITE(    *,*) 'NLC energy range: pretabulation starts'
      WRITE(    *,*) 'warning: it will take a bit time'
      DO  i=0,m_poin4
         X   = DFLOAT(i)/DFLOAT(m_poin4)
         ww  =m_WmaxLEP2+(m_WmaxNLC-m_WmaxLEP2)*X
         ...
         DO  j=0,m_poinT
            ...
            CALL rokanc(m_ibox,ibfla,-su,-sss,-st,qe,qf,xff,xfem,xfota)
            DO  kk=1,4
               m_clc(i+1,j+1,kk)=xff(kk)
            ENDDO
            m_clc(i+1,j+1,5 )=123.456d0
            m_clc(i+1,j+1,6 )=xfem
            m_clc(i+1,j+1,7 )=xfota
            WRITE(ndisk,m_fmt1) 'd',i,ww,j,cosi
            CALL DZface_QCDtab(0,ww,QCDcorN)
            WRITE(ndisk,m_fmt2) (m_clc(i+1,j+1,k),k=1,7), QCDcorN
         ENDDO
      ENDDO
      CLOSE(ndisk)
      WRITE(m_out,*) '... pretabulatin finished  now !'
...
*//
*//  In this version  ReaDataX and  ReaDataN  reset xpar or npar to zero!!!!
*//
...
*
      ...
      SUBROUTINE MY_DZface_Tabluj(DiskFile,indice)
*/////////////////////////////////////////////////////////////////////////////////////////
*//                                                                                     //
*//  Routine for evaluation and pretabulation of electroweak formfactors                //
*//                                                                                     //
...
* We get other params out of dizet using DZface_GetPrm.
      CALL DZface_GetPrm( amzD,m_gammz,gammz1,m_amw,m_gammw,gammw1,m_swsq)
C      ndisk=20
C      OPEN(ndisk,FILE=DiskFile)
C      WRITE(ndisk,m_fmt0) m_amz, m_amh, m_amt, m_swsq, m_gammz, m_amw, m_gammw
      header(1) = m_amz
      header(2) = m_amh
      header(3) = m_amt
      header(4) = m_swsq
...
*/////////////////////////////////////////////////////////////////
      ...
      DO i=0,m_poin1
         ...
         IF(MOD(i,10)  .EQ.  0) WRITE(m_out,*) 'a: i,ww= ',i,ww
         CALL DZface_MakeGSW( 0, ww, 0d0, GSW, QCDcorN) ! at theta=pi
C         WRITE(ndisk,m_fmt1) 'a',i,ww
C         WRITE(ndisk,m_fmt2) (GSW(k),k=1,7), QCDcorN 
         m_syy_p(i+1,indice) = QCDcorN
         DO kk=1,7
            m_cyy(i+1,kk)=GSW(kk)
            m_cyy_p(i+1,kk,indice) = GSW(kk)
         ENDDO
...
*/////////////////////////////////////////////////////////////////
      ...
      DO i=0,m_poin2
         ...
         IF(MOD(i,10)  .EQ.  0) WRITE(m_out,*) 'b: i,ww= ',i,ww
         CALL DZface_MakeGSW( 0, ww, 0d0, GSW, QCDcorN) ! at theta=pi
C         WRITE(ndisk,m_fmt1) 'b',i,ww
C         WRITE(ndisk,m_fmt2) (GSW(k),k=1,7), QCDcorN
         m_szz_p(i+1,indice) = QCDcorN
         DO kk=1,7
            m_czz(i+1,kk)=GSW(kk)
            m_czz_p(i+1,kk,indice)=GSW(kk)
         ENDDO
      ENDDO
*/////////////////////////////////////////////////////////////////////
*//   the region of boxes, LEP2,   m_ctt(m_poin3+1, m_poinT+1, 7)   //
*/////////////////////////////////////////////////////////////////////
      WRITE(m_out,*) 'LEP2 energy zone: pretabulation starts'
      WRITE(m_out,*) 'LEP2 energy zone: pretabulation starts: it will take a bit time'
      CALL DZface_MakeGSW( -1, ww, cosi, GSW, QCDcorN)
      DO  i=0,m_poin3
         X  = DFLOAT(i)/DFLOAT(m_poin3)
         ww =m_WmaxLEP1+(m_WmaxLEP2-m_WmaxLEP1)*X
         ...
         DO  j=0,m_poinT
            ...
            IF(j  .EQ.  m_poinT) cosi=cosi-.3d0/DFLOAT(m_poinT)
            CALL DZface_MakeGSW( 0, ww, cosi, GSW, QCDcorN)
C            WRITE(ndisk,m_fmt1) 'c',i,ww,j,cosi
C            WRITE(ndisk,m_fmt2) (GSW(k),k=1,7), QCDcorN
            m_stt_p(i+1,j+1,indice) = QCDcorN
            DO  kk=1,7
               m_ctt(i+1,j+1,kk)=GSW(kk) ! stored but, the storage unused
               m_ctt_p(i+1,j+1,kk,indice)=GSW(kk)
            ENDDO
         ENDDO
      ENDDO
*/////////////////////////////////////////////////////////////////////
...
*/////////////////////////////////////////////////////////////////////
      WRITE(m_out,*) 'NLC energy range: pretabulation starts'
      WRITE(m_out,*) 'NLC energy range: pretabulation starts: it will take a bit time'
      CALL DZface_MakeGSW(-1,ww,cosi,GSW,QCDcorN)
      DO  i=0,m_poin4
         X   = DFLOAT(i)/DFLOAT(m_poin4)
         ww  =m_WmaxLEP2+(m_WmaxNLC-m_WmaxLEP2)*X
         ...
         DO  j=0,m_poinT
            ...
            IF(j  .EQ.  m_poinT) cosi=cosi-.3d0/DFLOAT(m_poinT)
            CALL DZface_MakeGSW( 0, ww, cosi, GSW, QCDcorN)
C            WRITE(ndisk,m_fmt1) 'd',i,ww,j,cosi
C            WRITE(ndisk,m_fmt2) (GSW(k),k=1,7), QCDcorN
            m_slc_p(i+1,j+1,indice) = QCDcorN
            DO  kk=1,7
               m_clc(i+1,j+1,kk) = GSW(kk) ! stored but, the storage unused
               m_clc_p(i+1,j+1,kk,indice) = GSW(kk) 
            ENDDO
         ENDDO
      ENDDO
C      CLOSE(ndisk)
...
*
      ...
      ENDIF
*
      IF(SQS.GE.350D0 .AND. icont.LE.10 ) THEN
c stj
         icont=icont+1
        PRINT *,'Requested SQRT(S).GE.350 GeV. Program not foreseen above the t-tbar threshold!'
c        PRINT *,'Program STORs, since it is not foreseen'
c        PRINT *,'to run above the t-tbar threshold.'
      ENDIF
*
...
*                                                                         
CF      print *,'alfspi',alfspi
CF      print *,'alqed',alqed
CF      print *,'charqu',charqu
CF      print *,'coef02',coef02
CF      print *,'almt',almt
CF      print *,'rlmt',rlmt
CF      print *,'coef03',coef03
CF      print *,'qcd2v',qcd2v
CF      print *,'qcd4v',qcd4v
CF      print *,'sins',sins
CF      print *,'cai2',cai2
CF      print *,'cai3',cai3
CF      print *,'qcd2a',qcd2a
CF      print *,'qcd4a',qcd4a
CF      print *,'amcrun',amcrun
CF      print *,'sqs',sqs
CF      print *,'amt',amt
CF      print *,'ambrun',ambrun    
...
*//////////////////////////////////////////////////////////////////////////////
      ...
      SUBROUTINE QCDCOF_STJ(SQS,AMT,SW2,ALQED_STJ,ALFAS,ALFAT,ALFAXI,QCDCOR)
*
      IMPLICIT REAL*8(A-H,J-Z)
*
      COMMON/CDZSCT/ISCRE,ISCAL,IAFMT,IFACR,IFACT,IHIGS,IEWLC,ICZAK
...
* R_V
*
      QCDCON(2*IQ-1)=1+ALFSPI+1D0/4*ALQED_STJ/PI*CHARQU(IQ)**2*(3-ALFSPI)
     &          +(1.40923D0+(44D0/675-2D0/135*ALMT)*RLMT)*ALFSPI**2
     &          +(-12.76706D0)*ALFSPI**3
     &+12*AMQRUN(IQ)**2/SQS**2*ALFSPI*(1+8.7D0*ALFSPI+45.15D0*ALFSPI**2)
*
      QCDCOR(2*IQ-1)=1+ALFSPI+1D0/4*ALQED_STJ/PI*CHARQU(IQ)**2*(3-ALFSPI)
     &              +(COEF02+(44D0/675-2D0/135*ALMT)*RLMT)*ALFSPI**2
     &              + COEF03*ALFSPI**3
     &              +QCDC2V(IQ)+QCDC4V(IQ)
*
* R_A
*
      QCDCON(2*IQ  )=1+ALFSPI+1D0/4*ALQED_STJ/PI*CHARQU(IQ)**2*(3-ALFSPI)
     &   +(1.40923D0+(44D0/675-2D0/135*ALMT)*RLMT+SINS*CAI2)*ALFSPI**2
     &   +(-12.76706D0+SINS*CAI3)*ALFSPI**3
     &   -6*AMQRUN(IQ)**2/SQS**2*
     &                      (1+11D0/3*ALFSPI+(11.286D0+ALMT)*ALFSPI**2)
...
*
     ...
     &       -10D0*AMBRUN**2/AMT**2*(8D0/81-1D0/54*ALMT)*ALFSPI**2
      ...
      ENDIF
*
      QCDCOR(2*IQ  )=1+ALFSPI+1D0/4*ALQED_STJ/PI*CHARQU(IQ)**2*(3-ALFSPI)
     &        +(COEF02+(44D0/675-2D0/135*ALMT)*RLMT+SINS*CAI2)*ALFSPI**2
     &        +(COEF03+SINS*CAI3)*ALFSPI**3                            
     &        +QCDC2A(IQ)+QCDC4A(IQ)                                   
     &        +RASING
...
*
      IF(IMTADD.EQ.1) THEN
        CALL VTBANA(AMZ2,WWv2,WWv11,WWv12,WWbg)
      ELSE
        CALL VTBANA(SB,WWv2,WWv11,WWv12,WWbg)
      ENDIF
*
* WW-box via XBOX
*
...
* table is writen in non-standard output. Its name is in first line of
* the input file.
*/////////////////////////////////////////////////////////////////////////////
      SUBROUTINE tabini(amz, amh, amtop)
      IMPLICIT NONE
C      INCLUDE 'BornV.h'
*//////////////////////////////////////////////////////////////////////////////
*//                                                                          //
...
*-------------------------------------------------------------------------------
       ...
       WRITE (iout,*) '************Tabulation code using DIZET***********'
       WRITE (iout,*) 'M_Z = ',amz
       WRITE (iout,*) 'M_top = ', amtop
       WRITE (iout,*) 'M_Higgs = ', amh
       WRITE (iout,*) '**  Tabulation code using DIZET begins ***********'

*       amz = 91.187d0
*       amh = 100d0
*       amtop = 175d0
       KFini = 11       !! 11d0 in standard programm !!
       
      DO i=1,16 
       IF (i.EQ.1 .OR. i.EQ.2 .OR. i.EQ.5 .OR. i.EQ.13 
     &      .OR. i.EQ.15) THEN
        IF(i.eq.1) DiskFile= 'table.down'
        IF(i.eq.2) DiskFile= 'table.up'
        IF(i.eq.5) DiskFile= 'table.botom'
        IF(i.eq.13) DiskFile= 'table.mu'
        IF(i.eq.15) DiskFile= 'table.tau'

        KFfin = i
        ibox  = 1       !!!

*set EW params and run Dizet
        CALL DZface_Initialize( KFini,  KFfin, amz, amh,  amtop, ibox,  iout)

* calculate formfactor and  dump on disk
        CALL MY_DZface_Tabluj(DiskFile,i)
       ENDIF  
      ENDDO
       CALL DIZET_TO_KK(amz, amh, amtop)
      RETURN
      END
...
*
      ...
      SUBROUTINE VTBANA(S,WWv2,WWv11,WWv12,WWbg)   
*
...
*
     ...
     &           -(RWS+1D0-.25D0/RWS-(.5D0-RWS)*RTW)*(AJ0w-2D0)
     &           +(2D0*RWS+3D0/2/RTW1**2-2D0/RTW1+1D0/2
     &           -(.5D0-RWS)*RTW)*ALRT
     &           -(RWS+3D0/2/RTW1+3D0/4+.25D0/RWS-(.5D0-RWS)*RTW)
     &           )
...
* AL4PI is removed from ALFQCD as Tord suggested
 ...
 2    ALFQCD=0D0
      GO TO 5
 3    ALFQCD=ALQCDS(ALST,AMZ2,AMW2,AMT2)
CF      print *,'case 3',ALST,AMZ2,AMW2,AMT2
      GO TO 5
 4    CONTINUE
...
CF      print *,'in xfotf3, stage 1',xfotf3,XCHQ21,UDCSB/AL4PI
CF      print *,'in xfotf3, stage 1',DIMAG(XCHQQQ),ALFQCD,ITOP
*
* this is a correction to Im part of \alpha_em, which improved
* the agreement with TOPAZ0
*     XFOTF3=8*XCHQ21
...
* 
      ...
      ENDDO
C END FRANCK
CF      print *,'DIZET Z/W WIDTH input' ,DR,DRBIG,DRREM,QCDCOR,PARTZ,PARTW
*
...
*
      IF(INF.LE.4.OR.ABS(QCDCOR((MAX(0,2*INDQ(INF)-1)))-1).LT.1D-8) THEN
        RQCDV=1.D0+0.75D0*CALQED/PI*AQFI(INF)**2
        RQCDA=RQCDV
      ELSE
