...
C  get compressed code  for particle
      ...
      IF ( KC.GT.0) THEN
        ...
        IK = IW(JGRPL+2)
        IF ( IK .GT. NBR) THEN
          IF (IW(6).GT.0) WRITE(IW(6),'(''===KXL7BR ====you try to modi
     $fy a non existant line'',I8,'' maximum is '',I10)') IK,NBR
          GO TO 102
        ENDIF
...
C  get compressed code  for particle
      ...
      IF ( KC.GT.0) THEN
        ...
        IF ( NBR .NE. IW(JGMOB)-1) THEN
          IF (IW(6).GT.0) WRITE(IW(6),'(''===KXL7BR ==== BR numbers do
     $not agree for part '',I8,'' should be '',I10)') NKF2,NBR
          GO TO 101
        ENDIF
        IENTRY = MDCY(KC,2)
        DO 500 JJ=1,NBR
...
C store the card on the C list
 ...
 1000 IF (IW(6).GT.0) WRITE (IW(6),550) NKF1,K
  550 FORMAT(1X,'++++++KXL7BR+++++++ cannot extend decay list for partic
     $le',I10,'Pointer MDCE was',I10)
      END
