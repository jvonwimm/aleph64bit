...
C!   symmetric matrix previously reduced by TRID. D is a vector of lengt
C!   NP. On input it first n elments are the diagonal elements of the
C!   tridiagonal matrix. On output, it returns the eigenvalues. The vect
C!   E inputs the subdiagonal elements of th tridiagonal matrix with E(1
C!   arbitrary. On output E is destroyed.
C!   If the eigenvectors of the tridiagonal matrix are desired, the matr
C!   Z is input as the identity matrix.
C!   If the eigenvectors of a previously reduced matrix by TRID are desi
C!   Z is input as the outpt matrix of TRID.
C!   In either case, the kth column of Z returns the normalized eigenvec
