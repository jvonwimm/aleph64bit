...
C                                      if 0 all is ok
C                                      if 1, problems with covariance ma
C                                      of charged tracks in ychiv2
C                                      if 2, problems with covariance ma
C                                      of charged tracks in ynv0ve
C                                      if 3, problems with covariance ma
C                                      of charged tracks in ynvonf
C                                      if 4, problems in yvcame with frf
C                                      if 5, problems inverting covarian
C                                      matrix of reconstructed V0 vertex
C                                      candidate
C                                      if 6, the number of V0 candidates
C                                      greater than MAXV
C                                      if 7, no booked enlarged space fo
...
C                                      the track coming from V0 candidat
C                                      to the origin
C                QGA(*)/REAL           Q value in the gamma hypothesis
C                CHQGA(*)/REAL         chi2 of Q value in the gamma hypo
C                QLA(*)/REAL           Q value in the lambda hypothesis
C                CHQLA(*)/REAL         chi2 of Q value in the lambda hyp
C                QAL(*)/REAL           Q value in the antilambda hypothe
C                CHQAL(*)/REAL         chi2 of Q value in the antilambda
C                                      hypothesis
C                QK0(*)/REAL           Q value in the k0 hypothesis
C                CHQK0(*)/REAL         chi2 of Q value in the k0 hypothe
C                XMGA(*)/REAL          invariant mass in the gamma hypot
C                CXMGA(*)/REAL         chi2 of invariant mass in the gam
C                                      hypothesis
C                XMLA(*)/REAL          invariant mass in the lambda hypo
...
C                XMK0(*)/REAL          invariant mass in the k0 hypothes
...
C
             ...
             CVK0=QVAL*QVAL/ERQ2
                IF(IBOOK)THEN
                   ...
                   CALL HF1(JOFF+217,QVAL*QVAL/ERQ2,1.)
                ENDIF
        IF (QVAL*QVAL/ERQ2.LT.YV0CMA) ISK0=.TRUE.
             ISLA=.FALSE.
...
C
              ...
              CVLA=QVAL*QVAL/ERQ2
                  IF(IBOOK)THEN
                    ...
                    CALL HF1(JOFF+219,QVAL*QVAL/ERQ2,1.)
                  ENDIF
        IF (QVAL*QVAL/ERQ2.LT.YV0CMA) ISLA=.TRUE.
             ISAL=.FALSE.
...
C
              ...
              CVAL=QVAL*QVAL/ERQ2
                   IF(IBOOK)THEN
                       ...
                       CALL HF1(JOFF+219,QVAL*QVAL/ERQ2,1.)
                   ENDIF
      IF (QVAL*QVAL/ERQ2.LT.YV0CMA) ISAL=.TRUE.
      ISGA=.FALSE.
...
C
                   IF(IBOOK)THEN
                      ...
                      CALL HF1(JOFF+221,QVAL*QVAL/ERQ2,1.)
                   ENDIF
      IF (QVAL*QVAL/ERQ2.LT.YV0CMA) ISGA=.TRUE.
