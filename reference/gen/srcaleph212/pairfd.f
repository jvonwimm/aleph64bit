...
C         DZ2 = the distance(cm) apart the tracks are in z at the xy poi
...
C         DIN1(2) = Distance(cm) in radius that the furthest in coordina
C                   is inside of RMA for track 1(2).  0. if no coordinat
C                   information is available or there are no such coordi
...
C the tracks at these same points and at the origin.  Also the differenc
C in theta of the two tracks is returned. The average position of the tw
...
C point is also calculated.  For each track the number of cordinates "in
C the materialization point is returned.  A coordinate is "inside" if it
C has Z or Radius from the Z-Axis less than the materialization point.
