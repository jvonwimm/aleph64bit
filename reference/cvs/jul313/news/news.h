#if defined(DOC)
   on AXP/VMS:
        source:  JUL313:[*]*.F,JUL313:[INC]*.h
        library: JUL:JUL313.OLB, _D.OLB, .NEWS
        program: JUL:JUL313.EXE, _D.EXE


   on UNIX :  source:    $ALROOT/jul313/?_
              library:   $ALEPH/jul/libjul313.a  _dbx.a
              program:   $ALEPH/jul/jul313.3

 -----------------------------------------------------------------------------
 * 20010619 corr file 313.3 - Changes for official MC production

            In r_news.h
    RLOOPR : Add call ALEVNUM for official production monitoring (B.Bloch)
    RINJOB : call ADBCHK to check DB official production version (B.Bloch)

 * 20010205 corr file 313.2 - Fixes for LEP 2 reprocessing
            In l_news.h
    LPREMC : Remove offline pedestal follower also for 1997 data (P.Hansen)

            In r_news.h
    RDEFIO : Add bank KW4F to POT output list (M.Cattaneo)

 * 20001204 corr file 313.1 - Fixes from 2000 reprocessing

            In t_news.h
    TPADDX : Protection against ASIN argument > 1.  (M.Cattaneo)

! 20001026 JULIA 31.3 - Version for 2000 reprocessing

            In e_news.h
    ENIGHB : Protect against ISTO out of range (M.Cattaneo)
