*CD news
 ...
 ! 20010619 - galeph309.3 
   ASIJOB, ASIEVE - check database version no. and store last evt no.
                    executed during prodduction
   CAGERB, CAHERB, CAHGRB - add same protection as in CAHHRB (D.Smith)

 ! 20000302 - galeph309.2
   CAHHRB   - add a protection to avoid sqrt(-ve) (D.Smith)

 ! 20000201 - galeph309.1
   HCNSTR   - it is an INTEGER function but calculations were made as if 
              it was a REAL function.
              make all caluations in REAL and then fill the INTEGER function.
   CATINO   - HPANTP is a REAL function and not INTEGER.
   LCADC    - remove a cut on ADC count:
              The saturation occurs at about 160 GeV (which is already 
              implemented in LCROC).
              In LCADC there is also a hardwired truncation of wire-energies.
              That is more or less correct.(P.Hansen)
