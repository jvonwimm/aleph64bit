   on AXP/VMS:
              source:  ALEPH316:[*]*.F,ALEPH316:[INC]*.h
              library: ALE:ALEPHLIB.OLB, _D.OLB, .NEWS

   on UNIX :  source:  $ALROOT/aleph316/*/*
              library: $ALEPH/gen/libalephlib.a     -> libalephio316.a
              library: $ALEPH/gen/libalephlib_dbx.a -> libalephio316_dbx.a

   --------------------------------------------------------------------
 ! 20010619 correction file no.2 for ALEPHLIB 31.6
 ...
 ! 20010219 correction file no.1 for ALEPHLIB 31.6

   In /alef
    GETLE2 : Mods for pathological miniramps of Year 2000 data (J.Boucrot)

 ! 20001120 ALEPHLIB 316.0

   In /comp
    PPCORR : Fix problem found by S.Jezequel on Linux with call
             of ADBRPN. Solved by declaring it correctly.        (G.Dissertori)

   In /dedx
    TDEDXV : Remove check on ramping sectors at start of run     (G.Taylor) 
   ...
   In /kine
    KXP6BR : To allow usage of NREF cards, check the KF codes which have 
             changed (Psi' and Upsilon') from 30443/30553 to 100443/100553)
                                                                 (B.Bloch)
    KXP6CO : Modified to allow usage of NREF cards, ie give the 
             proper offset to MDM1 cards concerned and proper KF code 
             when modified from verison 7.3x to 6.1y             (B.Bloch)

   In /ptoj
    TPDZWC : remove the test on Julia version #.
             suppress the message at first usage of the routine  (A.Bonissent)
    TUN1NC : suppress the message at first usage of the routine  (A.Bonissent)

   In /yv0
    YMFV0V : More protections, plus bug fix: quality factor for antiLambda
             in QAL was that for Gamma                           (M.Cattaneo)
   --------------------------------------------------------------------
  ! 20001106 correction file no.7 for ALEPHLIB 31.5
   In /alef
    IQVERP : NEW - Transferred from Alpha: Return program version 
             used to produce the current file       (A.Bonissent)

  ! 20001106 correction file no.6 for ALEPHLIB 31.5
   In /ptoj
    TPDZWC : NEW - Returns for a given sector, padrow, and rphi
       coordinates of a hit the TPD no., the preamplifier no. for this TPD, 
       the pad channel no. for this TPD and a z correction for the hit due
       to pad-wire timing differences.              (W.Wiedenmann)
    TUN1NC : add TPD pad-wire correction factor for coordinates with
             wire information                       (W.Wiedenmann)

  ! 20001102 correction file no.5 for ALEPHLIB 31.5
   In /ytop
    YFMVTR : Fail if Eta greater the 7500km!! 
             (protection for run 58457 event 234)   (M.Cattaneo)

  ! 20000801 correction file no.4 for ALEPHLIB 31.5
   In /comp
    THLCIR : Protect against RHOC = 0. (M.Cattaneo)

  ! 20000725 correction file no.3 for ALEPHLIB 31.5
   In /alef
    GTT0GL : handle 2x2 bunch running in 2000 (W.Wiedenman)

   In /edir
    SELEVT : Call GETLEP instead of ALELEP (J.Boucrot)
 
  ! 20000502 correction file no.2 for ALEPHLIB 31.5

   In /tpc
    TFIXT0 : Fix T0 problem in early 2000 high energy runs (W.Wiedenman)

  ! 20000405 correction file no.1 for ALEPHLIB 31.5

   In /alef
    GETLE2, GETLEP: Fix wrong LEP energy in RLEP for some early
             year 2000 runs at 91.25 Gev              (J.Boucrot)

 ! 20000320 ALEPHLIB 315.0 - Version for 2000 data-taking
   ...
   In /sdes
    SICLID : sical cleaning of identified background sources      (B.Bloch)
    SIBKCL : returns identification flag                          (B.Bloch)
    SIFSUM : computes the energy per plane and number of pads (total
             and dead) in a given cluster                         (B.Bloch)

      SICLID and SIBKCL were formerly in ALPHA
      SIFSUM was formerly in Julia ( in the same deck as SIESUM)


   --------------------------------------------------------------------
  ! 20000128 correction file no.2 for ALEPHLIB 31.4
 ...
 ! 20000127 correction file no.1 for ALEPHLIB 31.4

   In /comp
    PPCORR : modification added for Y2K compliance           (G.Dissertori)
