...
#define CALLOC(ptr, nr, type) ptr = NULL; if ((nr) > 0 && !(ptr = (type *) calloc((size_t)(nr), sizeof(type)))) { fprintf(stderr, "calloc failed on line %d of file %s\n", __LINE__, __FILE__); exit(FAIL); } 

#define MALLOC(ptr, nr, type) ptr = NULL; if ((nr) > 0 && !(ptr = (type *) malloc((size_t)((nr) * sizeof(type))))) {fprintf(stderr, "malloc failed on line %d of file %s\n", __LINE__, __FILE__); exit(FAIL); }

#define REALLOC(ptr, nr, type) \
if ((nr)==0) { free(ptr); ptr = NULL; } else { \
   ptr = (type *) (ptr ? realloc(ptr,(size_t)(nr) * sizeof(type)) \
 		       : malloc((size_t)((nr) * sizeof(type))) ); \
   if (!ptr) { fprintf(stderr, "realloc failed on line %d of file %s\n", \
            __LINE__, __FILE__); exit(FAIL); }}
