...
C     Modify: Agnieszka Jacholkowska   17-Jul-95
C
...
C
   ...
   40 CONTINUE
C
C++   Discard SICAL events
C
      CALL STRIGF(IRUN,TLCAL,TRAND,TPHYS,TSCAL,FAIL)
      IF (.NOT.TLCAL.AND..NOT.TRAND
     &   .AND..NOT.TPHYS.AND..NOT.FAIL) THEN
        DO 41 JJ=1,4
          IF(TSCAL(JJ)) THEN
            KEEP = 0
          ENDIF
