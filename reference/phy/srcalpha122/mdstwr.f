...
C
      IF ( krun .GT. 4000 ) GOTO 998
      nwords = 2 + 8 * (klmct - kfmct + 1)
      ifwki = nbank('FWKI',0,nwords)
      IF ( ifwki .LE. 0 ) THEN
        WRITE (IW(6),*) 'Bank FWKI doesn''t exist'
        GOTO 999
      ENDIF
      iw(ifwki +  1) = 8
      iparmc = 0
      ifkin = IW(nafkin)
      DO 2 imc = kfmct , klmct
        IF ( kstabc(imc) .LE. 0 ) GOTO 2
        iparmc = iparmc + 1
        kpfkin = ktn(imc)
        RW(ifwki + 3 + (iparmc-1)*8 ) = qx(imc)
        RW(ifwki + 4 + (iparmc-1)*8 ) = qy(imc)
        RW(ifwki + 5 + (iparmc-1)*8 ) = qz(imc)
        RW(ifwki + 6 + (iparmc-1)*8 ) = qm(imc)
        IW(ifwki + 7 + (iparmc-1)*8 ) = ktpcod(imc)
        IW(ifwki + 8 + (iparmc-1)*8 ) = 0
        IW(ifwki + 9 + (iparmc-1)*8 ) = 0
        IW(ifwki +10 + (iparmc-1)*8 ) = IW(ifkin + 10 + (kpfkin-1)*8)
    2 CONTINUE
      iw(ifwki +  2) = iparmc
      nwords = 2 + 8*iparmc
      ifwki = nbank('FWKI',0,nwords)
C
      CALL bdrop(iw,'FKINFVER')
      ifkin = nbank('FKIN',0,nwords)
      ifver = nbank('FVER',0,2)
      CALL ucopy(IW(ifwki+1), IW(ifkin+1), nwords)
      IW(ifver+1) = 8
      IW(ifver+2) = 0
C
 998  CALL blist(iw,'T=',
     .    'EVEHPTHRNBIPEJETPDLTFWEVFWPAFWIDFWCHFWD0FWDXFKINFVER')
      CALL minnof(-1)
      CALL bwrite(IW,lout,
     .    'EVEHPTHRNBIPEJETPDLTFWEVFWPAFWIDFWCHFWD0FWDXFKINFVER')
      CALL minnof(+1)
      CALL bdrop(IW,'FWEVFWPAFWKIFWIDFWCHFWD0FWDXFKINFVER')
