...
C----------------------------------------------------------------------
C!  - Fit NTKD tracks in ITKD(I) to a common vertex.
C!
...
C
      ...
      IF(NTKD.LT.2)THEN
         CALL QWMESS
     +   ('_KVFITN_ Less than two daughters -- no fit done.')
         GOTO 999
      ENDIF
      ICLASS=IQVEC(ITKD(1),JQVECL)
      IF(ICLASS.EQ.0)THEN
         CALL QMTERM
     +   ('_KVFITN_ Particle with class = 0.')
      ENDIF
      DO 11 I=2,NTKD
      IF(IQVEC(ITKD(I),JQVECL).NE.ICLASS)THEN
         CALL QMTERM
     +   ('_KVFITN_ Particles of different classes mixed up.')
      ENDIF
11    CONTINUE
C
C--- Call vertexing routine
C
      CALL QYFIT(ITKD,NTKD,PSUM,VPSUM,ICHR,VOUT,VVOUT,VPSVX,CHI2)
C
C    check return code
C
...
9       CONTINUE
 ...
 7         CONTINUE
      ...
      IF (CNAM .NE. ' ')  THEN
        ...
        IW(KOQFPA+IFP*KCQFPA+ICLASS) = KVFITN
      ELSE
        IW(JR(JQVEPA)) = 0
      ENDIF
C
