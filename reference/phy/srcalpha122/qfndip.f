...
C  described in the aleph note.  The changes are all BACKWARDS COMPATIBL
...
C
C  Inputs;
C     BP(3),SIG2_BP(3)  :  Beam spot position and elipsoid.  The elipsoi
...
C                          RETURN IT AS OUTPUT.  If the GET_BP call fail
C                          NUSED will be returned as 0,0
C
C     NJET,JETS         :  Number of jets and unit vectors of their mome
C                          direction.
C
...
C                          your alpha cards (EFLJ means to cluster up th
...
C
C  Outputs;
C     IP,IP_ERR,CHI2_DOF   The found vertex, its correlation matrix, and
C                          CHI2/DOF of the "fit".
C
C     NUSED                The number of tracks used in the "fit", separ
...
C
     ...
     &       SIG_Z0 .GT. MAX_ERR(2) )THEN
          ...
          END IF
C
C  This is a good track; save it's 5 parameters and errors in the local
C
