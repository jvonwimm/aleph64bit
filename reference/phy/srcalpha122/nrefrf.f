...
C?   dangerous thing because not all tracks are kept on the NANO DST and
C?   remaining are reordered. So the pointers from other banks like YV0V
C?   to reset and there is no easy way to get the original 'JULIA' numbe
C?   only way is to link to the NDTK bank and ask for the JULIA track nu
C?   this bank. There is a one-to-one relation between the row numbers i
C?   and FRFT made from NDTK.
C?   Also there is a problem with the track quality. Normally only good
C?   are kept on the NANO DST but although those which are coming from a
C?   a gamma conversion candidate even if they are not 'good' ones. This
C?   done for quality studies (dE/dx, # of hits in TPC/ITC etc.). Unfort
C?   there is no proper way to put some quality flag into FRFT. So this
C?   information is put into the error matrix which is zero by default.
C?   value of QFRFEM(ITK,1,1) [99999.0] indicates that this is a 'bad' t
...
C--------------------- end of QCDE ------------------------------------
     ...
     &  JNDHP2,JNDHVX,JNDHVY,JNDHVZ,JNDHS1,JNDHS2,JNDHT1,JNDHT2,JNDHM1,
     &  JNDHM2,LNDHEA
