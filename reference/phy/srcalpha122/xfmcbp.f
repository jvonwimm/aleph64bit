...
C     Local variables declaration:
      ...
      INTEGER I, J, IPER, IWT(NMX), ISET, ITOT
...
C     Link to ALRP:
      KALRP = IW(NAALRP)
      IF (KALRP .LE. 0) THEN
        CALL QWMESE('_XFMCBP_ ALRP bank not found.')
        GO TO 1000
      ENDIF
      NALRP = LROWS(KALRP)
C
...
C
      IF (LBSIZ) THEN
        IF (LBPER)
     >    CALL QWMESS('_XFMCBP_ BSIZ card found; BPER card ignored.')
        IF (LBPWT)
     >    CALL QWMESS('_XFMCBP_ BSIZ card found; BPWT card ignored.')
        IF ((NBSIZ .NE. 2) .AND. (NBSIZ .NE. 3)) THEN
          CALL QMTERM('_XFMCBP_ Invalid syntax on BSIZ card.')
          GO TO 1000
        ENDIF
C
        IGNSET = .TRUE.
...
C     (The period names in ALRP can be of the form YYMM or 19YY.)
        IF (NALRP .GT. NMAX) THEN
          CALL QMTERM(
     >         '_XFMCBP_ Too many run periods--increase NSETMX, NMX.')
          GO TO 1000
        ENDIF
        DO IALRP=1,NALRP
...
C     Print the valid run periods if appropriate:
          IF (.NOT. BPEROK) THEN
            WRITE (IW(6),'(/1X,A)') 'Valid run periods for BPER card:'
            WRITE (IW(6),'(1X,20I5)') (JSETOK(I), I=1,NALRP)
            CALL QMTERM(' ')
            GO TO 1000
          ENDIF
C
C     BPWT with no BPER is invalid:
        ELSE
          CALL QMTERM(
     >       '_XFMCBP_ BPWT card must be accompanied by a BPER card.')
          GO TO 1000
        ENDIF
C
C     BPER and BPWT must have equal numbers of tokens:
        IF (LBPWT) THEN
          IF (NBPER .NE. NBPWT) THEN
            CALL QMTERM('_XFMCBP_ BPER and BPWT cards '//
...
C     If LEP2 periods are involved and NBPER > 1, BPWT must be given:
        IF (LEP2 .AND. (NBPER .GT. 1) .AND. (.NOT. LBPWT)) THEN
          CALL QMTERM('_XFMCBP_ Give a BPWT card to set '//
...
C     Watch out for crazy weights:
        ...
        IF (ERROR) THEN
          CALL QMTERM('_XFMCBP_ Invalid run period weights.')
          GO TO 1000
        ENDIF
C
...
C     Print the parameters:
        IF (NSET .EQ. 1) THEN
          ...
          CALL QWMESS('_XFMCBP_ Using luminous region parameters'//
     >                ' for run period '//CNAM)
          WRITE (IW(6),'(1X,A,3F8.4)') '_XFMCBP_ Average sigma_x,y,z =',
     >                              (VTX_SIZ(J,NSET), J=1,3)
C
        ELSE
          CALL QWMESS('_XFMCBP_ Luminous region parameters:')
          CALL QWMESS('         '//
     >                'Period   Weight   Average sigma_x,y,z (cm)')
          DO I=1,NSET
            WRITE (IW(6),'(1X,I13,F10.3,F10.4,F9.4,F7.2)')
...
C     error message:
          ...
          CALL QWMESE('_XFMCBP_ Luminous region parameters'//
