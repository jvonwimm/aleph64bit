...
C!   ===========
C!
C?   This subroutine creates the banks FKIN, FZFR and FVER from the bank
C?   and NDNT. A problem is that not enough information is stored on the
C?   DST to get back the same order of particles as in the original FKIN
...
C
C!..LOOP OVER ALL PARTICLES (NDNT FIRST) AND PUT ALL QUANTITIES IN AN AR
...
C!..MATCHED TRACK IN NDTK BANK ??
  ...
  110 CONTINUE
C
C!..EVERYTHING IS IN AN ARRAY NOW. START COPING IN ANOTHER ONE SO THAT A
C!..DAUGHTERS OF A MOTHER ARE TOGETHER. START WITH THOSE WHO HAVE NO MOT
C
      IPOINT=1
...
C!..SET VERTICES
  ...
  150   CONTINUE
        IF(NXVERT)THEN
          ...
          IVERAR(IVERT,3)=NDAU
C!..HERE WE HAVE TO CHECK WHETHER THE MOTHER IS A V0 OR NOT BECAUSE FOR
...
C
C!..NOW LOOP OVER ALL PARTICLES TO RECALCULATE THE MOMENTA OF THE UNSTAB
C
