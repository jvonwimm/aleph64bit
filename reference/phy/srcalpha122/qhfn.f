...
CKEY HIST /USER
C------------------------------------------------------------------
C!  fill N-tuple with variable number of arguments
...
C------------------------------------------------------------------
      REAL PV(80),RLOW(80),RHIGH(80)
      CHARACTER CHTITL,TAGS(80)
      NA = 80
      CALL HGIVEN(ID,CHTITL,NA,TAGS,RLOW,RHIGH)
      IF(NA.GT.80) THEN
        CALL QWMESS('_QHFN_  more than 80 arguments. Call ignored ')
        GO TO 90
      ENDIF
C
      IF (NA.LE.0) GO TO 90
