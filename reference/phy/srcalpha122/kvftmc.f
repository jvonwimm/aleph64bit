...
C----------------------------------------------------------------------
C!  - Fit NTKD tracks in ITKD(I) to a common vertex using a lagrange
C!    multipier mass constraint between a subset of the tracks.
C!
C!   Author   :- G. Taylor         3-DEC-1993
C!
C!   Inputs:
C!  NTKD      - Number of tracks to be fit to a vertex
C!  ITKD      - Array of track numbers
C!  CNAM      - alpha name of created track
C!  USE       - logical array of size NTKD which determines
C!              whether this track is used in mass constraint
C!  CONMAS    - mass to which tracks are constrained
C!
C!   Outputs:
...
C
      ...
      IF(NTKD.LT.2)THEN
       CALL QWMESS
     +   ('_KVFTMC_ Less than two daughters -- no fit done.')
       GOTO 999
      ENDIF
      ICLASS=IQVEC(ITKD(1),JQVECL)
      IF(ICLASS.EQ.0)THEN
       CALL QMTERM
     +   ('_KVFTMC_ Particle with class = 0.')
      ENDIF
      DO 11 I=2,NTKD
       IF(IQVEC(ITKD(I),JQVECL).NE.ICLASS)THEN
        CALL QMTERM
     +   ('_KVFTMC_ Particles of different classes mixed up.')
       ENDIF
   11 CONTINUE
C
C--- Call vertexing routine
C
      CALL QYFTMC(USE,CONMAS,ITKD,NTKD,PSUM,VPSUM,ICHR,VOUT,VVOUT
     &            ,VPSVX,CHI2,rmads,dmass)
C
C    check return code
C
...
C
    ...
    7 CONTINUE
      ...
      IF (CNAM .NE. ' ')  THEN
       ...
       IW(KOQFPA+IFP*KCQFPA+ICLASS) = KVFTMC
      ELSE
       IW(JR(JQVEPA)) = 0
      ENDIF
C
