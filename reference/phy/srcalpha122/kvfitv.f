...
C----------------------------------------------------------------------
C!  - Fit NTKD tracks ITKD(I) to a common vertex using vertex IVTX as
C!  - additional constraint.
C!
C!   Author   :- E. Blucher, H.G.Moser         26-AUG-1992
C!
C!   Inputs:
C!        - Vertex number
...
C
      ...
      IF(NTKD.LT.1)THEN
         CALL QWMESS
     +   ('_KVFITV_ Less than one track -- no fit done.')
         GOTO 999
      ENDIF
      ICLASS=IQVEC(ITKD(1),JQVECL)
      IF(ICLASS.EQ.0)THEN
         CALL QMTERM
     +   ('_KVFITV_ Particle with class = 0.')
      ENDIF
      DO 11 I=2,NTKD
      IF(IQVEC(ITKD(I),JQVECL).NE.ICLASS)THEN
         CALL QMTERM
     +   ('_KVFITV_ Particles of different classes mixed up.')
      ENDIF
11    CONTINUE
C
C--- Call vertexing routine
C
      CALL QYFITV(IVTX,ITKD,NTKD,PSUM,
     +VPSUM,ICHR,VOUT,VVOUT,VPSVX,CHI2)
C
C    check return code
C
...
9       CONTINUE
 ...
 7         CONTINUE
      ...
      IF (CNAM .NE. ' ')  THEN
        ...
        IW(KOQFPA+IFP*KCQFPA+ICLASS) = KVFITV
      ELSE
        IW(JR(JQVEPA)) = 0
      ENDIF
C
