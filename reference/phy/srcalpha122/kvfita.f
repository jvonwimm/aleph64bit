...
C----------------------------------------------------------------------
     ...
     +    ('_KVFITA_ No particle name given')
        IER = 1
        GO TO 80
      ENDIF
C
...
C
      ...
      IF (SXX .LE. 0. .OR. ICLASS .EQ. 0)  THEN
        NER = NER + 1
        IF (NER .LE. 10)  CALL QWMESE
     +    ('_KVFITA_ Covariance matrix not available')
        IER = 2
        GO TO 80
      ENDIF
      SXY = RW(J1(JQVEEM+1))
...
C
      ...
      IF (CS .LT. 1E-12)  THEN
        NER = NER + 1
        IF (NER .LE. 10)  CALL QWMESE  ('_KVFITA_ Singular matrix')
        IER = 3
        GO TO 80
      ENDIF
      CS = 1. / CS
...
C
      ...
      IF (FA .GT. FO + 1E-3)  THEN
        NER = NER + 1
        IF (NER .LE. 10)  CALL QWMESE  ('_KVFITA_ Chi2 increasing')
        IER = 4
        GO TO 80
      ENDIF
      FV = FO
      ...
      IF (ITERAT .LE. 10)  GO TO 10
        NER = NER + 1
        IF (NER .LE. 10)  CALL QWMESE  ('_KVFITA_ Too many iterations')
      IER = 5
      GO TO 80
C
C       new track number :
...
C
     ...
     +    RW(JR(JQVEEM+5)) .LE. 0. .OR. RW(JR(JQVEEM+9)) .LE. 0.)  THEN
        NER = NER + 1
        IF (NER .LE. 10)  CALL QWMESE
     +    ('_KVFITA_ Inconsistency in fitted cov. matrix')
        IER = 6
        GO TO 80
      ENDIF
C
...
C
      ...
      IW(KOQFPA+IFP*KCQFPA+ICLASS) = KVFITA
      IF (IW(J1(JQVEND)) .NE. 0)  CALL QAMOTH
     &  (KVFITA, IW(J1(JQVEND)), IW(KOQLIN+IW(J1(JQVEDL))+1))
C
C       KSAME pointer :
C
