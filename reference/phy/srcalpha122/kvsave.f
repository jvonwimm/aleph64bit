...
CKEY PART /USER
C----------------------------------------------------------------------
C! Save particle with name CNAM
C                                                    H.Albrecht  2.1.89
C----------------------------------------------------------------------
      SAVE IERR
...
C
      ...
      IF (CNAM .NE. ' ')  THEN
        ...
        IW(KOQFPA+IFP*KCQFPA+ICLASS) = KVSAVE
        IF (IW(J1(JQVEND)) .NE. 0)  CALL QAMOTH
     &    (KVSAVE, IW(J1(JQVEND)), IW(KOQLIN+IW(J1(JQVEDL))+1))
      ELSE
        IW(JR(JQVEPA)) = 0
        IF (ICLASS .EQ. 0)  GO TO 10
      ENDIF
...
C
   10 IERR = IERR + 1
     ...
     &  ('_KVSAVE_ Particle to be saved has zero attributes')
      IW(JR(JQVENP)) = 0
C
C       KSAME pointer :
C
