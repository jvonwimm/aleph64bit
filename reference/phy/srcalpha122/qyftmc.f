...
C
C!    interface for KVFTMC : simplified use of YTOP from alpha
C     H-G Moser 31-09-92
C
C     input parameters :    USE         use this track in mass constrain
C                           CONMAS      mass to constrain tracks to
C                           ILST        list of track numbers
...
C
C
      CALL YFMVTC(.TRUE.,USE,CONMAS,
     &    0,NCHR,NNEU,.TRUE.,.TRUE.,.TRUE.,.FALSE.,
     &    DUM  ,DUM   ,ITKLIST,
     &    5,15,
     &    FRF,VFRF,
     &    ITNLIST,5,15,
...
C
C     chi**2 / NDF  NDF = 2 * NTRACK - 3 + 3 * NVERTEX + NUM MASS CONST
C
      NDF = NTR * 2 - 3 + 1
      CHI2 = CHI2 / FLOAT(NDF)
C
      IF (CHI2.LT.500.) THEN
C Divide 1st to avoid arithmetic overflow in multiplications djaffe 17ja
        R1 = PSUM(1)/PSUM(4)
        R2 = PSUM(2)/PSUM(4)
        R3 = PSUM(3)/PSUM(4)
        R4 = RMADS/PSUM(4)
