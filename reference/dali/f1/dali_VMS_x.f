...
C-------------------End of DALI_CF commons----------------
      PARAMETER (DVI$_TT_PAGE = 170)
      INTEGER*2 L2
      INTEGER ISTAT,LIB$GETDVI,LATEXT,I,ILEN,K,KFIRST,KLAST,NUMMAC,
     1 STATUS,SYS$FAO,L1
     ...
     & TMSAVE(MACLEVMAX)
      ...
      LENTRY=.TRUE.
      ATEXT='LB'
      IF(LSCRN.EQ.0) ISTAT=LIB$GETDVI(DVI$_TT_PAGE,,'TT:',LSCRN,,)
  100 CALL DWRT('.................................................')
      IF(LENTRY) THEN
...
C
  ...
  470    NUMMAC=0
         ...
         CALL DGUPCS(TSCR,TSCR)
         STATUS=SYS$FAO('!UL macros with !UL lines were written to !AS',
     1    L2,TSCR1,%VAL(NUMMAC),%VAL(KMAX),TSCR(1:LENOCC(TSCR)))
...
C
      ...
      CHARACTER TSCR*128
      INTEGER LENSTR
      INTEGER I,NADDR,NKEY,NCHAR,ILEN
      LOGICAL FMOP
...
C   keyboard input.
C   EVFLAG(8) is set at a mouse M1 click in a selected area (top menu bar)
C   of the main window.
C
    5 CALL DGHMRK
      TON = 'Y'
...
CBSN            WRITE(*,'(1H+,X,A,$)') ICHSTR(1:LCHSTR)
CBSN 8-May-1996 CALL DWR_ADD(' '//ICHSTR(1:LCHSTR))
            IF((ICHSTR(1:LCHSTR).GE.'a').AND.(ICHSTR(1:LCHSTR).LE.'z'))
     &         FHLPLC=.TRUE.
            IF(.NOT.FHLPLC) CALL DWR_ADD(ICHSTR(1:LCHSTR))
            ICOM=ICOM+LCHSTR-1
         ELSE
...
C
    ...
    9 IF((ISTAT.NE.2).AND.(.NOT.FCANCL).AND.(.NOT.FMACDM))
     & WRITE(MUN9DU,'(1H+,A,$)') ICHSTR(1:LCHSTR)
      ICHSTR(LCHSTR+1:) = ' '
...
C-------------------End of DALI_CF commons----------------
      ...
      INTEGER*2 L2
      INTEGER ICNTRL,NUMMAC,STATUS,SYS$FAO,NRLINE,I1,LMACNF,L1,KMAC
...
C
      PARAMETER (MACMX=500, MACLEVMAX=8)
      CHARACTER TMNAME*20,TMAC*80,MACFIL*80/' '/,TMSAVE*20
      INTEGER LSCRN/0/,KMAX/0/,MACLEN,MACLEV/0/,MACSAVE
      LOGICAL FMSAVE
      COMMON /MACCMN/ LSCRN,KMAX,MACLEN(MACMX),MACFIL,TMNAME(MACMX),
     & TMAC(MACMX),MACLEV,MACSAVE(3,MACLEVMAX),FMSAVE(2,MACLEVMAX),
     & TMSAVE(MACLEVMAX)
...
C
   ...
   29    CLOSE(UNIT=NUNIDU)
         ...
         IF(ICNTRL.EQ.-2) THEN
            CALL DWRT_SETUP('TERMINAL=ON')
            CALL DGUPCS(MACFIL,MACFIL)
            STATUS=SYS$FAO('!UL lines with !UL macros were read from'//
     1       ' !AS',L2,TSCR,%VAL(NRLINE),%VAL(NUMMAC),
     2       MACFIL(1:LENOCC(MACFIL)))
            CALL DWRT(TSCR(1:L2))
            RETURN
         END IF
         IF(AMAC.EQ.'***') RETURN
...
C
      ...
      Character Ename*32, FullName*120, DFName*120
      Integer LenOcc
...
C  The VMS version is a dummy version.
C
      Implicit None
      Character*(*) FileName
      Character Ename*120
...
C
      Implicit None
      Character T72*72, Errhex*8
      Integer Istat, LIB$Spawn

      Call DGIFOC(1,0)
      Call DWR_HIGH_LIGHT_FROM_N1_TO_N2(
     & 'SP: Creating subprocess... Return by typing LOGOUT',1,2)
      Istat = LIB$Spawn(,,,,,,,,,,'DALI_SPAWN>')
      Call DGIFOC(0,-1)
      If (Istat .NE. 1) Then
         Write(Errhex,'(Z8.8)') Istat
         Call DWR_HIGH_LIGHT_FROM_N1_TO_N2(
     &    'SP: Error spawning. Status: '//Errhex,1,2)
      Else
         Call DWR_HIGH_LIGHT_FROM_N1_TO_N2(
...
C
      ...
      Istat = LIB$Spawn(LPR//' '//TMETA)
      IF (Istat.NE.1) Then
         Write(Errhex,'(Z8.8)') Istat
         CALL DWRT('Error printing or creating file. Status: '//Errhex)
      EndIf
      End
...
C
  ...
  100 LENOCC = I2
      ...
      Function NOP_SYS(ICASE)
C   Return an integer specifying what system we are using.
C
      Integer Icase
C VMS
      NOP_SYS = 1
...
C-------------------------------------------------------------DXPNTR
     ...
     & HSPVDX(MXCUDX),POINDX(32,MXCUDX,2)
C-------------------------------------------------------------

      CHARACTER*32 OUT_OF_BAND_CHARS/'CEHILR?'/
      LOGICAL FLGOOB,FLGDOP
      COMMON /OOBCMN/ FLGOOB(0:31),FLGDOP
...
CH
      ...
      CLOSE(UNIT=MUN9DU)
      CALL DCONLO
    2 OPEN(UNIT=MUN9DU,FILE='DALI.LOG',STATUS='NEW',
     & CarriageControl='FORTRAN')
      CALL DWRT('Output on file DALI.LOG')
      RETURN

      ENTRY DGETRE
C No longer required in X-windows. M. Parsons May 1991.
      RETURN
      END
...
C   CTRL-Z.
      ...
      WRITE(MUN9DU,'(1H+,A,$)') ILNSTR(1:LCHLIN)
...
CH ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
CH
C VMS only #ifdef VMS
      EXTERNAL DW_ERRLG
      LOGICAL LERON            !  Flag to turn error handler on/off
      DATA LERON/.TRUE./
      DATA IRFLG,IRTRC/0,0/    !  Error handling parameters
C
C
...
C  No message to unit 9, since it is NOT yet assigned to DALI.LOG
      IF (LERON) THEN
         CALL DWRT_SETUP('TERMINAL=OFF')
         CALL DWRT('DALI> Error handler established.')
         CALL DWRT_SETUP('TERMINAL=LAST')
         CALL LIB$ESTABLISH(DW_ERRLG)
         CALL DW_ERROR(IRFLG,IRTRC)
      END IF
C VMS only #endif /* VMS */
      RETURN
CH..............+++
CH
CH
...
C VMS Only #ifdef VMS
      IF (LERON) THEN
        CALL DWRT_SETUP('TERMINAL=OFF')
        CALL DW_ERROR(-1,0)
        CALL DWRT('DALI> Closing error handler ....')
        CALL DWRT_SETUP('TERMINAL=ON')
      ENDIF
C VMS Only #endif /* VMS */
      END
...
C
C
      SUBROUTINE DW_ERR(AFLAG,IER)
C
C----------------------------------------------------------------------
C   4-Dec-89  Chris Grab
C
C  Purpose:
C  Control the error handler DW_ERROR, which allows to turn off certain
C  error messages, and gives a summary at the end.
C
C  Note : Calls to DW_ERR(..) are only meaningful, if they are done
C         within the top level, since the error option table cannot
C         be arbitrarily modified.
C
C  Input : AFLAG = 'ON'  Turn errors+ messages on
C                = 'OF'  Turn error messages off +initialize error counters
C                = 'PR'  Print error statistics
C
C  Output : IER  > 0     Ok
C           IER  = 0     No action done
C           IER  < 0     Error occurred
C
C  --------------------------------------------------------------------
      CHARACTER *2 AFLAG
      EXTERNAL DW_ERRLG
C Initialize and default numbers for error handler
      DATA IRFON,IRTON/0,0/
      DATA IRFOF,IRTOF/0,0/
      DATA IRFPR,IRTPR/-1,0/
C
      IER=0
C Initialise error handler, turn errors off (reset error counters):
      IF (AFLAG.EQ.'OF') THEN
        CALL DWRT('DALI> Error handler established. Messages OFF')
        CALL LIB$ESTABLISH(DW_ERRLG)
        CALL DW_ERROR(IRFOF,IRTOF)
        IER = 1
      ENDIF
C
C Turn error messages back on again:
      IF (AFLAG.EQ.'ON') THEN
        CALL DWRT('DALI> Error handler reset, messages ON')
        CALL LIB$REVERT
        IER = 2
      ENDIF
C
C Turn error messages back on again:
      IF (AFLAG.EQ.'PR') THEN
        CALL DWRT('DALI> Calling error handler statistics')
        CALL DW_ERROR(IRFPR,IRTPR)
        IER = 3
      ENDIF
      RETURN
C
...
C *********************************************************************
C
      SUBROUTINE DW_ERROR(IFLAG,ITRACE)
C
C----------------------------------------------------------------------
C   4-Dec-89  Chris Grab
C
C  Error handler, adapted from M. Schmellings math error logger
C
C IFLAG .GE. 0 : INITIALISE ERROR HANDLING
C       .LT. 0 : PRINTOUT ERROR STATISTICS
C
C IFLAG   =  0 : INFINITE NUMBER OF ERRORS ALLOWED
C         >  0 : IFLAG    NUMBER OF ERRORS ALLOWED
C
C ITRACE       : NUMBER OF TRACEBACKS TO BE PRINTED
C----------------------------------------------------------------------
C
C
      COMMON    /ERRINF/ INDERR(100),NCOUNT(100),IFLSAV,ITRSAV
C
C--------Table of Fortran run time error numbers-----------------------
C
C     INCLUDE 'DALI_CF.INC'
C------------------------------------------------------------------- DW
      COMMON /DWORKT/ TXTADW
...
C-------------------End of DALI_CF commons----------------
      DIMENSION IERR(100)
      DATA      IERR/70,71,72,73,74,77
     +,              82,83,84,87,63,64, 88*0/
      DATA      NERR/12/
      CHARACTER *40 ERRMES(12)
      DATA      ERRMES( 1)/'Integer overflow               : '/
      DATA      ERRMES( 2)/'Integer zero divide            : '/
      DATA      ERRMES( 3)/'Floating overflow              : '/
      DATA      ERRMES( 4)/'Floating zero divide           : '/
      DATA      ERRMES( 5)/'Floating underflow             : '/
      DATA      ERRMES( 6)/'Subscript out of range         : '/
      DATA      ERRMES( 7)/'Undefined exponentiation       : '/
      DATA      ERRMES( 8)/'LOG of zero or negative value  : '/
      DATA      ERRMES( 9)/'SQRT of negative value         : '/
      DATA      ERRMES(10)/'Significance lost in math-lib  : '/
      DATA      ERRMES(11)/'Output conversion error        : '/
      DATA      ERRMES(12)/'Input  conversion error        : '/
C
C----------------------------------------------------------------------
      LOGICAL   LCOUNT
C
...
C
      ...
      IF(IFLAG.GE.0) THEN
        DO 10 I=1,NERR
          INDERR(I) = IERR(I)
          NCOUNT(I) = 0
          CALL ERRSET(INDERR(I),.TRUE.,LCOUNT,.FALSE.,.TRUE.,MAXLIM)
   10   CONTINUE
        IFLSAV = IFLAG
        ITRSAV = ITRACE
      ENDIF
C
C--      Printout of statistics
C
      IF(IFLAG.LT.0) THEN
        CALL DWRT_SETUP('TERMINAL=OFF')
        CALL DWR_LINE(10,'-----')
        CALL DWRT(' DALI> Run ERROR Summary :')
        DO 20 I=1,NERR
          IF(NCOUNT(I).GT.0) THEN
            WRITE(TXTADW,6901) I,ERRMES(I),NCOUNT(I)
 6901       FORMAT(I2,2X,A40,I10,' time(s)')
            CALL DWRC
          END IF
   20   CONTINUE
        CALL DWR_LINE(10,'-----')
...
C  *******************************************************************
C
      INTEGER FUNCTION DW_ERRLG(ISIG,IMEC)
C---------------------------------------------------------------------
C  4-Dec-89  C.Grab Exception handler, adapted
C                   from M. Schmellings math error logger
C
C---------------------------------------------------------------------
C
      COMMON /ERRINF/ INDERR(100),NCOUNT(100),IFLSAV,ITRSAV
C
C-------- CORRESPONDENCE BETWEEN ERROR NUMBERS IN INDERR AND ------
C-------- THE INDEX IDX RETURNED BY LIB$MATCH                ------
C
      DIMENSION          INDCOU(100)
      DATA               INDCOU/ 1, 2, 3, 3, 4, 4, 5, 5, 6
     +,                          7, 8, 9,10,11,12, 85*0./
C-------------------------------------------------------------------
C
C     INCLUDE 'DALI_CF.INC'
...
C-------------------End of DALI_CF commons----------------
      LOGICAL   LCOUNT
      INTEGER   ISIG(*),IMEC(*),IHELP
      REAL                      RHELP
      EQUIVALENCE (IHELP,RHELP)
      CHARACTER *40 ERRMES(12)
...
C
      IDX         = LIB$MATCH_COND(ISIG(2),SS$_INTOVF
     +,                                    SS$_INTDIV
     +,                                    SS$_FLTOVF
     +,                                    SS$_FLTOVF_F
     +,                                    SS$_FLTDIV
     ...
     +,                                   FOR$_INPCONERR)
CC
      IF(IDX.EQ.0) GOTO 9999
      IDSAVE = IDX
C
C--      HANDLING OF KNOWN EXCEPTIONS
C
      IDX         = INDCOU(IDX)
      IF(IDSAVE.NE.4.AND.IDSAVE.NE.6.AND.IDSAVE.NE.8)
     +NCOUNT(IDX) = NCOUNT(IDX) + 1
      IF(NCOUNT(IDX).EQ.ITRSAV+1) THEN
        IF(IFLSAV.EQ.0) THEN
          LCOUNT = .FALSE.
          MAXLIM = 10
        ENDIF
        IF(IFLSAV.GT.0) THEN
          LCOUNT = .TRUE.
          MAXLIM = MAX(0,IFLSAV+1-ITRSAV)
        ENDIF
        CALL ERRSET(INDERR(IDX),.TRUE.,LCOUNT,.FALSE.,.FALSE.,MAXLIM)
      ENDIF
C

      IF (IDSAVE.NE.14) THEN      ! not allowed for out-con-error
        IF (PARADU(45).EQ.0.) THEN
          CALL DWRT(' DALI ERROR encountered: '//ERRMES(IDX))
        ENDIF
      ENDIF
C
    9 DW_ERRLG  = SS$_RESIGNAL
      RETURN
C
C--      UNKOWN EXCEPTION
C
 9999 CONTINUE
      DW_ERRLG  = SS$_RESIGNAL
      RETURN
      END
C for VMS #endif /* VMS */
