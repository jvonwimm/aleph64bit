 ...
 * AIX support introduced on 13-May-1997 (puha!).
 * This version is also used by Atlantis. 5-May-1997, BSN.
...
#endif
#if !defined(drand48)
CXX double drand48(void);
#endif
#endif /* Missing in Linux */
...
Font font[2][2], mainfont, helpfont, greekfont, boldhelpfont, italhelpfont;
...
static XFontStruct *italhelpfontinfo, *greekfontinfo;
...
static logical wait_for_keyinput = True;
...
static XtInputCallbackProc get_nonX_input(XtPointer, int *, XtInputId *);
...
CXX void dgalgr(int *iwind, int *lev, char *tlet, int idir, float h1, float v1,
	float h2, float v2);
CXX XtEventHandler keyinput(Widget w, XtPointer junk, XKeyEvent *cdata);
void drawtextnonh(float *htxt, float *vtxt, int xoffset, int yoffset,
	char *text, int *n, int fontflag, int nonv);
void dgrotg(int np, float x[], float y[], float h, float v);
void setXcursor(char data[], int fg, int bg, int hsx, int hsy);
void text_to_PS(unsigned char *cin, int n, unsigned char *cout);
void write_colors_to_meta(void);
CXX void dgbtit();
CXX void dgcome(FortranString *
#ifdef __VMS
	);
#else
        , int tcoml);
#endif /* VMS */
void mdgclpa(int *, float*, float *, float *, float*),
  mdgalph(int*, float *, float*, int*),
  mdgpoin(float *, float*),
  mdgplot(int *, float *, float *),
  mdgdraw(int *, float *, float *),
  mdgtext(float *, float *, FortranString *, int *, int *),
  mdgtxtg1(float *, float *, FortranString *, int *, int *, int *),
  mdgtxtg2(int *, int *, FortranString *, int *),
  mdgtdir(int *),
  mdglevl(int *),
  mdglevs(int *, int*),
  mdgarcl(int *, float *, float *, int *),
  mdgarea(int *, float *, float *),
  mdgcler(float *, float *, float *, float *),
  make_PS_postamble(),
  mdgalgr(int *, int*, char *, int *, float *, float *, float *, float *,
    FortranString *, int *),
  mdgdash(int *, int *);

CXX void dgsppt(int *, int*), dgswpt(int *);
#ifdef _AIX
CXX int dgooba(int *);
#else
CXX int dgooba(int);
#endif
int make_PS_prolog(int, int, int);
CXX int tty_set(void);
CXX int tty_reset(void);
void tty_end(void);
...
static unsigned char dali_bits[] = {
...
#ifdef ATLAS
		printf("\n\n*** You have iconized the ATLANTIS window! ***");
#else
...
#ifdef ATLAS
	fprintf(stderr, " a) Keep the ATLANTIS.LOG.x file from this execution.\n");
#else
...
"    For DECNET, use \"setenv DISPLAY node ::0\" (note the double colon)."
	...
	static char italhelpfontname[4][80] = {
		"                                                                     ",
		"-Adobe-Courier-Medium-O-Normal--12-120-75-75-M-70-ISO8859-1",
		"-Adobe-Courier-Medium-O-Normal--14-100-100-100-M-*-ISO8859-1",
		" "
	};
	static char fontname00[6][80] = {
		"                                                                     ",
	...
	static char greekfontname[3][80] = {
		"                                                                     ",
...
#ifdef ATLAS
	disp = XtOpenDisplay(Dali_context, NULL, "Atlantis", "ATLANTIS", NULL,
		0, &argc, argv);
#else
	disp = XtOpenDisplay(Dali_context, NULL, "xdali", "XDali", NULL,
		0, &argc, argv);
#endif
	if (disp == NULL) {
#ifdef __VMS
 ...
 */
	iconpix = XCreateBitmapFromData(disp, XDefaultRootWindow(disp),
		(char *)dali_bits, dali_width, dali_height);
/*
 * Determine visual used and colormap.
 */
	our_screen = XDefaultScreen(disp);
#if defined(__cplusplus) || defined(c_plusplus)
	our_visual_class = XDefaultVisual(disp, our_screen)->c_class;
#else
...
#ifdef ATLAS
	toplevel[0] = XtAppCreateShell("Atlantis", "ATLANTIS",
#else
 ...
 */
	...
	strcpy(italhelpfontname[0],defdata.italhelpfont);
 ...
 */
	...
	for (index = 0; index<(int)strlen(defdata.debug); index++) {
 ...
 */
		...
		KeybInputId = XtAppAddInput(Dali_context, QIO_event_flag,
			(XtPointer)QIO_iosb, (XtInputCallbackProc)get_nonX_input, NULL);
/*
 ...
 */
	...
	if (!debug_mask[(int)('i'-'a')]) {
		KeybInputId = XtAppAddInput(Dali_context, fileno(stdin),
			(XtPointer)XtInputReadMask, (XtInputCallbackProc)get_nonX_input, NULL);
#ifndef __osf__
/*
 * The next works for all implementations but Digital UNIX 4.x
 */
		KeybInputId = XtAppAddInput(Dali_context, fileno(stdin),
			(XtPointer)XtInputExceptMask, (XtInputCallbackProc)get_nonX_input, NULL);
#endif /* Out for DU 4.x */
		if (tty_set()) {
...
/*
 * Help window font and bold variant.
 */
 ...
 */
			...
			int i;
			for (i=0; i<=(int)strlen(cp);i++)
				boldname[i] = _tolower(cp[i]);
			if(!(cp = strstr(boldname, "medium"))) goto after_bold;
			*cp = 'b'; cp++;
			*cp = 'o'; cp++;
			*cp = 'l'; cp++;
			*cp = 'd'; cp++;
			...
			*cp = '\0';
			...
			if (fontlist == NULL) goto after_bold;
			boldhelpfontinfo = XLoadQueryFont(disp, *fontlist);
			if (boldhelpfontinfo == NULL) goto after_bold;
...
after_bold:
	...
	XFreeFontNames(fontlist);
/*
 * An Oblique/Italic font for the Help window.
 */
	for (index = 0; italhelpfontname[index][0] != ' '; index++) {
		fontlist = XListFonts(disp, italhelpfontname[index], 1, &num_fonts);
		if (fontlist == NULL) continue;
		italhelpfontinfo = XLoadQueryFont(disp, *fontlist);
		if (italhelpfontinfo != NULL) break;
		XFreeFontNames(fontlist);
	}
	if (italhelpfontinfo == NULL) {
		printf("\nNo Italic help font could be loaded.\n"); fflush(stdout);
		exit(0);
	} else {
		italhelpfont = italhelpfontinfo->fid;
		if (debugf) {
			printf("\nHelp font: %s", *fontlist); fflush(stdout);
		}
		XFreeFontNames(fontlist);
 ...
 */
		...
		for (index = ((int)dusdac.areadu + 1); index < ddefcn.numcdd; index++) {
			ddefco.ltabdd[index] =
 ...
 */
		...
		if (dtvccc.f256dt) {
			int nplanes=1, ncol=ddefcn.numcdd;
/*
 ...
 */
			if ((our_visual_class != TrueColor) &&
			 !XAllocColorCells(disp, cmap, True, plane_mask, nplanes, pixels, ncol)) {
				...
				printf("should therefore stop it and start this program again before restarting the\n");
 ...
 */
			...
			for (index = 0; index < 16; index++) {
				if (ddefco.ltabdd[1] == (int)colors[index].pixel)
					store1 = index;
				if (ddefco.ltabdd[defdata.bg] == (int)colors[index].pixel)
					store2 = index;
			}
...
/*
 * Create a new window with (almost) the same properties as the main DALI
 * (Atlantis) window. The upper left corner should be at (xco,yco). The
 * number of the window is winno. The number of the main DALI window is 0.
 */
{
	int index, i;
#ifdef __VMS
...
#endif
#ifdef ATLAS
	if (txt[0] == ' ') sprintf(txt, "ATLANTIS %d", window_num);
#else
...
/*
 * Select window number winno for output, and return the new window
 * number in newwin.
 * Put in on top of the window stack if doraise is non-zero.
 */
{
	if (toplevel[*winno] == 0) {
 ...
 */
{
	...
	cliparea.y = fiy ? (short) *v1cl :
		(short) (pixdata[window_num].y - (short) *v2cl);
...
{
	dgalgr(NULL, leval, (char *)"ALEPH", 0, *h1, *v1, *h2, *v2);
}
...
)
{
	...
	XTextExtents(fontinfo[0][1], "ALEPH", 5, &dirctn, &ascent,
		&descent, &overall);
	height = ascent;
	XDrawString(disp, drawin, gc253, (int) *halef, fiy ? (int) *valef :
 ...
 */
{
	...
	XDrawPoint(disp, drawin, grcon, (int) *hpoi, fiy ? (int) *vpoi :
		(pixdata[window_num].y - (int) *vpoi));
 ...
 */
{
	...
	for (index = 0; index < *n; index++)
		XDrawPoint(disp, drawin, grcon, (int) h[index],
			fiy ? (int) v[index] : (pixdata[window_num].y - (int) v[index]));
 ...
 */
{
	...
	for (index = 1; index < *n; index++) {
		ih = (int) h[index - 1];
		iv = fiy ? (int) v[index - 1] :
			(pixdata[window_num].y - (int) v[index - 1]);
		ih2 = (int) h[index];
		iv2 = fiy ? (int) v[index] :
 ...
 */
{
	...
	if (verticaltext) {
#ifdef __VMS
		drawtextnonh(htxt, vtxt, dtxth, dtxtv, (char *) text->dsc$a_pointer,
#else
		drawtextnonh(htxt, vtxt, dtxth, dtxtv, (char *) text,
#endif /* VMS characters */
			n, 0, 0);
	} else {
		int tint = ((int) *vtxt + dtxtv - (mainfontinfo->descent +
			mainfontinfo->ascent));
		x = (int) *htxt + dtxth;
		y = fiy ? tint : (pixdata[window_num].y - tint);
		if (fiy) {
#ifdef __VMS
			drawtextnonh(htxt, vtxt, dtxth, dtxtv, (char *) text->dsc$a_pointer,
#else
	...
	for (index = 0; index < *n; index++) {
		...
		if (index == nposgr[ig]-1 && ig < *ntotgr) {
			...
			if (verticaltext) {
				xf = (float) x;
				yf = (float) y;
				drawtextnonh(&xf, &yf, dtxth, dtxtv, &txchar, &i, 0, 0);
			} else {
				int tintx = x + dtxth;
				int tinty = (y + dtxtv - (mainfontinfo->descent +
					mainfontinfo->ascent));
				if (fiy) {
					xf = (float)tintx;
					yf = (float)tinty;
					drawtextnonh(&xf, &yf, dtxth, dtxtv, &txchar, &i, 0, 1);
				} else {
					XDrawString(disp, drawin, grcon, x + dtxth,
						fiy ? tinty : (pixdata[window_num].y - tinty), &txchar, 1);
				}
			}
#ifdef __VMS
...
)
{
	...
	if (duiscc.fmetdu) mdglevs(l0, (int *)&grcon_val.foreground);
}
 ...
 */
	...
	for (index = 0; index < narc; index++) {
		...
		points[index].y = fiy ? (short) v[index] :
			((short) pixdata[window_num].y - (short) v[index]);
	}
 ...
 */
{
	...
	XChangeGC(disp, grcon, GCForeground, &grcon_val);
	XFillRectangle(disp, drawin, grcon, (int) *hc1,
		fiy ? (int) *vc2 : (pixdata[window_num].y - (int) *vc2),
 ...
 */
	if (astcm1.fbox) {
		...
		ivc1 =  fiy ? (int) *vc1 : (pixdata[window_num].y - (int) *vc1);
		ivc2 =  fiy ? (int) *vc2 : (pixdata[window_num].y - (int) *vc2);
		if (ivc1 <= ivc2) {
			ivc1 =  fiy ? (int) *vc2 : (pixdata[window_num].y - (int) *vc2);
			ivc2 =  fiy ? (int) *vc1 : (pixdata[window_num].y - (int) *vc1);
		}
		for (index = 0; index < 4; index++) {
			pxt = (int) astcm1.pxx[index];
			pyt = fiy ? (int) astcm1.pyy[index] :
 ...
 */
{
	...
	XWarpPointer(disp, None, XtWindow(toplevel[window_num]), 0, 0, 0, 0,
		(int) *hs, fiy ? (int) *vs : (pixdata[window_num].y - (int) *vs));
	xptrco = (int) *hs;
	yptrco = fiy ? (int) *vs : (pixdata[window_num].y - (int) *vs);
}
 ...
 */
{
	...
	if (debug_mask[(int)('p'-'a')]) {
		printf("\n%d %d %d %d %d %d %d", (int)rootwin, (int)childwin, xrootco,
			yrootco, xwinco, ywinco, state);
		fflush(stdout);
	}

	*hs = (float) xwinco;
	*vs = (float) fiy ? ywinco : (pixdata[window_num].y - ywinco);
}
 ...
 */
{
	...
	grcon_val.foreground = grcon_val.background;
	XChangeGC(disp, grcon, GCForeground, &grcon_val);
	XFillRectangle(disp, drawin, grcon, 0, 0, pixdata[window_num].x,
		pixdata[window_num].y);
	grcon_val.foreground = temp_fg;
	XChangeGC(disp, grcon, GCForeground, &grcon_val);
	duiscc.nersdu = 0;
 ...
 */
{
	...
	x = (int) h1 + dtxth;
	temp = (int) v1 + dtxtv - (greekfontinfo->descent + greekfontinfo->ascent);
	y = fiy ? temp : (pixdata[window_num].y - temp);
	cout[0] = ' ';
	cout[1] = '\0';
/*
 ...
 */
	if (!strcmp(tlet, "PHI  ")) {
		...
		if (idir == 0) {
			if (fiy) {
				temp = 1;
				drawtextnonh(&h1, &v1, dtxth, dtxtv, (char *)"f", &temp, 1, 1);
			} else {
				grcon_val.font = greekfont;
				XChangeGC(disp, grcon, GCFont, &grcon_val);
				XDrawString(disp, drawin, grcon, x, y, (char *)"f", 1);
				grcon_val.font = mainfont;
				XChangeGC(disp, grcon, GCFont, &grcon_val);
			}
		} else {
			temp = 1;
			drawtextnonh(&h1, &v1, dtxth, dtxtv, (char *)"f", &temp, 1, 0);
		}
	} else
/*
 ...
 */
	if (!strcmp(tlet, "THETA")) {
		...
		if (idir == 0) {
			if (fiy) {
				temp = 1;
				drawtextnonh(&h1, &v1, dtxth, dtxtv, (char *)"q", &temp, 1, 1);
			} else {
				grcon_val.font = greekfont;
				XChangeGC(disp, grcon, GCFont, &grcon_val);
				XDrawString(disp, drawin, grcon, x, y, (char *)"q", 1);
				grcon_val.font = mainfont;
				XChangeGC(disp, grcon, GCFont, &grcon_val);
			}
		} else {
			temp = 1;
			drawtextnonh(&h1, &v1, dtxth, dtxtv, (char *)"q", &temp, 1, 0);
		}
	} else
/*
 ...
 */
	if (!strcmp(tlet, "RHO  ")) {
		...
		if (idir == 0) {
			if (fiy) {
				temp = 1;
				drawtextnonh(&h1, &v1, dtxth, dtxtv, (char *)"r", &temp, 1, 1);
			} else {
				grcon_val.font = greekfont;
				XChangeGC(disp, grcon, GCFont, &grcon_val);
				XDrawString(disp, drawin, grcon, x, y, (char *)"r", 1);
				grcon_val.font = mainfont;
				XChangeGC(disp, grcon, GCFont, &grcon_val);
			}
		} else {
			temp = 1;
			drawtextnonh(&h1, &v1, dtxth, dtxtv, (char *)"r", &temp, 1, 0);
		}
	}
 ...
 */
{
	static unsigned char curale_bits[2][32] = {
		{
	...
	static unsigned char curale_mask[2][32] = {
		{
	...
	static unsigned char currb_bits[4][32] = {
		{
	...
	static unsigned char currb_mask[4][32] = {
		{
			0x00,0x00,0x80,0x00,0x80,0x00,0x80,0x00,
			0x80,0x00,0x80,0x00,0x80,0x00,0x80,0x00,
			0xff,0x7f,0x80,0x00,0x80,0x00,0x80,0x00,
	...
	static unsigned char pntzer_bits[32] = {
		0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
		0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
		0x80,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
		0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00
	};
	static unsigned char pntzer_mask[32] = {
		0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
		0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
		0x80,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
		0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00
	...
	static unsigned char curpic_bits[2][32] = {
		{
	...
	static unsigned char curpic_mask[2][32] = {
		{
	...
	static unsigned char curpnt_bits[2][32] = {
		{
	...
	static unsigned char curpnt_mask[2][32] = {
		{
	...
	static unsigned char wtcpnt_bits[8][32] = {
		{
	...
	static unsigned char wtcpnt_mask[32] = {
	...
	static unsigned char wtcpnt_mono_bits[32] = {
...
CXX void dgtlnx_wk()
/*
 * Swap wait_for_keyinput
 */
{
	wait_for_keyinput = !wait_for_keyinput;
}
 ...
 */
{
	...
	while (index1 < *maxlen && !finished) {
		...
		while(!(evflag[0]|evflag[5])) {
			if (!wait_for_keyinput && (QLength(disp) == 0)) {
				xiosb[0] = 0;
				goto the_end;
			}
 ...
 */
	...
	if (debugd) XFlush(disp);
	fflush(stdout);
	while(!(evflag[0]||evflag[1]||evflag[2]||evflag[3]||evflag[4]||evflag[7])) {
		XtAppProcessEvent(Dali_context,XtIMAll);
	}
}
 ...
 */
	...
	for (y = 0; y < fontinfo->ascent + fontinfo->descent; y++) {
		...
		if (ip > nworkdw) {
			printf("\nBeware: Temporary working space allocated too small in drawtextnonh\n");
			fflush(stdout);
		}
		xco = (int) *htxt + y + xoffset;
		yco = fiy ? ((int) *vtxt + yoffset) :
			(pixdata[window_num].y - ((int) *vtxt + yoffset));
...
another_server:
	if (fiy && (nonv == 1)) goto invert_horisontal;
	for (y = 0; y < fontinfo->ascent + fontinfo->descent; y++) {
		for (x = 0; x < overall.width; x++) {
			if (XGetPixel(ximage, x, y) == 1) {
				xco = (int) *htxt + y + xoffset;
				yco = fiy ? ((int) *vtxt + x + yoffset) :
					(pixdata[window_num].y - ((int) *vtxt + x + yoffset));
				XDrawPoint(disp, drawin, grcon, xco, yco);
			}
		}
	}
...
invert_horisontal:
	for (y = 0; y < fontinfo->ascent + fontinfo->descent; y++) {
		for (x = 0; x < overall.width; x++) {
			if (XGetPixel(ximage, x, y) == 1) {
				xco = (int) *htxt + x + xoffset;
				yco = fiy ? ((int) *vtxt - y + yoffset) :
					(pixdata[window_num].y - ((int) *vtxt + y + yoffset));
				XDrawPoint(disp, drawin, grcon, xco, yco);
			}
		}
	}
 ...
 */
#ifdef ATLAS
	static char title[] = "ATLANTIS     dd mmm yyyy  version xx                        ";
#else
	static char title[] = "ALEPH-XDALI  dd mmm yyyy  version xx    X11/XUIT            ";
#endif
...
#ifdef ATLAS
	title[34] = dcftvt.tfildc[9];
	title[35] = dcftvt.tfildc[10];
#else
...
{
	time_t tnow = time((time_t *)NULL);
	struct tm *ts = localtime(&tnow);

	*i = ts->tm_mon + 1;
	*j = ts->tm_mday;
	*k = (ts->tm_year)%100; /* This gives the 2 last digits in the year */
}
...
static XtInputCallbackProc get_nonX_input(
	XtPointer cd,
	int *s,
	XtInputId *id
)
...
/*
 * Date routines. We make our own because of Y2K.
 */
CXX void date2(FortranString *buf)
{
	time_t tnow = time((time_t *)NULL);
	char *ts = ctime (&tnow);

	if (buf->dsc$w_length < 9) {
		printf("\ndate2 was called with argument less than 9 characters long.\n");
		exit(0);
	}
	strncpy((char *)buf->dsc$a_pointer, "dd-mmm-yy", 9);
	if (ts[8] == ' ') ts[8] = '0'; 
	strncpy((char *)buf->dsc$a_pointer,	&ts[8],  2);
	strncpy((char *)(buf->dsc$a_pointer+3), &ts[4],  3);
	strncpy((char *)(buf->dsc$a_pointer+7), &ts[22], 2);
}

CXX void date4(FortranString *buf)
{
	time_t tnow = time((time_t *)NULL);
	char *ts = ctime (&tnow);

	if (buf->dsc$w_length < 11) {
		printf("\ndate4 was called with argument less than 11 characters long.\n");
		exit(0);
	}
	strncpy((char *)buf->dsc$a_pointer,	"dd-mmm-yyyy", 11);
	if (ts[8] == ' ') ts[8] = '0'; 
	strncpy((char *)buf->dsc$a_pointer,		&ts[8],  2);
	strncpy((char *)(buf->dsc$a_pointer+3), &ts[4],  3);
	strncpy((char *)(buf->dsc$a_pointer+7), &ts[20], 4);
}
 ...
 * Date routines. We make our own because of Y2K.
 */
CXX void date2_(char *buf, int len)
{
	time_t tnow = time((time_t *)NULL);
	char *ts = ctime (&tnow);
...
CXX void date4_(char *buf, int len)
{
	time_t tnow = time((time_t *)NULL);
	char *ts = ctime (&tnow);
	...
	strncpy(buf, "dd-mmm-yyyy", 11);
	if (ts[8] == ' ') ts[8] = '0'; 
	strncpy(buf,        &ts[8],  2);
	strncpy(&buf[3],    &ts[4],  3);
	strncpy(&buf[7],   &ts[20],  4);
}

#endif /* VMS */
/*
 * Last, a few interface routines missing for g77. IBM of course use their
 * own definitions, so I prefer to use these routines also for AIX.
 * These are my own creations, and the following emulations are not well
 * tested yet.
 * (BSN, first time 25-Nov-1995).
 */
#if defined(__linux) || defined(_AIX)
CXX float ran_(void)
{
/*
 * Emulate ran. This ran does not return a seed and can therefore not be reset.
 */
	float ran4;
...
CXX float secnds_(float *tim0)
{
/*
 * This was secnds for CPU time, not elapsed time.
 */
/*
 *	clock_t time1;
 *	float CPUt, CPUu, CPUs;
 ...
 */
	...
	return ((float)tnow - *tim0);
}

CXX void sleep_(int *Itime)
{
	unsigned int secs;
...
#ifdef __linux
/*
 * Dummy funct routine needed by the Linux CERNlib arithm implementation. Why?
 */
CXX float funct_()
{
	printf("\n\nYou should never have dropped in funct_\nNow you better tell Bjorn S. Nilsson, it is a mistake.\n\n");
	exit(0);
	return 1.0;
}
#ifdef PRE_RH6
/*
 * From Anders Waananen. Catch floating point problems.
 * Switch by setting testfpu nonzero with the debugger
 */
#include <fpu_control.h>
/* static void __attribute__ ((constructor)) */
CXX void fpucw_ (int *testfpu)
{
	if (testfpu != 0)
		__setfpucw (0x1373);
/*		__setfpucw (0x1372); */
