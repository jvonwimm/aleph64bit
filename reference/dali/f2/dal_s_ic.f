...
C------------------------------------------------------------------- DC
      PARAMETER (MCLCDC=2000)
      COMMON /DCOLC/ FDLCDC,FVLCDC,NCLCDC(MCLCDC)
      LOGICAL FDLCDC,FVLCDC
C------------------------------------------------------------------- DC
...
C       21.2.2002: the 2 previous open statements were moved up, but the problem
C       that almost all output into BOSLO2.LOG is found in fort.90 in the case
C       of ABSOFT (!), remains. All is found in BOSLO2.LOG in the case of g77.
...
C
        ...
        CALL DQHL0
        CALL DQHLP('<-')
c       CALL DGOPEN( 0,'BOSLO1.LOG',5,*1,ISTAT)
c       CALL DGOPEN(90,'BOSLO2.LOG',5,*1,ISTAT)
        GO TO 2
...
C-------------------End of DALI_CF commons----------------
      ...
      CALL DPARGV( 85,'ONZ',2,DFWIDU(0))
      CALL DPARGV( 85,'OZO',2,DFWIDU(1))

      CALL DPARGV(104,'PLM',2,CCMIDU)
      CALL DPARGV(104,'PEM',2,ECMIDU)
      CALL DPARGV(104,'PHM',2,HCMIDU)
      CALL DPARGV(104,'PFO',2,DSOFDU)
      CALL DPARGV(104,'PTO',2,TPOFDU)
      CALL DPARGV(104,'PHC',2,HSTRDU)
      CALL DPARGV(104,'PRE',2,REALDU)
      CALL DPARGV(104,'PDZ',2,FTDZDU)
      CALL DPARGV(104,'PQZ',2,SHTFDU(1))
      CALL DPARGV(104,'PSH',2,SHTFDU(2))
      CALL DPARGV(104,'PVZ',2,VRDZDU)
      CALL DPARGV(104,'PAR',2,AREADU)
      CALL DPARGV(104,'PFL',2,DFLGDU)

      CALL DPARGV(105,'PSL',2,SLOWDU)

      CALL DPARGV(106,'HHD',2,HEADDU)

      CALL DPARGV(107,'ETF',2,CHTFDU)
      CALL DPARGV(104,'ECL',2,RNCLDU)
...
C------------------------------------------------------------------- DC
      PARAMETER (MCITDC=2000)
      COMMON /DCOIT/ FDITDC,FVITDC,NCITDC(MCITDC)
      LOGICAL FDITDC,FVITDC
C------------------------------------------------------------------- DC
      PARAMETER (MCLCDC=2000)
      COMMON /DCOLC/ FDLCDC,FVLCDC,NCLCDC(MCLCDC)
      LOGICAL FDLCDC,FVLCDC
C------------------------------------------------------------------- DC
      ...
      LOGICAL FDECDC,FVECDC
C------------------------------------------------------------------- DC
      PARAMETER (MCEODC=2000)
      COMMON /DCOEO/ FDEODC,FVEODC,NCEODC(MCEODC)
      LOGICAL FDEODC,FVEODC
C------------------------------------------------------------------- DC
      PARAMETER (MCHCDC=2000)
      COMMON /DCOHC/ FDHCDC,FVHCDC,NCHCDC(MCHCDC)
      COMMON /DCOHC1/ LCHCDC
