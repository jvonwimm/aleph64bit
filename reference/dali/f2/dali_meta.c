...
CXX int gethostname(char *__name, size_t __len);
#endif /* gethostname */
#if !defined(cuserid)
CXX char *cuserid(char *__name);
#endif /* cuserid */
#endif /* Not defined in Linux */
#include "commons.h"
...
FILE *fmeta;
/*
 * On VMS, tot_meta_chars sums the # bytes written.
 */
...
static int max_tot_meta_chars = 0;
...
void mprintf(char *line, int mnew);
void write_colors_to_meta(void);
void mdglevl(int *lev);
CXX void dgtint(int *integ, int *linteg);
 ...
 */
	...
	tot_meta_chars += fprintf(fmeta,
		"%%%%Comment: The following BoundingBox values are good for an A4 printer.\n");
	tot_meta_chars += fprintf(fmeta,
		"%%%%Comment: For inclusion in LaTeX etc, you may consider to change the\n");
	tot_meta_chars += fprintf(fmeta,
		"%%%%Comment: BoundingBox values to e.g. 0 0 756 531\n");
	tot_meta_chars += fprintf(fmeta,"%%%%BoundingBox: 41 35 572 791\n");
 ...
 * but after introduction of the 2-slide colour printer in Jan 1998
 * it had to be changed to:
 */
	tot_meta_chars += fprintf(fmeta,"572 35 translate\n");
 ...
 *		bound_x, bound_y);
 */
	tot_meta_chars += fprintf(fmeta,"%%%%BoundingBox: 0 0 1 1\n");
 ...
 */
	...
	fgetpos(fmeta,&ipos);
#endif /* vms */
	meta_marks_chars[number_of_meta_marks - 1] = fsetpos(fmeta,&ipos);
	if (*iprnt > 0) printf("\nMark number %d inserted at offset %d.",
			       number_of_meta_marks, meta_marks_chars[number_of_meta_marks - 1]);
}
 ...
 */
	unsigned int index;
#ifdef __VMS
	unsigned int tmet_l = tmet->dsc$w_length;
#endif /* VMS */
 ...
 */
	static int xsize = 991, ysize = 696;
...
{
/*
 * Output routine for PostScript metafile. If mnew == 1, start a new line.
