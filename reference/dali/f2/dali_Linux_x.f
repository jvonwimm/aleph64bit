...
CBSN     WRITE(TSCR1,'(I<I1>,A)') NUMMAC, ' macros with '
         WRITE(TFORM(3:3),'(I1)') I1
         WRITE(TSCR1,TFORM) NUMMAC, ' macros with '
         L2 = LENOCC(TSCR1)
         I1 = 1
         IF(KMAX.GT.9) I1 = 2
         IF(KMAX.GT.99) I1 = 3
CBSN     WRITE(TSCR1(L2+2:),'(I<I1>,A)')  KMAX,
         WRITE(TFORM(3:3),'(I1)') I1
         WRITE(TSCR1(L2+2:),TFORM)  KMAX,
...
C
     ...
     & TMSAVE(MACLEVMAX)
      ...
      INTEGER IW
      COMMON/BCS/IW(6)    ! BOS
      CHARACTER*8 TFORM1/'(A,I ,A)'/, TFORM2/'(I ,A)'/

C F77      NKEY=%LOC(NADDR)
      NKEY=LOC(NADDR)
...
CBSN        WRITE(TSCR,'(A,I<I>,A)') 'Macro learning is ON, ',IPNTDM-1,
            WRITE(TFORM1(5:5),'(I1)') I
            WRITE(TSCR,TFORM1) 'Macro learning is ON, ',IPNTDM-1,
     &       ' characters buffered'
            CALL DWRT(TSCR)
         END IF
...
CBSN        WRITE(TSCR,'(I<I>,A)') NCHAR, ' characters written so far '
            WRITE(TFORM2(3:3),'(I1)') I
            WRITE(TSCR,TFORM2) NCHAR, ' characters written so far '
            CALL DWR_ADD(TSCR)
         EndIf
...
C Next Linux
            TSTR=' '
            TSTR(2:)=ICHSTR(1:LCHSTR)
C Linux     CALL DWR_ADD(' '//ICHSTR(1:LCHSTR))
            CALL DWR_ADD(TSTR(1:1+LCHSTR))
            ICOM=ICOM+LCHSTR-1
         ELSE
...
C
     ...
     &     THEN
            CALL DGPESC(JCHSTR(1:MOD(INT(IOSB(4)),256)), LCHSTR, AMAC)
            LCHSTR=-LCHSTR
            L1=LENOCC(AMAC)
...
C
      ...
      END IF
    9 CONTINUE
      IF((ISTAT.NE.2).AND.(.NOT.FCANCL).AND.(.NOT.FMACDM))
...
C
      ...
      DATA ((ETAB(I1,I2),I1=256,316),I2=1,3)/
...
CBSN        WRITE(TSCR,'(I<I1>,A)') NRLINE, ' lines with'
            WRITE(TFORM(3:3),'(I1)') I1
            WRITE(TSCR,TFORM) NRLINE, ' lines with'
            L2 = LENOCC(TSCR)
            I1 = 1
            IF(NUMMAC.GT.9) I1 = 2
            IF(NUMMAC.GT.99) I1 = 3
CBSN        WRITE(TSCR(L2+2:),'(I<I1>,A)')  NUMMAC,
            WRITE(TFORM(3:3),'(I1)') I1
            WRITE(TSCR(L2+2:),TFORM)  NUMMAC,
...
C Next Linux
      TSTR='Macro '
      TSTR(7:)=AMAC(1:L1)
      TSTR(7+L1:)=' is not defined.'
C Linux CALL DWRT('Macro '//AMAC(1:L1)//' is not defined.')
      CALL DWRT(TSTR)
      FNTADM=.FALSE.
...
C
      ...
      Character FullName*120, DFName*120, Tstr*120
      Integer LenOcc
...
C
      FullName = DFName(FileName)
C      Open(Unit=Nunit,File=FullName,Status='OLD',Err=210,READONLY,
      Open(Unit=Nunit,File=FullName,Status='OLD',Err=210,
     & IOstat=Istat)
      GoTo 230
C
...
C      FullName = DFName('DAL:'//FileName)
      Tstr = 'DAL:'
      Tstr(5:) = FileName
      FullName = DFName(Tstr)
C      Open(Unit=Nunit,File=FullName,Status='OLD',Err=220,READONLY,
      Open(Unit=Nunit,File=FullName,Status='OLD',Err=220,
     & IOstat=Istat)
      GoTo 230
  220 Return 1
...
C
      FullName = DFName(FileName)
C F77      Open(Unit=Nunit,File=FullName,Status='OLD',Access='APPEND',
      Open(Unit=Nunit,File=FullName,Status='OLD',
...
C
      Implicit None
      Character T72*72
      Integer Istat, estat, tty_reset, tty_set
...
C
C Init a few things that g77 cannot do on Linux yet.
C
      LSCRN=0
      KMAX=0
      MACLEV=0
      MACFIL=' '
      TPLFD0 = 'LINUX'   ! It may not be known yet
      TWRTD0 = CHAR(10)
      CALL SCRSIZE(LSCRN,WSCRN)
C End extra Linux init
C
C  This is the first kludge. DEC Fortran opens stdout with LIST carriage
C  control:-(
C
C      OPEN(Unit=MUN6DU,CarriageControl='FORTRAN',Status='UNKNOWN')
C      OPEN(Unit=MUN6DU,Status='UNKNOWN')
      OPEN(Unit=MUN6DU)
C     OSF/1
C
...
CH
    ...
    2 NFile = NFile + 1
     ...
     & ERR=2)
C     & CarriageControl='FORTRAN',ERR=2)
      WRITE(MUN6DU,'(A)')
      WRITE(MUN9DU,'(A)')
      CALL DWRT('Output on file '//File_Name(1:12))
      WRITE(MUN6DU,'(A)') CHAR(10)
      WRITE(MUN9DU,'(A)') CHAR(10)
      RETURN
...
C========================================================================
      ...
      Function SIND(XD)
C  Sin with argument in degrees.
      DATA DToR/.01745329252/
      SIND = SIN(XD*DToR)
      Return
      End
      Function COSD(XD)
C  Cos with argument in degrees.
      DATA DToR/.01745329252/
      COSD = COS(XD*DToR)
      Return
      End
      Function TAND(XD)
C  Tan with argument in degrees.
      DATA DToR/.01745329252/
      TAND = TAN(XD*DToR)
      Return
      End
      Function ACOSD(ARG)
C  Acos with result in degrees.
      DATA RToD/57.2957795/
      ACOSD = ACOS(ARG)*RToD
      Return
      End
      Function ATAND(ARG)
C  Atan with result in degrees.
      DATA RToD/57.2957795/
      ATAND = ATAN(ARG)*RToD
      Return
      End
      Function ATAN2D(ARG1,ARG2)
C  Atan2 with result in degrees.
      DATA RToD/57.2957795/
      ATAN2D = ATAN2(ARG1,ARG2)*RToD
      Return
      End
