...
C      DATA J3/3/
      LOGICAL FNOFI
      CALL DO_BAR_ANSWER_PLATFORM_TEXT('fi')
...
CC         ....................................................... Specify run.
          CALL DGSWPT(-5)
          CALL DO_BAR_ANSWER_PLATFORM_TEXT('fe')
          CALL DWRT(
...
C ---------------------------------------------------------------------
 ...
 1000 FORMAT(A)
    ...
    2 READ(NUNIT,1000,END=90) TXTADW
      ...
      CALL DW_GET_PLATFORM_TEXT('ED',TFIL,55)
      LFIL=LENOCC(TFIL)+1
...
c      CALL DO_STR('RE"DE: read (and display) single event.')
c      IF(TANSW.EQ.'BI'.OR.TANSW.EQ.'DI') THEN
c        CALL DJ_READ_SINGLE_EVENT(PR1)
c        IF(TANSW(1:1).EQ.'D') CALL DDISPP
c        GO TO 930
c      END IF
      ...
      END IF
C      CALL DO_STR('LB')
C      IF(TANSW.EQ.'LB') THEN
C        CALL DJLBNK(MUN6DU,MUN9DU)
C        GO TO 930
C      END IF
C      CALL DO_STR('LI')
C      IF(TANSW.EQ.'LI') THEN
C        CALL DJDRLI
C        GO TO 930
C      END IF
      CALL DO_STR('FI')
     ...
     &    //DT6(FLOAT(IRUNDE(1)))//' stored.')
      ...
      CALL DO_STR('SI:  store IW')
      IF(TANSW.EQ.'SI') THEN
        CALL DV_STORE_IW
        GO TO 930
      END IF
      CALL DO_STR('RI:  read IW')
      IF(TANSW.EQ.'RI') THEN
        CALL DV_read_IW
        GO TO 930
      END IF
...
C     DATA NTIM/7/
    ...
    2 WRITE(TXTADW,1011)
     ...
     &  ,TMP(NDO)
      ...
      IF(JWF.EQ.1) THEN
        CALL DJ_FILE_EDIR_NAME(40,TENAM)
        LENAM=LENOCC(TENAM)
        WRITE(81,2011) TENAM//'  :'
 2011   FORMAT('============== ',A,' ==============')
        WRITE(81,2010) TXTADW(1:55)//'  :'
 2010   FORMAT(1X,A)
      END IF
      CALL DWRC
...
C     ................................................................ LI
  ...
  920 IF(TANSW.EQ.'LE') THEN
     ...
     &    ,TMP(NDO)
      ...
      IF(TANSW.EQ.'WF') THEN
        JWF=-JWF
        IF(JWF.EQ.1) THEN
          CALL DWRT('Event file FOR081.DAT opened')
          JLIN=1
          GO TO 2
        ELSE
          CLOSE(UNIT=81)
          CALL DWRT('Event file FOR081.DAT closed')
          RETURN
        END IF
      END IF
  ...
  940 NTIM=PR2(2,3)
      ...
      DO K=1,JTOT
        ...
        END IF
        IF(JWF.EQ.1) WRITE(81,2010) TXTADW(1:55)//'  :'
        CALL  DWRC
  ...
  943   JLIN=JLIN+NDO
     ...
     &          TMP(NDO)
          ...
          END IF
        END IF
        IF(JTIM.GE.NTIM.AND.JWF.NE.1) GO TO 936
      END DO
