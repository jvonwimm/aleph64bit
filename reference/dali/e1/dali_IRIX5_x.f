...
C
    ...
    9 IF((ISTAT.NE.2).AND.(.NOT.FCANCL).AND.(.NOT.FMACDM))
     & WRITE(MUN9DU,'(A,$)') ICHSTR(1:LCHSTR)  ! SGI
      ICHSTR(LCHSTR+1:) = ' '
...
CSGI  Character FullName*120, DFName*120
      Character FullName*120, DFName*120, Str1*132, Str2*132
      Integer LenOcc
...
C
C  This looks like a compiler bug. We crash here on SGI, but not on
C  other machines. The expanded code seems to work. BSN, 5-Oct-1995
C
CSGI  FullName = DFName('DAL:'//FileName)
      Str1 = 'DAL:'//FileName
      Str2 = DFName(Str1)
      FullName = Str2
CSGI
...
C-------------------------------------------------------------
     ...
     & HPOPOS,VPOPOS
      ...
      MACLEV=0
      MACFIL=' '
      TPLFD0 = 'SGI'   ! It may not be known yet
      TWRTD0 = CHAR(10)
...
CH
    ...
    2 NFile = NFile + 1
      ...
      OPEN(UNIT=MUN9DU,FILE=File_Name,STATUS='NEW',ERR=2)
...
C   CTRL-Z. 
      ...
      WRITE(MUN9DU,'(A)') ILNSTR(1:LCHLIN)                   ! SGI
