...
C     :::::::::::::::::::::::::::::::::::::::::::::::::::::::
      TPARDA=
     &  'J_PTE,J_PIT,J_PFR,J_PTO,J_PAS,J_PDS,J_PHM,J_PDH'
      CALL DPARAM(10
     &  ,J_PTE,J_PIT,J_PFR,J_PTO,J_PAS,J_PDS,J_PHM,J_PDH)
      TPARDA=
     &  'J_CFL'
...
C         SIGNED RHO
          FSR=.TRUE.
C          IF(IAREDO.EQ.0) THEN
C            CALL DQRER(3,-RTO,-RTO,RTO,RTO,HRB,VRB)
C          ELSE
C            CALL DQRER(0,-RTO,-RTO,RTO,RTO,HRB,VRB)
C          END IF
...
C     ....................... FC = TRUE only at last step of iteration
      ...
      ELSE
         VM=DHELIX(FM,IVRODV)
      END IF
      IF(SIGN(1.,V1).NE.SIGN(1.,V2)) THEN
         D=0.
         FC=.TRUE.
         RETURN
      END IF
...
C     ::.::::::::::::::::::::::::::::::::::::::::::::::::::::
      TE=PARADA(2,J_PTE)
      IF(PARADA(4,J_PFI).EQ.-1.) TE=360.-TE
      ST=SIND(TE)
      CT=COSD(TE)
      R1=PARADA(2,J_PFR)
      R2=PARADA(2,J_PTO)
      ...
      IF(PARADA(2,J_PBM).LT.5.) THEN
         D2=0.5*(R2-R1)
         TM=MOD(TE+3600.,360.)
         IF(TM.LT.40..OR.TM.GT.320.) THEN
            HM=RM
            VM=HM*ST/CT
         ELSE IF(TM.LE.140.) THEN
            VM=RM
            HM=VM*CT/ST
         ELSE IF(TM.LT.220.) THEN
            HM=-RM
            VM=HM*ST/CT
         ELSE
            VM=-RM
      ...
      ELSE
         ...
         ELSE
            ...
            H34=V34*CT/ST
         END IF
         R=SQRT((H34-H12)**2+(V34-V12)**2)
         Q=1./MAX(ABS(ST),0.018)
         IF(PARADA(2,J_PAS).EQ.0.) THEN
            D2=0.5*R*Q
         ELSE
            D2=0.5*R*Q/PARADA(2,J_PAS)
         END IF
         HRB(1)=H12-D2
         HRB(2)=H12+D2
         HRB(3)=H34+D2
