...
C
C   This is a UNIX version first prepared on 2-Feb-1993 on then tried
C   out on HP on 19-Aug-1994. For HP, compile with
C   f77 +e -K +U77 +ppu -v -o ic ic.f
C
...
C             requirements!
C             It is possible  that the here given limits will once be
C             too low! So ajust these PARAMETERs here AND in all sub-
C             routines and functions when needed.
C

      INTEGER max_lin_rout, max_lin_dcf, max_com_bl
      PARAMETER (max_lin_rout = 3000)
      PARAMETER (max_lin_dcf  =  800)
...
C
     ...
     &                    lu_file_out
      ...
      DATA lu_file_in  / 21 /
      DATA lu_file_out / 22 /
C      INTEGER Istat, LIB$Get_Foreign, Len_Str, Flag, I1, I2,
C     & Status
      INTEGER iargc, Flag, Status
      INTEGER Ilen
...
C  label variables
C
      INTEGER dcf_open_err, inf_open_err, outf_open_err, exit_prog,
     &        eof_dali_cf , eof_file_in , next_rout , next_line
...
C
     ...
     &             com_bl_lin (max_com_bl), numb_com_bl
      ...
      Assign 999 to exit_prog
C
C   Get filenames from command line.
C
      Flag  = 0
C      Istat = LIB$Get_Foreign(Arg_Str, ' ', Len_Str, Flag)
C      If (Istat .NE. 1) Then
C        Write(*,'(a)') ' Incorrect program call.'
C        Goto exit_prog
C       Endif
C
C      I1 = Index(Arg_Str(1:Len_Str), ' ')
C      If (I1 .LE. 1) Then
C        Write(*,'(a)') ' Incorrect program call.'
C        Goto exit_prog
C       Endif
C      File1 = Arg_Str(1:I1-1)
C
C      I2 = I1 + 1
C      I1 = Index(Arg_Str(I2:Len_Str), ' ')
C      If (I1 .LE. 1) Then
C        Write(*,'(a)') ' Incorrect program call.'
C        Goto exit_prog
C       Endif
C      File2 = Arg_Str(I2:I2+I1-2)
C
C      If (I1+I2 .GT. Len_Str) Then
C        Write(*,'(a)') ' Incorrect program call.'
C        Goto exit_prog
C       Endif
C      File3 = Arg_Str(I2+I1:Len_Str)
...
C
      ...
      Assign 910 to dcf_open_err
      Assign 920 to inf_open_err
      Assign 920 to outf_open_err
      Assign 199 to eof_dali_cf
      Assign 299 to eof_file_in

      Assign 130 to next_rout
      Assign 140 to next_line
...
C
      ...
      Open (unit=lu_dali_cf, status='OLD', READONLY, File = File1,
  ...
  100 Continue                          ! next line of DALI_CF.INC
     ...
     &     cols_dali_cf(rows_dali_cf), text_dali_cf(rows_dali_cf)
  ...
  199 Continue                          ! eof_dali_cf
      ...
      Open (unit=lu_file_in, status='OLD', READONLY, File = File2,
     &       err = 920)
      Open (unit=lu_file_out, status='UNKNOWN', File = File3,
     &       err = 930)
C HP &       carriagecontrol='LIST',  err = 930)
      LInclude = .True.
      Inline = 0
  ...
  140 Continue                          ! read next line
     ...
     &     cols_rout(rows_rout), text_rout(rows_rout)
      Inline = Inline + 1
...
C
  ...
  940 Write(lu_message, '(a,i5,a)') ' Increase max_com_bl (',
     & max_com_bl, ' at present).'
      IPstat = 1
      Goto exit_prog

  950 Write(lu_message, '(a,i5,a)') ' Increase max_lin_dcf (',
     & max_lin_dcf, ' at present).'
      IPstat = 1
      Goto exit_prog

  960 Write(lu_message, '(a,i5,a)') ' Increase max_lin_rout (',
     & max_lin_rout, ' at present).'
      IPstat = 1
      Goto exit_prog
...
C -
      ...
      INTEGER   nrw
      PARAMETER (nrw = 32)
      CHARACTER*12 resw(nrw)

      DATA resw /'CHARACTER','DIMENSION','INTEGER','FORMAT','ACCEPT',
     &           'ACCESS','ALLOCATE','PARAMETER','ASSIGN','REWIND',
     &           'BACKSPACE','LOGICAL','EXTERNAL','INTRINSIC','RETURN',
     &           'EQUIVALENCE','COMMON','COMPLEX','CONTINUE','DELETE',
     &           'DISPOSE','DOUBLE','ELSEIF','ENDFILE','STRUCTURE',
     &           'UNLOCK','INCLUDE','IMPLICIT','REWRITE','OPTIONS',
     &           'SUBROUTINE','FUNCTION'/
...
C
      ...
      Character*(*) string
      Integer LineNo
      Character*8 Cond_Ident(NIdents)
     . /'UNIX    ', 'HP     ', 'ULTRIX  '/, TC
      Integer I, L1, L2
      Integer I2
      ...
      If (string(1:6) .eq. '#endif') Then
        If (.NOT. LPhase) Then
          Write(*,'(a,i5)') ' Incorrect nesting, unexpected '//
  ...
  110   LPhase = .True.
      ...
      Write(*,'(a,i5)') ' Incorrect keyword '//string(1:I2)//
     .' in line ', LineNo
      Call Exit(1)
